...
- **Code Sovereignty**: External models have **zero filesystem write access**, all modifications by Droid
- **Stop-Loss Mechanism**: Do not proceed to next phase until current phase output is validated
- **Planning Only**: This command allows reading context and writing to `.factory/plan/*` plan files, but **NEVER modify production code**
...
```
Bash({
  command: "~/.factory/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}- \"$PWD\" <<'EOF'
...
| Phase | Codex | Gemini |
|-------|-------|--------|
| Analysis | `~/.factory/.ccg/prompts/codex/analyzer.md` | `~/.factory/.ccg/prompts/gemini/analyzer.md` |
| Planning | `~/.factory/.ccg/prompts/codex/architect.md` | `~/.factory/.ccg/prompts/gemini/architect.md` |
...
**If ace-tool MCP is NOT available**, use Factory Droid built-in tools as fallback:
...
1. **Codex Backend Analysis**:
   - ROLE_FILE: `~/.factory/.ccg/prompts/codex/analyzer.md`
...
2. **Gemini Frontend Analysis**:
   - ROLE_FILE: `~/.factory/.ccg/prompts/gemini/analyzer.md`
...
1. **Codex Plan Draft** (Backend authority):
   - ROLE_FILE: `~/.factory/.ccg/prompts/codex/architect.md`
...
2. **Gemini Plan Draft** (Frontend authority):
   - ROLE_FILE: `~/.factory/.ccg/prompts/gemini/architect.md`
...
2. Save plan to `.factory/plan/<feature-name>.md` (extract feature name from requirement, e.g., `user-auth`, `payment-module`)
3. Output prompt in **bold text** (MUST use actual saved file path):

---
**Plan generated and saved to `.factory/plan/actual-feature-name.md`**
...
```
/ccg:execute .factory/plan/actual-feature-name.md
```
---
...
- **First planning**: `.factory/plan/<feature-name>.md`
- **Iteration versions**: `.factory/plan/<feature-name>-v2.md`, `.factory/plan/<feature-name>-v3.md`...
...
2. Update `.factory/plan/<feature-name>.md` file
...
```bash
/ccg:execute .factory/plan/<feature-name>.md
```
