...
```bash
mkdir -p .factory/PRPs/reports
```

Write report to `.factory/PRPs/reports/{plan-name}-report.md`:
...
```bash
mkdir -p .factory/PRPs/plans/completed
mv "$ARGUMENTS" .factory/PRPs/plans/completed/
```
...
### Artifacts
- Report: `.factory/PRPs/reports/{name}-report.md`
- Archived Plan: `.factory/PRPs/plans/completed/{name}.plan.md`
