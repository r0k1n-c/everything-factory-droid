---
description: Load the most recent session file from ~/.factory/session-data/ and resume work with full context from where the last session ended.
---
...
- Any time you have a session file and want Droid to fully absorb it before proceeding

## Usage

```
/resume-session                                                      # loads most recent file in ~/.factory/session-data/
/resume-session 2024-01-15                                           # loads most recent session for that date
/resume-session ~/.factory/session-data/2024-01-15-abc123de-session.tmp  # loads a current short-id session file
/resume-session /absolute/path/to/2024-01-15-session.tmp                 # loads a specific session file by path
```
...
1. Check `~/.factory/session-data/`
2. Pick the most recently modified `*-session.tmp` file
3. If the folder does not exist or has no matching files, tell the user:
   ```
   No session files found in ~/.factory/session-data/
...
- If it looks like a date (`YYYY-MM-DD`), search `~/.factory/session-data/` for files matching
  `YYYY-MM-DD-session.tmp` (legacy filename format) or `YYYY-MM-DD-<shortid>-session.tmp` (current format)
...
```
SESSION LOADED: /Users/you/.factory/session-data/2024-01-15-abc123de-session.tmp
════════════════════════════════════════════════
