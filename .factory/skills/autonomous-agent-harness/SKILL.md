...
Turn Factory Droid into a persistent, self-directing agent system using only native features and MCP servers.
...
Use Factory Droid's built-in memory system enhanced with MCP memory server for structured data.

**Built-in memory** (`~/.factory/projects/*/memory/`):
...
Write to ~/.factory/projects/*/memory/ for cross-session recall
...
Use Factory Droid's scheduled tasks to create recurring agent operations.
...
# Via droid exec (programmatic mode)
echo "Review open PRs and summarize" | droid exec --cwd /path/to/repo
```
...
Trigger Factory Droid agents remotely for event-driven workflows.
...
# GitHub webhook → dispatch → Droid agent → fix → PR

# Trigger from another agent
droid exec "Analyze the output of the security scan and create issues for findings"
```
...
Write task queue to ~/.factory/projects/*/memory/task-queue.md
...
| Hermes Component | EFD Equivalent | How |
|------------------|---------------|-----|
| Gateway/Router | Factory Droid dispatch + crons | Scheduled tasks trigger agent sessions |
...
| Orchestration | EFD skills + agents | Skill definitions direct agent behavior |
| Computer Use | computer-use MCP | Native browser and desktop control |
| Context Manager | Session management + memory | EFD 2.0 session lifecycle |
...
Ensure these are in `~/.factory/settings.json`:
...
droid exec "Create a scheduled task: every weekday at 9am, review my GitHub notifications, open PRs, and calendar. Write a morning briefing to memory."

# Continuous learning
droid exec "Create a scheduled task: every Sunday at 8pm, extract patterns from this week's sessions and update the learned skills."
```
...
droid exec "Create memory entities for: me (user profile), my projects, my key contacts. Add observations about current priorities."
```
