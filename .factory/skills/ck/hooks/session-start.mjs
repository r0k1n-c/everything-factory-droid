 ...
 * - Goal mismatch detection vs AGENTS.md (with CLAUDE.md fallback)
...
import { resolveRegisteredProject } from '../commands/shared.mjs';

const CK_HOME         = resolve(homedir(), '.factory', 'ck');
const CURRENT_SESSION = resolve(CK_HOME, 'current-session.json');
const SKILL_FILE      = resolve(homedir(), '.factory', 'skills', 'ck', 'SKILL.md');
...
function extractProjectGoal(projectPath) {
  for (const filename of ['AGENTS.md', 'CLAUDE.md']) {
    const p = resolve(projectPath, filename);
    if (!existsSync(p)) continue;
    try {
      const md = readFileSync(p, 'utf8');
      const m = md.match(/## Current Goal\n([\s\S]*?)(?=\n## |$)/);
      if (m) {
        return { goal: m[1].trim().split('\n')[0].trim(), source: filename };
      }
    } catch { /* try next file */ }
  }
  return null;
...
function main() {
  ...
  if (entry?.contextDir) {
    ...
    if (context) {
      ...
      if (projectGoal?.goal && context.goal &&
          projectGoal.goal.toLowerCase().trim() !== context.goal.toLowerCase().trim()) {
        summaryLines.push(`WARNING Goal mismatch — ck: "${context.goal.slice(0, 40)}" · ${projectGoal.source}: "${projectGoal.goal.slice(0, 40)}"`);
