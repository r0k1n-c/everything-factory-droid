...
Use this skill when you need to dispatch multiple Factory Droid agents to work on coding tasks in parallel. Each agent runs in an isolated git worktree with full tooling.

Requires a running Claude DevFleet instance connected via MCP:
```bash
droid mcp add devfleet --transport http http://localhost:18801/mcp
```
