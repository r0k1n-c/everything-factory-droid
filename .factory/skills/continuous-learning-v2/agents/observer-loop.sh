...
# parallel Droid analysis processes.
...
ANALYSIS_COOLDOWN="${EFD_OBSERVER_ANALYSIS_COOLDOWN:-60}"
IDLE_TIMEOUT_SECONDS="${EFD_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800}"
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${EFD_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping droid analysis on Windows due to known non-interactive hang issue (#295). Set EFD_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi

  if ! command -v droid >/dev/null 2>&1; then
    echo "[$(date)] droid CLI not found, skipping analysis" >> "$LOG_FILE"
    return
  fi
  ...
  MAX_ANALYSIS_LINES="${EFD_OBSERVER_MAX_ANALYSIS_LINES:-500}"
  observer_tmp_dir="${PROJECT_DIR}/.observer-tmp"
  mkdir -p "$observer_tmp_dir"
  analysis_file="$(mktemp "${observer_tmp_dir}/efd-observer-analysis.XXXXXX.jsonl")"
  ...
  # prefixes (e.g. /c/Users/...) that the Droid subprocess cannot resolve.
  analysis_relpath=".observer-tmp/$(basename "$analysis_file")"

  prompt_file="$(mktemp "${observer_tmp_dir}/efd-observer-prompt.XXXXXX")"
  cat > "$prompt_file" <<PROMPT
IMPORTANT: You are running in non-interactive droid exec mode. You MUST use the Write tool directly to create files. Do NOT ask for permission, do NOT ask for confirmation, do NOT output summaries instead of writing. Just read, analyze, and write.
...
PROMPT
  ...
  timeout_seconds="${EFD_OBSERVER_TIMEOUT_SECONDS:-120}"
  max_turns="${EFD_OBSERVER_MAX_TURNS:-10}"
  exit_code=0
  ...
  # Pass prompt via exec subcommand instead of stdin redirect for Windows compatibility (#842).
  EFD_SKIP_OBSERVE=1 EFD_HOOK_PROFILE=minimal droid exec --model claude-haiku-4-5-20251001 \
  ...
  (
    ...
    if kill -0 "$droid_pid" 2>/dev/null; then
      echo "[$(date)] Droid analysis timed out after ${timeout_seconds}s; terminating process" >> "$LOG_FILE"
  ...
  if [ "$exit_code" -ne 0 ]; then
    echo "[$(date)] Droid analysis failed (exit $exit_code)" >> "$LOG_FILE"
  fi
