...
HOMUNCULUS_DIR = Path.home() / ".factory" / "homunculus"
...
def _ensure_global_dirs():
    for d in [GLOBAL_PERSONAL_DIR, GLOBAL_INHERITED_DIR,
              GLOBAL_EVOLVED_DIR / "skills", GLOBAL_EVOLVED_DIR / "commands", GLOBAL_EVOLVED_DIR / "droids",
              PROJECTS_DIR]:
...
def _same_project_root(left: Optional[str], right: Optional[str]) -> bool:
    return bool(left) and bool(right) and os.path.abspath(left) == os.path.abspath(right)


def _same_project_parent(left: Optional[str], right: Optional[str]) -> bool:
    return bool(left) and bool(right) and os.path.dirname(os.path.abspath(left)) == os.path.dirname(os.path.abspath(right))
...
def _hash_project_id(value: str) -> str:
    return hashlib.sha256(value.encode()).hexdigest()[:12]


def _legacy_project_ids(project_root: str, remote_url: str) -> set[str]:
    stripped_remote = re.sub(r"://[^@]+@", "://", remote_url or "")
    candidates = {_hash_project_id(project_root)}
    if stripped_remote:
        candidates.add(_hash_project_id(stripped_remote))
    if remote_url:
        candidates.add(_hash_project_id(remote_url))
    return candidates


def _read_project_metadata(project_id: str) -> Optional[dict]:
...
def _project_id_belongs_to_root(project_id: str, project_root: str, registry: dict) -> bool:
    entry = registry.get(project_id)
    if isinstance(entry, dict) and _same_project_root(entry.get("root"), project_root):
        return True

    metadata = _read_project_metadata(project_id)
    return bool(metadata and _same_project_root(metadata.get("root"), project_root))


def _project_id_is_taken(project_id: str, project_root: str, registry: dict) -> bool:
    if _project_id_belongs_to_root(project_id, project_root, registry):
        return False
    ...
    metadata = _read_project_metadata(project_id)
    return bool(metadata and not _same_project_root(metadata.get("root"), project_root))


def _is_rename_candidate_project(record: dict, project_root: str, remote_url: str) -> bool:
    root = record.get("root", "")
    if not root or _same_project_root(root, project_root):
        return False
    if not _same_project_parent(root, project_root):
        return False
...
def _merge_project_tree(source_dir: Path, target_dir: Path) -> None:
    target_dir.mkdir(parents=True, exist_ok=True)

    for child in source_dir.iterdir():
        target_child = target_dir / child.name
        if child.is_dir() and not child.is_symlink():
            _merge_project_tree(child, target_child)
        elif not target_child.exists():
            try:
                os.replace(child, target_child)
            except OSError:
...
def _resolve_project_storage(project_name: str, project_root: str, remote_url: str) -> tuple[str, list[str]]:
    registry = load_registry()
    prior_ids = {
        pid for pid, entry in registry.items()
        if isinstance(entry, dict) and _same_project_root(entry.get("root"), project_root)
    }

    if PROJECTS_DIR.exists():
        for project_dir in PROJECTS_DIR.iterdir():
            if project_dir.is_dir() and _project_id_belongs_to_root(project_dir.name, project_root, registry):
                prior_ids.add(project_dir.name)

    rename_candidates: set[str] = set()
    for pid, entry in registry.items():
        if isinstance(entry, dict) and _is_rename_candidate_project(entry, project_root, remote_url):
            rename_candidates.add(pid)

    if PROJECTS_DIR.exists():
        for project_dir in PROJECTS_DIR.iterdir():
            if not project_dir.is_dir():
                continue
            metadata = _read_project_metadata(project_dir.name) or {}
            metadata.setdefault("id", project_dir.name)
            if _is_rename_candidate_project(metadata, project_root, remote_url):
                rename_candidates.add(project_dir.name)

    if len(rename_candidates) == 1:
        prior_ids.update(rename_candidates)

    prior_ids.update(_legacy_project_ids(project_root, remote_url))

    base_project_id = _sanitize_project_id(project_name)
    project_id = base_project_id
    suffix = 2
    while _project_id_is_taken(project_id, project_root, registry) and project_id not in prior_ids:
    ...
    previous_ids = sorted(pid for pid in prior_ids if pid and pid not in {"global", project_id})
    for previous_id in previous_ids:
        previous_dir = PROJECTS_DIR / previous_id
        if previous_dir.exists():
            _merge_project_tree(previous_dir, target_dir)

    return project_id, previous_ids
...
def detect_project() -> dict:
    ...
    project_id, previous_ids = _resolve_project_storage(project_name, project_root, raw_remote_url or remote_url)
    ...
    for d in [
        ...
        project_dir / "evolved" / "droids",
    ]:
        d.mkdir(parents=True, exist_ok=True)

    # Update registry
    _update_registry(project_id, project_name, project_root, remote_url, aliases_to_remove=previous_ids)
...
def _update_registry(pid: str, pname: str, proot: str, premote: str, aliases_to_remove: Optional[list[str]] = None) -> None:
    ...
    try:
        ...
        created_at = now
        alias_ids = set(aliases_to_remove or [])

        for existing_id in list(registry.keys()):
            entry = registry.get(existing_id) or {}
            if existing_id == pid or existing_id in alias_ids or _same_project_root(entry.get("root"), proot):
                created_at = entry.get("created_at", created_at)
                if existing_id != pid:
                    registry.pop(existing_id, None)
...
def cmd_evolve(args) -> int:
    ...
    # Droid candidates (complex multi-step patterns)
    droid_candidates = [c for c in skill_candidates if len(c['instincts']) >= 3 and c['avg_confidence'] >= 0.75]
    if droid_candidates:
        print(f"\n## DROID CANDIDATES ({len(droid_candidates)})\n")
        for cand in droid_candidates[:3]:
            droid_name = cand['trigger'].replace(' ', '-')[:20] + '-droid'
            print(f"  {droid_name}")
    ...
    if args.generate:
        ...
        generated = _generate_evolved(skill_candidates, workflow_instincts, droid_candidates, evolved_dir)
        if generated:
...
def cmd_projects(args) -> int:
    ...
    if not registry:
        ...
        print("Projects are auto-detected when you use Factory Droid in a git repo.")
        return 0
...
def _generate_evolved(skill_candidates: list, workflow_instincts: list, droid_candidates: list, evolved_dir: Path) -> list[str]:
    """Generate skill/command/droid files from analyzed instinct clusters."""
    generated = []
    ...
    # Generate droids from complex clusters
    for cand in droid_candidates[:3]:
        trigger = cand['trigger'].strip()
        droid_name = re.sub(r'[^a-z0-9]+', '-', trigger.lower()).strip('-')[:20]
        if not droid_name:
            continue

        droid_file = evolved_dir / "droids" / f"{droid_name}.md"
        domains = ', '.join(cand['domains'])
        instinct_ids = [i.get('id', 'unnamed') for i in cand['instincts']]

        content = f"---\nmodel: claude-sonnet-4-6\ntools: Read, Grep, Glob\n---\n"
        content += f"# {droid_name}\n\n"
        content += f"Evolved from {len(cand['instincts'])} instincts "
        content += f"(avg confidence: {cand['avg_confidence']:.0%})\n"
        ...
        droid_file.write_text(content, encoding="utf-8")
        generated.append(str(droid_file))
