"""Run scenarios via droid exec and parse tool calls from stream-json output."""
...
CLI_BIN = os.environ.get("SKILL_COMPLY_CLI", "droid")
...
def run_scenario(
    ...
    result = subprocess.run(
        [
            CLI_BIN, "exec",
            "--model", model,
            "--enabled-tools", "Read,Write,Edit,Bash,Glob,Grep",
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"{CLI_BIN} exec failed (rc={result.returncode}): {result.stderr[:500]}"
        )
...
def _parse_stream_json(stdout: str) -> list[ObservationEvent]:
    """Parse droid exec stream-json output into ObservationEvents.
