...
capabilities: [communication, triage, email, messaging]
...
- **Rules are system-injected**: `.factory/rules/*.md` files load automatically every session. Unlike prompt instructions, the LLM cannot choose to ignore them.

## Example Invocations

```bash
droid /mail                    # Email-only triage
droid /slack                   # Slack-only triage
droid /today                   # All channels + calendar + todo
droid /schedule-reply "Reply to Sarah about the board meeting"
```

## Prerequisites

- [Factory Droid](https://docs.factory.ai/cli/getting-started/overview.md)
