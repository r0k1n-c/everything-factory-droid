...
Bash({
  command: "~/.factory/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}resume <SESSION_ID> - \"$PWD\" <<'EOF'
...
| Phase | Codex | Gemini |
|-------|-------|--------|
| Implementation | `~/.factory/.ccg/prompts/codex/architect.md` | `~/.factory/.ccg/prompts/gemini/frontend.md` |
| Review | `~/.factory/.ccg/prompts/codex/reviewer.md` | `~/.factory/.ccg/prompts/gemini/reviewer.md` |
...
1. **Identify Input Type**:
   - Plan file path (e.g., `.factory/plan/xxx.md`)
...
1. Call Gemini (use `~/.factory/.ccg/prompts/gemini/frontend.md`)
...
1. Call Codex (use `~/.factory/.ccg/prompts/codex/architect.md`)
2. Input: Plan content + retrieved context + target files
3. OUTPUT: `Unified Diff Patch ONLY. Strictly prohibit any actual modifications.`
...
1. **Codex Review** (`run_in_background: true`):
   - ROLE_FILE: `~/.factory/.ccg/prompts/codex/reviewer.md`
...
2. **Gemini Review** (`run_in_background: true`):
   - ROLE_FILE: `~/.factory/.ccg/prompts/gemini/reviewer.md`
   - Input: Changed Diff + target files
