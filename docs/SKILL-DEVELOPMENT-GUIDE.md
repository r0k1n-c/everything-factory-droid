...
A comprehensive guide to creating effective skills for Everything Factory Droid (EFD).
...
Skills are **knowledge modules** that Factory Droid loads based on context. They provide:
...
Unlike **agents** (specialized subassistants) or **commands** (user-triggered actions), skills are passive knowledge that Factory Droid references when relevant.

### When Skills Activate

Skills activate when:
- The user's task matches the skill's domain
- Factory Droid detects relevant context
...
Describe scenarios where Droid should use this skill.
...
| `origin` | No | Source identifier (e.g., `EFD`, `community`, project name) |
...
Write content that Droid can **immediately use**:
...
1. **Copy to Factory Droid skills directory**:
   ```bash
   cp -r skills/your-skill-name ~/.factory/skills/
   ```

2. **Test with Factory Droid**:
   ```
   You: "I need to [task that should trigger your skill]"

   Droid should reference your skill's patterns.
   ```

3. **Verify activation**:
   - Ask Droid to explain a concept from your skill
