...
**Symptoms:** Hook runs successfully, but Factory Droid still shows `Hook Error` in the transcript.
...
- On some current Factory Droid builds, lower values may reduce the compaction threshold instead of extending it.
...
- Use EFD's `strategic-compact` guidance instead of forcing a lower auto-compact threshold.
...
- If your Factory Droid build supports it, add a `PostCompact` reminder hook that warns you to re-check connector auth after compaction.
...
- Restart the Factory Droid session after changing hooks.
- Advanced users sometimes script a local `/reload` command around `kill -HUP $PPID`, but EFD does not ship that because it is shell-dependent and not universally reliable.

### Repeated `529 Overloaded` responses

**Symptoms:** Factory Droid starts failing under high hook/tool/context pressure.
