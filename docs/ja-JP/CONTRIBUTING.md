# Everything Factory Droidに貢献する

貢献いただきありがとうございます！このリポジトリはFactory Droidユーザーのためのコミュニティリソースです。
...
# 4. ローカルでテスト
cp -r skills/my-skill ~/.factory/skills/  # スキルの場合
# その後、Factory Droidでテスト
...
スキルは、コンテキストに基づいてFactory Droidが読み込む知識モジュールです。
...
description: このエージェントが実行する操作と、Droid が呼び出すべき時期。具体的に！
...
フックはFactory Droidイベントによってトリガーされる自動的な動作です。
