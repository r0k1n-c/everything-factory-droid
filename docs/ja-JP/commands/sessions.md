...
Factory Droidセッション履歴を管理 - `~/.factory/session-data/` に保存されたセッションのリスト表示、読み込み、エイリアス設定、編集を行います。
...
console.log('Path: ~/.factory/session-data/' + session.filename);
console.log('');
...
- セッションは `~/.factory/session-data/` にMarkdownファイルとして保存されます
- エイリアスは `~/.factory/session-aliases.json` に保存されます
