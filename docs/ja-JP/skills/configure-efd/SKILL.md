---
name: configure-efd
description: Everything Factory Droid のインタラクティブなインストーラー — スキルとルールの選択とインストールをユーザーレベルまたはプロジェクトレベルのディレクトリへガイドし、パスを検証し、必要に応じてインストールされたファイルを最適化します。
---

# Configure Everything Factory Droid (EFD)

Everything Factory Droid プロジェクトのインタラクティブなステップバイステップのインストールウィザードです。`AskUserQuestion` を使用してスキルとルールの選択的インストールをユーザーにガイドし、正確性を検証し、最適化を提供します。

## 起動タイミング

- ユーザーが "configure efd"、"install efd"、"setup everything factory droid" などと言った場合
- ユーザーがこのプロジェクトからスキルまたはルールを選択的にインストールしたい場合
- ユーザーが既存の EFD インストールを検証または修正したい場合
- ユーザーがインストールされたスキルまたはルールをプロジェクト用に最適化したい場合

## 前提条件

このスキルは起動前に Factory Droid からアクセス可能である必要があります。ブートストラップには2つの方法があります：
1. **プラグイン経由**: `droid plugin marketplace add https://github.com/r0k1n-c/everything-factory-droid` を実行してから `droid plugin install everything-factory-droid@everything-factory-droid` を実行してください — プラグインがこのスキルを自動的にロードします
2. **手動**: このスキルのみを `~/.factory/skills/configure-efd/SKILL.md` にコピーし、"configure efd" と言って起動します

---

## ステップ 0: EFD リポジトリのクローン

インストールの前に、最新の EFD ソースを `/tmp` にクローンします：
...
以降のすべてのコピー操作のソースとして `EFD_ROOT=/tmp/everything-factory-droid` を設定します。

クローンが失敗した場合（ネットワークの問題など）、`AskUserQuestion` を使用してユーザーに既存の EFD クローンへのローカルパスを提供するよう依頼します。
...
```
Question: "EFD コンポーネントをどこにインストールしますか？"
Options:
  - "User-level (~/.factory/)" — "すべての Factory Droid プロジェクトに適用されます"
  - "Project-level (.factory/)" — "現在のプロジェクトのみに適用されます"
  - "Both" — "共通/共有アイテムはユーザーレベル、プロジェクト固有アイテムはプロジェクトレベル"
```

選択を `INSTALL_LEVEL` として保存します。ターゲットディレクトリを設定します：
- User-level: `TARGET=~/.factory`
- Project-level: `TARGET=.factory`（現在のプロジェクトルートからの相対パス）
...
インストールを実行：
```bash
# 共通ルール（rules/ 配下へディレクトリごとコピー）
cp -r $EFD_ROOT/rules/common $TARGET/rules/

# 言語固有のルール（rules/ 配下へディレクトリごとコピー）
cp -r $EFD_ROOT/rules/typescript $TARGET/rules/   # 選択された場合
cp -r $EFD_ROOT/rules/python $TARGET/rules/        # 選択された場合
cp -r $EFD_ROOT/rules/golang $TARGET/rules/        # 選択された場合
```
...
**プロジェクトレベルのインストールの場合**、`~/.factory/` パスへの参照をフラグします：
- スキルが `~/.factory/settings.json` を参照している場合 — これは通常問題ありません（設定は常にユーザーレベルです）
- スキルが `~/.factory/skills/` または `~/.factory/rules/` を参照している場合 — プロジェクトレベルのみにインストールされている場合、これは壊れている可能性があります
...
- `continuous-learning-v2` は `~/.factory/homunculus/` ディレクトリを参照します
...
2. **行**: 行番号
3. **問題**: 何が間違っているか（例: "~/.factory/skills/python-patterns を参照していますが、python-patterns がインストールされていません"）
4. **推奨される修正**: 何をすべきか（例: "python-patterns スキルをインストール" または "パスを .factory/skills/ に更新"）
...
**重要**: インストール先（`$TARGET/`）のファイルのみを変更し、ソース EFD リポジトリ（`$EFD_ROOT/`）のファイルは決して変更しないでください。
...
### "スキルが Factory Droid に認識されません"
- スキルディレクトリに `SKILL.md` ファイルが含まれていることを確認します（単なる緩い .md ファイルではありません）
- ユーザーレベルの場合: `~/.factory/skills/<skill-name>/SKILL.md` が存在するか確認します
- プロジェクトレベルの場合: `.factory/skills/<skill-name>/SKILL.md` が存在するか確認します

### "ルールが機能しません"
- ルールは言語ディレクトリ構造を維持します: `$TARGET/rules/common/coding-style.md` が正しく、`$TARGET/rules/coding-style.md` のようなフラット配置は不正です
- ルールをインストール後、Factory Droid を再起動します

### "プロジェクトレベルのインストール後のパス参照エラー"
- 一部のスキルは `~/.factory/` パスを前提としています。ステップ4の検証を実行してこれらを見つけて修正します。
- `continuous-learning-v2` の場合、`~/.factory/homunculus/` ディレクトリは常にユーザーレベルです — これは想定されており、エラーではありません。
