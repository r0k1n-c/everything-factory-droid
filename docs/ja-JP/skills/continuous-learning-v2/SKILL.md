...
Factory Droidセッションを信頼度スコアリング付きの小さな学習済み行動である「インスティンクト」を通じて再利用可能な知識に変える高度な学習システム。
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "${FACTORY_PROJECT_DIR}/skills/continuous-learning-v2/hooks/observe.sh pre"
      }]
    }],
    "PostToolUse": [{
      "matcher": "*",
      "hooks": [{
        "type": "command",
        "command": "${FACTORY_PROJECT_DIR}/skills/continuous-learning-v2/hooks/observe.sh post"
      }]
    }]
  }
}
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "~/.factory/skills/continuous-learning-v2/hooks/observe.sh pre"
      }]
    }],
    "PostToolUse": [{
      "matcher": "*",
      "hooks": [{
        "type": "command",
        "command": "~/.factory/skills/continuous-learning-v2/hooks/observe.sh post"
      }]
    }]
  }
}
...
```bash
mkdir -p ~/.factory/homunculus/{instincts/{personal,inherited},evolved/{agents,skills,commands}}
touch ~/.factory/homunculus/observations.jsonl
```
...
```json
{
  ...
  "observation": {
    "enabled": true,
    "store_path": "~/.factory/homunculus/observations.jsonl",
    "max_file_size_mb": 10,
    "archive_after_days": 7
  },
  "instincts": {
    "personal_path": "~/.factory/homunculus/instincts/personal/",
    "inherited_path": "~/.factory/homunculus/instincts/inherited/",
    "min_confidence": 0.3,
  ...
  "evolution": {
    "cluster_threshold": 3,
    "evolved_path": "~/.factory/homunculus/evolved/"
  }
}
```
...
v2はv1と完全に互換性があります：
- 既存の`~/.factory/skills/learned/`スキルは引き続き機能
