...
description: Factory Droid 세션에서 재사용 가능한 패턴을 자동으로 추출하여 향후 사용을 위한 학습된 스킬로 저장합니다.
origin: EFD
---

# 지속적 학습 스킬

Factory Droid 세션 종료 시 자동으로 평가하여 학습된 스킬로 저장할 수 있는 재사용 가능한 패턴을 추출합니다.

## 활성화 시점

- Factory Droid 세션에서 자동 패턴 추출을 설정할 때
- 세션 평가를 위한 Stop Hook을 구성할 때
- `~/.factory/skills/learned/`에서 학습된 스킬을 검토하거나 큐레이션할 때
...
3. **스킬 추출**: 유용한 패턴을 `~/.factory/skills/learned/`에 저장
