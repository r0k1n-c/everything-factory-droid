---
name: eval-harness
description: 평가 주도 개발(EDD) 원칙을 구현하는 Factory Droid 세션용 공식 평가 프레임워크
origin: EFD
...
Factory Droid 세션을 위한 공식 평가 프레임워크로, 평가 주도 개발(EDD) 원칙을 구현합니다.

## 활성화 시점

- AI 지원 워크플로우에 평가 주도 개발(EDD) 설정 시
- Factory Droid 작업 완료에 대한 합격/불합격 기준 정의 시
...
Task: Description of what Droid should accomplish
Success Criteria:
  - [ ] Criterion 1
  - [ ] Criterion 2
  - [ ] Criterion 3
...
```
`.factory/evals/feature-name.md`에 평가 정의 파일 생성
...
- `.factory/evals/<feature>.md` 정의
- `.factory/evals/<feature>.log` 실행 이력
- `docs/evals/<version>/summary.md` 릴리스 스냅샷
