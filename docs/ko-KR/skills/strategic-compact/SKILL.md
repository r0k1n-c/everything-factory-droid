    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node \"${FACTORY_PROJECT_DIR}/scripts/hooks/run-with-flags.js\" \"pre:edit-write:suggest-compact\" \"scripts/hooks/suggest-compact.js\" \"standard,strict\""
          }
        ],
...
| 유지됨 | 손실됨 |
|----------|------|
| AGENTS.md 지침 | 중간 추론 및 분석 |
| TodoWrite 작업 목록 | 이전에 읽은 파일 내용 |
| 메모리 파일 (`~/.factory/memory/`) | 다단계 대화 컨텍스트 |
