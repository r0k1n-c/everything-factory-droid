...
Some recent Factory Droid builds have community reports that `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` can only lower the compaction threshold, which means values below the default may compact earlier instead of later. If that happens in your setup, remove the override and rely on manual `/compact` plus EFD's `strategic-compact` guidance. See [Troubleshooting](./TROUBLESHOOTING.md).
...
## Parallel Context Cost Warning

[Mixed Models](https://docs.factory.ai/cli/configuration/mixed-models.md) and subagents let you split work across models and contexts. Each additional context window consumes tokens separately.
...
## Future: configure-efd Integration

The `configure-efd` install wizard could offer to set these environment variables during setup, with explanations of the cost tradeoffs. This would help new users optimize from day one rather than discovering these settings after hitting limits.
...
# Environment variables (add to ~/.factory/settings.json "env" block)
MAX_THINKING_TOKENS=10000
CLAUDE_CODE_SUBAGENT_MODEL=haiku
