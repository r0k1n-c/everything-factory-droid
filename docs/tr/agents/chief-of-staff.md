...
- **Kurallar sistem enjektelidir**: `.factory/rules/*.md` dosyaları her oturumda otomatik yüklenir. İstem talimatlarının aksine, LLM bunları görmezden gelmeyi seçemez.

## Örnek Çağrılar

```bash
droid /mail                    # Yalnızca e-posta triyajı
droid /slack                   # Yalnızca Slack triyajı
droid /today                   # Tüm kanallar + takvim + yapılacaklar
droid /schedule-reply "Yönetim kurulu toplantısı hakkında Sarah'ya yanıt ver"
```
