...
Bash({
  command: "~/.factory/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend codex - \"$PWD\" <<'EOF'
...
Bash({
  command: "~/.factory/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend codex resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <role prompt path>
<TASK>
Requirement: <enhanced requirement (veya enhance edilmediyse $ARGUMENTS)>
Context: <önceki fazlardan proje context'i ve analiz>
...
| Phase | Codex |
|-------|-------|
| Analysis | `~/.factory/.ccg/prompts/codex/analyzer.md` |
| Planning | `~/.factory/.ccg/prompts/codex/architect.md` |
| Review | `~/.factory/.ccg/prompts/codex/reviewer.md` |
