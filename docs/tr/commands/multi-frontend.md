...
Bash({
  command: "~/.factory/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend gemini --gemini-model gemini-3-pro-preview - \"$PWD\" <<'EOF'
...
Bash({
  command: "~/.factory/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend gemini --gemini-model gemini-3-pro-preview resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <role prompt path>
<TASK>
Requirement: <enhanced requirement (veya enhance edilmediyse $ARGUMENTS)>
Context: <önceki fazlardan proje context'i ve analiz>
...
| Phase | Gemini |
|-------|--------|
| Analysis | `~/.factory/.ccg/prompts/gemini/analyzer.md` |
| Planning | `~/.factory/.ccg/prompts/gemini/architect.md` |
| Review | `~/.factory/.ccg/prompts/gemini/reviewer.md` |
...
Claude planı sentezler, kullanıcı onayından sonra `.factory/plan/task-name.md`'ye kaydet.
