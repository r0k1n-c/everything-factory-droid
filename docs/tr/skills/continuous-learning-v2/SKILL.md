...
Factory Droid oturumlarınızı güven skorlaması ile atomik "instinct'ler" - küçük öğrenilmiş davranışlar - aracılığıyla yeniden kullanılabilir bilgiye dönüştüren gelişmiş bir öğrenme sistemi.
...
- Factory Droid oturumlarından otomatik öğrenme ayarlarken
...
| Özellik | v2.0 | v2.1 |
|---------|------|------|
| Depolama | Global (~/.factory/homunculus/) | Proje kapsamlı (projects/<hash>/) |
...
Her proje 12 karakterlik bir hash ID alır (örn. `a1b2c3d4e5f6`). `~/.factory/homunculus/projects.json` dosyasındaki kayıt dosyası ID'leri insanların okuyabileceği isimlerle eşler.

## Hızlı Başlangıç

### 1. Gözlem Hook'larını Aktifleştirin

`~/.factory/settings.json` dosyanıza ekleyin.
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "${FACTORY_PROJECT_DIR}/skills/continuous-learning-v2/hooks/observe.sh"
      }]
    }],
    "PostToolUse": [{
      "matcher": "*",
...
**`~/.factory/skills` dizinine manuel kuruluysa**:
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "~/.factory/skills/continuous-learning-v2/hooks/observe.sh"
      }]
    }],
    "PostToolUse": [{
      "matcher": "*",
...
```bash
# Global dizinler
mkdir -p ~/.factory/homunculus/{instincts/{personal,inherited},evolved/{agents,skills,commands},projects}
...
- `~/.factory/homunculus/instincts/` içindeki mevcut global instinct'ler hala global instinct olarak çalışır
- v1'den `~/.factory/skills/learned/` skill'leri hala çalışır
...
- [EFD-Tools GitHub App](https://github.com/apps/efd-tools) - Repo geçmişinden instinct'ler oluştur
