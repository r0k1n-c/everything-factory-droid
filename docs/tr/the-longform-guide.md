# Factory Droid'un Her Şeyine Dair Uzun Kılavuz

![Header: The Longform Guide to Everything Factory Droid](../../assets/images/longform/01-header.png)

---

> **Ön Koşul**: Bu kılavuz [Factory Droid'un Her Şeyine Dair Kısa Kılavuz](./the-shortform-guide.md) üzerine kuruludur. Skill'leri, hook'ları, subagent'ları, MCP'leri ve plugin'leri henüz kurmadıysanız önce onu okuyun.

![Reference to Shorthand Guide](../../assets/images/longform/02-shortform-reference.png)
*Kısa Kılavuz - önce onu okuyun*

Kısa kılavuzda, temel kurulumu ele aldım: etkili bir Factory Droid iş akışının omurgasını oluşturan skill'ler ve command'lar, hook'lar, subagent'lar, MCP'ler, plugin'ler ve yapılandırma desenleri. Bu kurulum kılavuzu ve temel altyapıydı.
...
Kısa ve uzun kılavuzlarda ele alınan her şey GitHub'da mevcuttur: `github.com/affaan-m/everything-factory-droid`
...
Oturumlar arasında memory paylaşımı için, ilerlemeyi özetleyen ve kontrol eden, ardından `.factory` klasörünüzde bir `.tmp` dosyasına kaydeden ve oturumunuz sonuna kadar ona ekleyen bir skill veya command en iyi bahistir. Ertesi gün bunu context olarak kullanabilir ve kaldığı yerden devam edebilir, her oturum için yeni bir dosya oluşturun böylece eski context'i yeni işe kirletmezsiniz.

![Session Storage File Tree](../../assets/images/longform/03-session-storage.png)
*Oturum depolama örneği -> <https://github.com/affaan-m/everything-factory-droid/tree/main/examples/sessions>*

Droid mevcut durumu özetleyen bir dosya oluşturur. İnceleyin, gerekirse düzenlemeler isteyin, ardından yeniden başlayın. Yeni konuşma için, sadece dosya yolunu sağlayın. Özellikle context limitlerini aşarken ve karmaşık işi sürdürmeniz gerektiğinde kullanışlıdır. Bu dosyalar şunları içermelidir:
...
Planınız hazır ve context temizlendiğinde (artık Factory Droid'da plan modunda varsayılan seçenek), plandan çalışabilirsiniz. Bu, yürütmeyle artık ilgili olmayan çok fazla keşif context'i biriktirdiğinizde kullanışlıdır. Stratejik sıkıştırma için, otomatik sıkıştırmayı devre dışı bırakın. Mantıksal aralıklarla manuel olarak sıkıştırın veya bunu sizin için yapan bir skill oluşturun.

**Gelişmiş: Dinamik System Prompt Enjeksiyonu**

Aldığım bir desen: her oturumu yükleyen AGENTS.md'ye (kullanıcı kapsamı) veya `.factory/rules/`'a (proje kapsamı) her şeyi sadece koymak yerine, context'i dinamik olarak enjekte etmek için CLI flag'lerini kullanın.

```bash
droid --system-prompt "$(cat memory.md)"
```
...
```bash
# Günlük geliştirme
alias droid-dev='droid --system-prompt "$(cat ~/.factory/contexts/dev.md)"'

# PR inceleme modu
alias droid-review='droid --system-prompt "$(cat ~/.factory/contexts/review.md)"'

# Araştırma/keşif modu
alias droid-research='droid --system-prompt "$(cat ~/.factory/contexts/research.md)"'
```
...
Bu hook'ları oluşturdum ve repo'da `github.com/affaan-m/everything-factory-droid/tree/main/hooks/memory-persistence` adresindeler

---

### Sürekli Öğrenme / Memory

Bir prompt'u birden çok kez tekrarlamanız gerekti ve Droid aynı probleme takıldı veya daha önce duyduğunuz bir yanıt verdi - bu desenlerin skill'lere eklenmesi gerekir.

**Problem:** Boşa giden token'lar, boşa giden context, boşa giden zaman.

**Çözüm:** Factory Droid önemsiz olmayan bir şey keşfettiğinde - bir hata ayıklama tekniği, bir geçici çözüm, projeye özgü bir desen - bu bilgiyi yeni bir skill olarak kaydeder. Benzer bir problem bir dahaki sefer ortaya çıktığında, skill otomatik olarak yüklenir.

Bunu yapan bir sürekli öğrenme skill'i oluşturdum: `github.com/affaan-m/everything-factory-droid/tree/main/skills/continuous-learning`
...
*50 görevlik benchmark'ımızda, mgrep + Factory Droid, grep tabanlı iş akışlarına kıyasla benzer veya daha iyi değerlendirilen kalitede ~2 kat daha az token kullandı. Kaynak: @mixedbread-ai tarafından mgrep*
...
Çoklu Droid terminal kurulumunda konuşmaları fork'larken, fork ve orijinal konuşmadaki eylemler için kapsamın iyi tanımlandığından emin olun. Kod değişiklikleri söz konusu olduğunda minimum örtüşme hedefleyin.
...
*Boris (Anthropic) birden fazla Droid instance'ı çalıştırma üzerine*

Boris'in paralelleştirme hakkında ipuçları var. 5 Droid instance'ını yerel olarak ve 5'ini ayrı bir proje ortamında çalıştırmak gibi şeyler önerdi. Keyfi terminal miktarları belirlemeye karşı tavsiyede bulunurum. Bir terminalin eklenmesi gerçek bir zorunluluktan olmalıdır.
...
# Her worktree kendi Droid instance'ını alır
cd ../project-feature-a && droid
```

Instance'larınızı ölçeklendirmeye başlıyorsanız VE birbirleriyle örtüşen kod üzerinde çalışan birden fazla Droid instance'ınız varsa, git worktree'leri kullanmanız ve her biri için çok iyi tanımlanmış bir plana sahip olmanız zorunludur. Tüm sohbetlerinizi adlandırmak için `/rename <name here>` kullanın.
...
Birden fazla Factory Droid instance'ı çalıştırırken, "cascade" deseniyle organize edin:
...
Kendi iş akışı yönetimim için, boş bir repo'yu 2 açık Droid instance'ıyla başlatmayı seviyorum.

**Instance 1: Scaffolding Agent**
- İskeleyi ve temelleri atar
- Proje yapısını oluşturur
- Yapılandırmaları kurar (AGENTS.md, rules, agents)
...
`/statusline` kullanarak ayarlayabilirsiniz - ardından Droid birinin olmadığını söyleyecek ancak sizin için kurabilir ve içinde ne istediğinizi soracak.

Ayrıca bakın: ccstatusline (özel Factory Droid status line'ları için topluluk projesi)

### Ses Transkripsiyon

Factory Droid ile sesinizle konuşun. Birçok insan için yazmaktan daha hızlı.

- Mac'te superwhisper, MacWhisper
- Transkripsiyon hataları olsa bile, Droid amacı anlar
...
- [YK: 32 Factory Droid İpucu](https://agenticcoding.substack.com/p/32-claude-code-tips-from-basics-to)
...
*Her iki kılavuzda ele alınan her şey GitHub'da [everything-factory-droid](https://github.com/affaan-m/everything-factory-droid) adresinde mevcuttur*
