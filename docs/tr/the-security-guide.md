...
_everything factory droid / araştırma / güvenlik_

---

Son makalemden bu yana epey zaman geçti. EFD devtooling ekosistemini geliştirmeye zaman harcadım. Bu süreçte sıcak ancak önemli konulardan biri agent güvenliği oldu.

Açık kaynak agent'ların yaygın olarak benimsenmesi burada. OpenClaw ve diğerleri bilgisayarınızda dolaşıyor. Factory Droid ve Codex (EFD kullanarak) gibi sürekli çalışma harness'leri yüzey alanını artırıyor; ve 25 Şubat 2026'da, Check Point Research konuşmanın "bu olabilir ama olmaz / abartılıyor" fazını kesinlikle sona erdirmesi gereken bir Factory Droid ifşası yayınladı. Araçlar kritik kütleye ulaştıkça, exploit'lerin ağırlığı katlanır.
...
Video: Bad Rudi Exploit — good rudi (çocuklar için grok animasyonlu AI karakteri) hassas bilgileri ifşa etmek için tekrarlanan denemelerden sonra bir prompt jailbreak ile exploit edilir. eğlenceli bir örnek ama yine de olasılıklar çok daha ileri gider.
...
## Factory Droid CVE'leri (Şubat 2026)

Check Point Research, Factory Droid bulgularını 25 Şubat 2026'da yayınladı. Sorunlar Temmuz ve Aralık 2025 arasında bildirildi, ardından yayından önce yamalandı.
...
>
> Sahte hook eylemleriyle zehirlenmiş yapılandırma dosyaları aracılığıyla Factory Droid kullanıcılarını ele geçirme.
>
...
> _Factory Droid'u hack'ledim! "Agentic"in sadece shell almanın süslü yeni bir yolu olduğu ortaya çıktı. Tam RCE elde ettim ve organizasyon API anahtarlarını ele geçirdim. CVE-2025-59536 | CVE-2026-21852_
...
Anthropic'in kendi dokümanları bu gerçeği yansıtıyor. Proje ayarları `.factory/` içinde yaşıyor. Proje kapsamlı MCP server'ları `.mcp.json` içinde yaşıyor. Kaynak kontrol aracılığıyla paylaşılıyorlar. Bir güven sınırı tarafından korunmaları gerekiyor. Bu güven sınırı tam olarak saldırganların peşine düşeceği şey.

## Son Bir Yılda Ne Değişti

Bu konuşma 2025 ve erken 2026'da hızlı ilerledi.

Factory Droid'un repo tarafından kontrol edilen hook'ları, MCP ayarları ve env-var güven yolları kamuya açık olarak test edildi. Amazon Q Developer, VS Code extension'ında kötü niyetli prompt payload içeren 2025 tedarik zinciri olayına, ardından yapı altyapısında aşırı geniş GitHub token maruziyetiyle ilgili ayrı bir ifşaya sahipti. Zayıf kimlik bilgisi sınırları artı agent'a yakın araçlar, fırsatçılar için bir giriş noktasıdır.
...
| İstatistik | Detay |
|------|--------|
| **CVSS 8.7** | Factory Droid hook / güven öncesi yürütme sorunu: CVE-2025-59536 |
...
Root erişimi tehlikelidir. Geniş yerel erişim tehlikelidir. Aynı makinede uzun ömürlü kimlik bilgileri tehlikelidir. "YOLO, Droid beni koruyor" burada doğru yaklaşım değildir. Cevap izolasyondur.
...
Anthropic, Factory Droid'un oturum başlangıcında memory yüklediğini belgeliyor. Bu yüzden memory'yi dar tutun:
...
Anthropic, Factory Droid'u sertleştirdi ve güven, izinler, MCP, memory, hook'lar ve izole ortamlar etrafında somut güvenlik rehberliği yayınladı.
...
Ve özellikle EFD kullanıyorsanız, AgentShield'i bunun için oluşturduğum problem alanı da budur: şüpheli hook'lar, gizli prompt injection desenleri, aşırı geniş izinler, riskli MCP yapılandırması, secret maruziyeti ve insanların manuel incelemede kesinlikle kaçıracağı şeyler.
...
- Check Point Research, "Caught in the Hook: RCE and API Token Exfiltration Through Factory Droid Project Files" (25 Şubat 2026): [research.checkpoint.com](https://research.checkpoint.com/2026/rce-and-api-token-exfiltration-through-claude-code-project-files-cve-2025-59536/)
...
- Factory Droid docs, "Settings": [docs.factory.ai](https://docs.factory.ai/cli/configuration/settings)
- Factory Droid docs, "MCP": [docs.factory.ai](https://docs.factory.ai/cli/configuration/mcp)
- Factory Droid docs, "Security": [docs.factory.ai](https://docs.factory.ai/cli/account/security)
- Factory Droid docs, "Memory": [docs.factory.ai](https://docs.factory.ai/guides/power-user/memory-management)
...
> [Factory Droid'un Her Şeyine Dair Kısa Kılavuz](https://x.com/affaanmustafa/status/2012378465664745795)
