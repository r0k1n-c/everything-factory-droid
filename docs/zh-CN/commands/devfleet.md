---
description: 通过Claude DevFleet协调并行Factory Droid代理——从自然语言规划项目，在隔离的工作树中调度代理，监控进度，并读取结构化报告。
---

# DevFleet — 多智能体编排

通过 Claude DevFleet 编排并行的 Factory Droid 智能体。每个智能体在隔离的 git worktree 中运行，并配备完整的工具链。

需要 DevFleet MCP 服务器：`droid mcp add devfleet --transport http http://localhost:18801/mcp`
