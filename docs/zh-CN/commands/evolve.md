...
或者如果 `FACTORY_PROJECT_DIR` 未设置（手动安装）：
...
* **命令**：当本能描述用户调用的操作时
* **技能**：当本能描述自动触发的行为时
* **Droid**：当本能描述复杂的、多步骤的流程时

## 使用方法

```
/evolve                    # 分析所有本能并建议进化方向
/evolve --generate         # 同时在 evolved/{skills,commands,droids} 目录下生成文件
```
...
6. 如果传入了 `--generate`，则将文件写入：
   * 项目范围：`~/.factory/homunculus/projects/<project-id>/evolved/`
   * 全局回退：`~/.factory/homunculus/evolved/`
