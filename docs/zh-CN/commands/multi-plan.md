...
* **仅限规划**：此命令允许读取上下文并写入 `.factory/plan/*` 计划文件，但 **绝不修改生产代码**
...
| 阶段 | Codex | Gemini |
|-------|-------|--------|
| 分析 | `~/.factory/.ccg/prompts/codex/analyzer.md` | `~/.factory/.ccg/prompts/gemini/analyzer.md` |
| 规划 | `~/.factory/.ccg/prompts/codex/architect.md` | `~/.factory/.ccg/prompts/gemini/architect.md` |
...
**如果 ace-tool MCP 不可用**，使用 Factory Droid 内置工具作为备用方案：
...
1. **Codex 后端分析**：
   * ROLE\_FILE：`~/.factory/.ccg/prompts/codex/analyzer.md`
   * 重点：技术可行性、架构影响、性能考虑、潜在风险
   * 输出：多视角解决方案 + 优缺点分析

2. **Gemini 前端分析**：
   * ROLE\_FILE：`~/.factory/.ccg/prompts/gemini/analyzer.md`
...
1. **Codex 计划草案**（后端权威）：
   * ROLE\_FILE：`~/.factory/.ccg/prompts/codex/architect.md`
   * 输出：分步计划 + 伪代码（重点：数据流/边缘情况/错误处理/测试策略）

2. **Gemini 计划草案**（前端权威）：
   * ROLE\_FILE：`~/.factory/.ccg/prompts/gemini/architect.md`
...
2. 将计划保存到 `.factory/plan/<feature-name>.md`（从需求中提取功能名称，例如 `user-auth`，`payment-module`）

3. 以 **粗体文本** 输出提示（必须使用实际保存的文件路径）：

***

**计划已生成并保存至 `.factory/plan/actual-feature-name.md`**
...
* **首次规划**：`.factory/plan/<feature-name>.md`
* **迭代版本**：`.factory/plan/<feature-name>-v2.md`，`.factory/plan/<feature-name>-v3.md`...
...
1. 根据用户反馈调整计划内容
2. 更新 `.factory/plan/<feature-name>.md` 文件
3. 重新呈现修改后的计划
4. 提示用户再次审阅或执行
