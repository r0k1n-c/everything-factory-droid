...
```bash
python3 "${FACTORY_PROJECT_DIR}/skills/continuous-learning-v2/scripts/instinct-cli.py" promote [instinct-id] [--force] [--dry-run]
```

或者如果未设置 `FACTORY_PROJECT_DIR`（手动安装）：

```bash
python3 ~/.factory/skills/continuous-learning-v2/scripts/instinct-cli.py promote [instinct-id] [--force] [--dry-run]
```
...
3. 否则，查找跨项目候选本能，这些本能：
   * 出现在至少 2 个项目中
   * 满足置信度阈值
4. 将提升后的本能写入 `~/.factory/homunculus/instincts/personal/`，并设置 `scope: global`
