---
description: 分析一个草稿提示，输出一个经过优化、充分利用 EFD 组件的版本，准备粘贴并运行。不执行任务——仅输出咨询分析。
---

# /prompt-optimize

分析并优化以下提示语，以实现最大化的 EFD 杠杆效应。

## 你的任务

对下方用户的输入应用 **prompt-optimizer** 技能。遵循6阶段分析流程：

0. **项目检测** — 读取 AGENTS.md，从项目文件（package.json, go.mod, pyproject.toml 等）检测技术栈
1. **意图检测** — 对任务类型进行分类（新功能、错误修复、重构、研究、测试、评审、文档、基础设施、设计）
2. **范围评估** — 评估复杂度（简单 / 低 / 中 / 高 / 史诗级），如果检测到代码库，则使用其大小作为信号
3. **EFD 组件匹配** — 映射到特定的技能、命令、代理和模型层级
4. **缺失上下文检测** — 识别信息缺口。如果缺少3个以上关键项，请在生成前请用户澄清
5. **工作流与模型** — 确定生命周期阶段，推荐模型层级，如果复杂度为高/史诗级，则将其拆分为多个提示语

## 输出要求

* 呈现诊断结果、推荐的 EFD 组件以及使用 prompt-optimizer 技能中输出格式的优化后提示语
