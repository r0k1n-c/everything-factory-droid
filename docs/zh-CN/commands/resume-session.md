---
description: 从 ~/.factory/session-data/ 加载最新的会话文件，并从上次会话结束的地方恢复工作，保留完整上下文。
---
...
* 任何拥有会话文件并希望 Droid 在继续前完全吸收其内容的时候

## 用法

```
/resume-session                                                      # 加载 ~/.factory/session-data/ 目录下最新的文件
/resume-session 2024-01-15                                           # 加载该日期最新的会话
/resume-session ~/.factory/session-data/2024-01-15-session.tmp           # 加载特定的旧格式文件
/resume-session ~/.factory/session-data/2024-01-15-abc123de-session.tmp  # 加载当前短ID格式的会话文件
```
...
1. 检查 `~/.factory/session-data/`
2. 选择最近修改的 `*-session.tmp` 文件
3. 如果文件夹不存在或没有匹配的文件，告知用户：
   ```
   在 ~/.factory/session-data/ 中未找到会话文件。
...
* 如果看起来像日期 (`YYYY-MM-DD`)，则在 `~/.factory/session-data/` 中搜索匹配
...
```
SESSION LOADED: /Users/you/.factory/sessions/2024-01-15-abc123de-session.tmp
════════════════════════════════════════════════
