---
description: 将当前会话状态保存到 ~/.factory/session-data/ 目录下带日期的文件中，以便在未来的会话中恢复完整上下文并继续工作。
---
...
在用户的 Factory Droid 主目录中创建规范的会话文件夹：
...
创建 `~/.factory/session-data/YYYY-MM-DD-<short-id>-session.tmp`，使用今天的实际日期和一个满足 `session-manager.js` 中 `SESSION_FILENAME_REGEX` 强制规则的短 ID：
...
* 该文件旨在通过 `/resume-session` 在下次会话开始时由 Droid 读取
* 使用规范的全局会话存储：`~/.factory/session-data/`
