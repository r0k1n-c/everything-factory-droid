---
description: 管理Factory Droid会话历史、别名和会话元数据。
---

# Sessions 命令

管理 Factory Droid 会话历史 - 列出、加载、设置别名和编辑存储在 `~/.factory/session-data/` 中的会话。
...
* 会话以 Markdown 文件形式存储在 `~/.factory/session-data/`
* 别名存储在 `~/.factory/session-aliases.json`
