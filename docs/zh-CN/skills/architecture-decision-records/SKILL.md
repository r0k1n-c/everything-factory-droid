...
description: 在Factory Droid会话期间，将做出的架构决策捕获为结构化的架构决策记录（ADR）。自动检测决策时刻，记录上下文、考虑的替代方案和理由。维护一个ADR日志，以便未来的开发人员理解代码库为何以当前方式构建。
origin: EFD
---
...
| ADR | 标题 | 状态 | 日期 |
|-----|-------|--------|------|
| 0001 | 使用 Next.js 作为前端框架 | 已采纳 | 2026-01-15 |
| 0002 | 主数据存储选用 PostgreSQL 而非 MongoDB | 已采纳 | 2026-01-20 |
| 0003 | 选用 REST API 而非 GraphQL | 已采纳 | 2026-02-01 |
```
