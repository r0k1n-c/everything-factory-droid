...
* **零运行时风险** — 纯 Markdown 技能。整个仓库仅包含 `.md` 文件 — 无钩子、无 shell 脚本、无可执行代码、无 `package.json`、无构建步骤。安装或调用时，除了 Factory Droid 的原生 Markdown 技能加载器外，不运行任何内容。

## 安装

此技能随 Everything Factory Droid 附带。安装 EFD 时无需单独安装。

### 完整 EFD 安装

如果您从 EFD 仓库检出中工作，请验证技能是否存在：

```bash
test -f skills/blueprint/SKILL.md
```

后续更新时，请在更新前查看 EFD 的差异：
...
如果您在完整 EFD 安装之外仅内嵌此技能，请将 EFD 仓库中已审查的文件复制到 `~/.factory/skills/blueprint/SKILL.md`。内嵌副本没有 git 远程仓库，因此应通过从已审查的 EFD 提交中重新复制文件来更新，而不是运行 `git pull`。

## 要求

* Factory Droid（用于 `/blueprint` 斜杠命令）
