---
name: configure-efd
description: Everything Factory Droid 的交互式安装程序 — 引导用户选择并安装技能和规则到用户级或项目级目录，验证路径，并可选择优化已安装文件。
origin: EFD
---

# 配置 Everything Factory Droid (EFD)

一个交互式、分步安装向导，用于 Everything Factory Droid 项目。使用 `AskUserQuestion` 引导用户选择性安装技能和规则，然后验证正确性并提供优化。

## 何时激活

* 用户说 "configure efd"、"install efd"、"setup everything factory droid" 或类似表述
* 用户想要从此项目中选择性安装技能或规则
* 用户想要验证或修复现有的 EFD 安装
* 用户想要为其项目优化已安装的技能或规则
...
如果克隆失败（网络问题等），使用 `AskUserQuestion` 要求用户提供现有 EFD 克隆的本地路径。
...
问题："EFD 组件应安装在哪里？"
选项：
  - "用户级别 (~/.factory/)" — "适用于您所有的Factory Droid项目"
  - "项目级别 (.factory/)" — "仅适用于当前项目"
...
* 用户级别：`TARGET=~/.factory`
* 项目级别：`TARGET=.factory`（相对于当前项目根目录）
* 两者：`TARGET_USER=~/.factory`，`TARGET_PROJECT=.factory`
...
默认为 **核心（推荐给新用户）** — 从 `skills/` 复制选定的核心技能目录（包括 `search-first`）。此捆绑包涵盖工程、评估、验证、安全、战略压缩、前端设计以及 Anthropic 跨职能技能（文章写作、内容引擎、市场研究、前端幻灯片）。
...
下方有8个可选的类别组。后续的详细确认列表涵盖了8个类别中的45项技能，外加1个独立模板。使用 `AskUserQuestion` 与 `multiSelect: true`：
...
**对于项目级别安装**，标记任何对 `~/.factory/` 路径的引用：

* 如果技能引用 `~/.factory/settings.json` — 这通常没问题（设置始终是用户级别的）
* 如果技能引用 `~/.factory/skills/` 或 `~/.factory/rules/` — 如果仅安装在项目级别，这可能损坏
...
* `continuous-learning-v2` 引用 `~/.factory/homunculus/` 目录
...
1. **文件**：包含问题引用的文件
2. **行号**：行号
3. **问题**：哪里出错了（例如，"引用了 ~/.factory/skills/python-patterns 但 python-patterns 未安装"）
4. **建议的修复**：该怎么做（例如，"安装 python-patterns 技能" 或 "将路径更新为 .factory/skills/"）
...
**关键**：只修改安装目标（`$TARGET/`）中的文件，**绝不**修改源 EFD 仓库（`$EFD_ROOT/`）中的文件。
...
* 对于用户级别：检查 `~/.factory/skills/<skill-name>/SKILL.md` 是否存在
* 对于项目级别：检查 `.factory/skills/<skill-name>/SKILL.md` 是否存在

### "规则不工作"

* 规则应保留语言目录结构：`$TARGET/rules/common/coding-style.md`（正确）而不是将文件平铺到 `$TARGET/rules/coding-style.md`
* 安装规则后重启 Factory Droid

### "项目级别安装后出现路径引用错误"

* 有些技能假设 `~/.factory/` 路径。运行步骤 4 验证来查找并修复这些问题。
* 对于 `continuous-learning-v2`，`~/.factory/homunculus/` 目录始终是用户级别的 — 这是预期的，不是错误。
