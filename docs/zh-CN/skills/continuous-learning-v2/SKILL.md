...
| 特性 | v2.0 | v2.1 |
|---------|------|------|
| 存储 | 全局 (~/.factory/homunculus/) | 项目作用域 (projects/<hash>/) |
...
每个项目都会获得一个 12 字符的哈希 ID (例如 `a1b2c3d4e5f6`)。`~/.factory/homunculus/projects.json` 处的注册表文件将 ID 映射到人类可读的名称。

## 快速开始

### 1. 启用观察钩子

添加到你的 `~/.factory/settings.json` 中。
...
* `~/.factory/homunculus/instincts/` 中现有的全局本能仍然作为全局本能工作
* 来自 v1 的现有 `~/.factory/skills/learned/` 技能仍然有效
