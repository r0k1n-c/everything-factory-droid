...
一个后台代理，用于分析 Factory Droid 会话中的观察结果，以检测模式并创建本能。
...
* 项目：`~/.factory/homunculus/projects/<project-hash>/observations.jsonl`
* 全局后备：`~/.factory/homunculus/observations.jsonl`
...
* 项目：`~/.factory/homunculus/projects/<project-hash>/instincts/personal/`
* 全局：`~/.factory/homunculus/instincts/personal/`（用于通用模式）
