...
description: 自动从Factory Droid会话中提取可重复使用的模式，并将其保存为学习到的技能以供将来使用。
origin: EFD
---

# 持续学习技能

自动评估 Factory Droid 会话的结尾，以提取可重用的模式，这些模式可以保存为学习到的技能。

## 何时激活

* 设置从 Factory Droid 会话中自动提取模式
* 为会话评估配置停止钩子
* 在 `~/.factory/skills/learned/` 中审查或整理已学习的技能
* 调整提取阈值或模式类别
...
2. **模式检测**：从会话中识别可提取的模式
3. **技能提取**：将有用的模式保存到 `~/.factory/skills/learned/`
