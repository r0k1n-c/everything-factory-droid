...
一个用于 Factory Droid 会话的正式评估框架，实现了评估驱动开发 (EDD) 原则。

## 何时激活

* 为 AI 辅助工作流程设置评估驱动开发 (EDD)
* 定义 Factory Droid 任务完成的标准（通过/失败）
* 使用 pass@k 指标衡量代理可靠性
* 为提示或代理变更创建回归测试套件
* 跨模型版本对代理性能进行基准测试
...
在 `.factory/evals/feature-name.md` 处创建评估定义文件
...
* `.factory/evals/<feature>.md` 定义
* `.factory/evals/<feature>.log` 运行历史
* `docs/evals/<version>/summary.md` 发布快照
