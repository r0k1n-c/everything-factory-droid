...
Plankton（作者：@alxfazio）的集成参考，这是一个用于 Factory Droid 的编写时代码质量强制执行系统。Plankton 通过 PostToolUse 钩子在每次文件编辑时运行格式化程序和 linter，然后生成 Claude 子进程来修复代理未捕获的违规。
...
每次 Factory Droid 编辑或写入文件时，Plankton 的 `multi_linter.sh` PostToolUse 钩子都会运行：
...
无需安装命令，无需插件配置。当你运行 Factory Droid 时，`.factory/settings.json` 中的钩子会在 Plankton 目录中被自动拾取。

### 按项目集成

要在你自己的项目中使用 Plankton 钩子：

1. 将 `.factory/hooks/` 目录复制到你的项目
2. 复制 `.factory/settings.json` 钩子配置
...
1. 将 EFD 安装为你的插件（代理、技能、命令、规则）
2. 添加 Plankton 钩子以实现编写时质量强制执行
3. 使用 AgentShield 进行安全审计
4. 在 PR 之前使用 EFD 的 verification-loop 作为最后一道关卡

### 避免钩子冲突

如果同时运行 EFD 和 Plankton 钩子：

* EFD 的 Prettier 钩子和 Plankton 的 biome 格式化程序可能在 JS/TS 文件上冲突
* 解决方案：使用 Plankton 时禁用 EFD 的 Prettier PostToolUse 钩子（Plankton 的 biome 更全面）
* 两者可以在不同的文件类型上共存（EFD 处理 Plankton 未覆盖的内容）

## 配置参考

Plankton 的 `.factory/hooks/config.json` 控制所有行为：
