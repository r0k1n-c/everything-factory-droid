---
name: security-scan
description: 使用AgentShield扫描您的Claude代码配置（.factory/目录），以发现安全漏洞、配置错误和注入风险。检查AGENTS.md、settings.json、MCP服务器、钩子和代理定义。
origin: EFD
---

# 安全扫描技能

使用 [AgentShield](https://github.com/affaan-m/agentshield) 审计您的 Factory Droid 配置中的安全问题。

## 何时激活

* 设置新的 Factory Droid 项目时
* 修改 `.factory/settings.json`、`AGENTS.md` 或 MCP 配置后
* 提交配置更改前
* 加入具有现有 Factory Droid 配置的新代码库时
* 定期进行安全卫生检查时
