---
description: "用于审计 Droid 技能和命令的质量。支持快速扫描（仅变更技能）和全面盘点模式，采用顺序子代理批量评估。"
origin: EFD
---

# skill-stocktake

斜杠命令 (`/skill-stocktake`)，用于使用质量检查清单 + AI 整体判断来审核所有 Droid 技能和命令。支持两种模式：用于最近更改技能的快速扫描，以及用于完整审查的全面盘点。
...
| 路径 | 描述 |
|------|-------------|
| `~/.factory/skills/` | 全局技能（所有项目） |
| `{cwd}/.factory/skills/` | 项目级技能（如果目录存在） |
...
如果项目没有 `.factory/skills/` 目录，则只评估全局技能和命令。
...
**结果缓存：** `~/.factory/skills/skill-stocktake/results.json`

## 快速扫描流程

仅重新评估自上次运行以来发生更改的技能（5–10 分钟）。

1. 读取 `~/.factory/skills/skill-stocktake/results.json`
2. 运行：`bash ~/.factory/skills/skill-stocktake/scripts/quick-diff.sh \   ~/.factory/skills/skill-stocktake/results.json`
   （项目目录从 `$PWD/.factory/skills` 自动检测；仅在需要时显式传递）
3. 如果输出是 `[]`：报告“自上次运行以来无更改。”并停止
4. 使用相同的第 2 阶段标准仅重新评估那些已更改的文件
5. 沿用先前结果中未更改的技能
6. 仅输出差异
7. 运行：`bash ~/.factory/skills/skill-stocktake/scripts/save-results.sh \   ~/.factory/skills/skill-stocktake/results.json <<< "$EVAL_RESULTS"`

## 全面盘点流程

### 第 1 阶段 — 清单

运行：`bash ~/.factory/skills/skill-stocktake/scripts/scan.sh`
...
```
扫描中：
  ✓ ~/.factory/skills/         (17 个文件)
  ✗ {cwd}/.factory/skills/    (未找到 — 仅限全局技能)
```
...
* 评估是盲目的：无论来源如何（EFD、自创、自动提取），所有技能都应用相同的检查清单
* 归档 / 删除操作始终需要明确的用户确认
* 不按技能来源进行判定分支
