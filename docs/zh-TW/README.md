...
[**English**](../../README.md) | [简体中文](../zh-CN/README.md) | [繁體中文](README.md) | [日本語](../../docs/ja-JP/README.md) | [한국어](../ko-KR/README.md)

</div>

---

**面向現代工程團隊的完整 Factory Droid 工作流系統。**

一組為 Factory Droid 準備的精選內容，涵蓋 agents、skills、hooks、rules、MCP 設定、安裝器與指令相容層，用於支援規劃、實作、審查、安全、研究與營運工作流。
...
<img src="https://github.com/user-attachments/assets/1a471488-59cc-425b-8345-5245c7efbcef" alt="Everything Factory Droid 簡明指南" />
</a>
</td>
<td width="50%">
<a href="https://x.com/affaanmustafa/status/2014040193557471352">
<img src="https://github.com/user-attachments/assets/c9ca43bc-b149-427f-b551-af6840c368f0" alt="Everything Factory Droid 完整指南" />
</a>
</td>
</tr>
<tr>
...
> WARNING: **重要提示：** Factory Droid 外掛程式無法自動分發 `rules`，需要手動安裝：
...
1. **環境變數**：`FACTORY_DROID_PACKAGE_MANAGER`
2. **專案設定**：`.factory/package-manager.json`
3. **package.json**：`packageManager` 欄位
4. **鎖定檔案**：從 package-lock.json、yarn.lock、pnpm-lock.yaml 或 bun.lockb 偵測
5. **全域設定**：`~/.factory/package-manager.json`
6. **備援方案**：第一個可用的套件管理器
...
或在 Factory Droid 中使用 `/setup-pm` 指令。

---

## 內容概覽

本儲存庫是一個 **Factory Droid 外掛程式** - 可直接安裝或手動複製元件。
...
|
|-- rules/            # 必須遵守的準則（複製到 ~/.factory/rules/）
|   |-- security.md         # 強制性安全檢查
...
從您的儲存庫自動生成 Factory Droid 技能。

[安裝 GitHub App](https://github.com/apps/skill-creator) | [efd.tools](https://efd.tools)

分析您的儲存庫並建立：
- **SKILL.md 檔案** - 可直接用於 Factory Droid 的技能
...
使用本儲存庫最簡單的方式 - 安裝為 Factory Droid 外掛程式：
...
cp -r everything-factory-droid/rules/typescript ~/.factory/rules/   # 依您的技術棧選擇
...
將 `hooks/hooks.json` 中的鉤子複製到您的 `~/.factory/settings.json`。

#### 設定 MCP

將 `mcp-configs/mcp-servers.json` 中需要的 MCP 伺服器定義複製到官方 Factory Droid 設定 `~/.factory/settings.json`，或複製到專案級 `.mcp.json` 以啟用儲存庫本地 MCP。
...
這套設定來自長期的 Factory Droid 日常使用，並已在多個真實產品工作流中驗證。
...
- **簡明指南（從這裡開始）：** [Everything Factory Droid 簡明指南](https://x.com/affaanmustafa/status/2012378465664745795)
- **完整指南（進階）：** [Everything Factory Droid 完整指南](https://x.com/affaanmustafa/status/2014040193557471352)
