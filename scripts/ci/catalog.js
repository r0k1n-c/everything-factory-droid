...
const DOCS_PT_BR_README_PATH = path.join(ROOT, 'docs', 'pt-BR', 'README.md');
const DOCS_TR_README_PATH = path.join(ROOT, 'docs', 'tr', 'README.md');
...
function parseReadmeExpectations(readmeContent) {
  ...
  const parityPatterns = [
    {
      category: 'agents',
      regex: /^\|\s*(?:\*\*)?Agents(?:\*\*)?\s*\|\s*(\d+)\s+agents\s*\|\s*Factory Droid\s*\|$/im,
      source: 'README.md parity table'
    },
    {
      category: 'commands',
      regex: /^\|\s*(?:\*\*)?Commands(?:\*\*)?\s*\|\s*(\d+)\s+commands\s*\|\s*Factory Droid\s*\|$/im,
      source: 'README.md parity table'
    },
    {
      category: 'skills',
      regex: /^\|\s*(?:\*\*)?Skills(?:\*\*)?\s*\|\s*(\d+)\s+skills\s*\|\s*Factory Droid\s*\|$/im,
      source: 'README.md parity table'
    }
  ];
...
function parseZhDocsReadmeExpectations(readmeContent) {
  ...
  const tablePatterns = [
    { category: 'agents', regex: /\|\s*智能体\s*\|\s*(?:(?:PASS:|\u2705)\s*)?(\d+)\s*个\s*\|\s*Factory Droid\s*\|/i, source: 'docs/zh-CN/README.md comparison table' },
    { category: 'commands', regex: /\|\s*命令\s*\|\s*(?:(?:PASS:|\u2705)\s*)?(\d+)\s*个\s*\|\s*Factory Droid\s*\|/i, source: 'docs/zh-CN/README.md comparison table' },
    { category: 'skills', regex: /\|\s*技能\s*\|\s*(?:(?:PASS:|\u2705)\s*)?(\d+)\s*项\s*\|\s*Factory Droid\s*\|/i, source: 'docs/zh-CN/README.md comparison table' }
  ];
  ...
  const parityPatterns = [
    {
      category: 'agents',
      regex: /^\|\s*(?:\*\*)?智能体(?:\*\*)?\s*\|\s*(\d+)\s+个\s*\|\s*Factory Droid\s*\|$/im,
      source: 'docs/zh-CN/README.md parity table'
    },
    {
      category: 'commands',
      regex: /^\|\s*(?:\*\*)?命令(?:\*\*)?\s*\|\s*(\d+)\s+个\s*\|\s*Factory Droid\s*\|$/im,
      source: 'docs/zh-CN/README.md parity table'
    },
    {
      category: 'skills',
      regex: /^\|\s*(?:\*\*)?技能(?:\*\*)?\s*\|\s*(\d+)\s+项\s*\|\s*Factory Droid\s*\|$/im,
      source: 'docs/zh-CN/README.md parity table'
    }
  ];
...
function parsePtBrReadmeExpectations(readmeContent) {
  const quickStartMatch = readmeContent.match(/Você agora tem acesso a\s+(\d+)\s+agentes,\s*(\d+)\s+skills\s+e\s*(\d+)\s+comandos/i);
  if (!quickStartMatch) {
    throw new Error('docs/pt-BR/README.md is missing the quick-start catalog summary');
  }

  if (!readmeContent.includes('|-- .factory/')) {
    throw new Error('docs/pt-BR/README.md is missing the .factory structure marker');
  }

  if (readmeContent.includes('|-- .claude-plugin/')) {
    throw new Error('docs/pt-BR/README.md still references the legacy .claude-plugin layout');
  }

  const agentsStructureMatch = readmeContent.match(/\|--\s*agents\/\s*#\s*(\d+)\s+subagentes/i);
  if (!agentsStructureMatch) {
    throw new Error('docs/pt-BR/README.md is missing the agents structure row');
  }

  return [
    { category: 'agents', mode: 'exact', expected: Number(quickStartMatch[1]), source: 'docs/pt-BR/README.md quick-start summary' },
    { category: 'skills', mode: 'exact', expected: Number(quickStartMatch[2]), source: 'docs/pt-BR/README.md quick-start summary' },
    { category: 'commands', mode: 'exact', expected: Number(quickStartMatch[3]), source: 'docs/pt-BR/README.md quick-start summary' },
    { category: 'agents', mode: 'exact', expected: Number(agentsStructureMatch[1]), source: 'docs/pt-BR/README.md structure row' }
  ];
}

function parseTrReadmeExpectations(readmeContent) {
  const quickStartMatch = readmeContent.match(/Artık\s+(\d+)\s+agent,\s*(\d+)\s+skill\s+ve\s*(\d+)\s+command(?:['’]a)?/i);
  if (!quickStartMatch) {
    throw new Error('docs/tr/README.md is missing the quick-start catalog summary');
  }

  if (!readmeContent.includes('|-- .factory/')) {
    throw new Error('docs/tr/README.md is missing the .factory structure marker');
  }

  if (readmeContent.includes('|-- .claude-plugin/')) {
    throw new Error('docs/tr/README.md still references the legacy .claude-plugin layout');
  }

  const agentsStructureMatch = readmeContent.match(/\|--\s*agents\/\s*#.*?(\d+)\s+özel subagent/i);
  if (!agentsStructureMatch) {
    throw new Error('docs/tr/README.md is missing the agents structure row');
  }

  return [
    { category: 'agents', mode: 'exact', expected: Number(quickStartMatch[1]), source: 'docs/tr/README.md quick-start summary' },
    { category: 'skills', mode: 'exact', expected: Number(quickStartMatch[2]), source: 'docs/tr/README.md quick-start summary' },
    { category: 'commands', mode: 'exact', expected: Number(quickStartMatch[3]), source: 'docs/tr/README.md quick-start summary' },
    { category: 'agents', mode: 'exact', expected: Number(agentsStructureMatch[1]), source: 'docs/tr/README.md structure row' }
  ];
}
...
function syncEnglishReadme(content, catalog) {
  ...
  nextContent = replaceOrThrow(
    nextContent,
    /^(\|\s*(?:\*\*)?Agents(?:\*\*)?\s*\|\s*)(\d+)(\s+agents\s*\|\s*Factory Droid\s*\|)$/im,
    (_, prefix, __, suffix) => `${prefix}${catalog.agents.count}${suffix}`,
    'README.md parity table (agents)'
  );
  nextContent = replaceOrThrow(
    nextContent,
    /^(\|\s*(?:\*\*)?Commands(?:\*\*)?\s*\|\s*)(\d+)(\s+commands\s*\|\s*Factory Droid\s*\|)$/im,
    (_, prefix, __, suffix) => `${prefix}${catalog.commands.count}${suffix}`,
    'README.md parity table (commands)'
  );
  nextContent = replaceOrThrow(
    nextContent,
    /^(\|\s*(?:\*\*)?Skills(?:\*\*)?\s*\|\s*)(\d+)(\s+skills\s*\|\s*Factory Droid\s*\|)$/im,
    (_, prefix, __, suffix) => `${prefix}${catalog.skills.count}${suffix}`,
...
function syncZhDocsReadme(content, catalog) {
  ...
  nextContent = replaceOrThrow(
    nextContent,
    /(\|\s*智能体\s*\|\s*(?:(?:PASS:|\u2705)\s*)?)(\d+)(\s*个\s*\|\s*Factory Droid\s*\|)/i,
    (_, prefix, __, suffix) => `${prefix}${catalog.agents.count}${suffix}`,
    'docs/zh-CN/README.md comparison table (agents)'
  );
  nextContent = replaceOrThrow(
    nextContent,
    /(\|\s*命令\s*\|\s*(?:(?:PASS:|\u2705)\s*)?)(\d+)(\s*个\s*\|\s*Factory Droid\s*\|)/i,
    (_, prefix, __, suffix) => `${prefix}${catalog.commands.count}${suffix}`,
    'docs/zh-CN/README.md comparison table (commands)'
  );
  nextContent = replaceOrThrow(
    nextContent,
    /(\|\s*技能\s*\|\s*(?:(?:PASS:|\u2705)\s*)?)(\d+)(\s*项\s*\|\s*Factory Droid\s*\|)/i,
    (_, prefix, __, suffix) => `${prefix}${catalog.skills.count}${suffix}`,
    'docs/zh-CN/README.md comparison table (skills)'
  );
  nextContent = replaceOrThrow(
    nextContent,
    /^(\|\s*(?:\*\*)?智能体(?:\*\*)?\s*\|\s*)(\d+)(\s+个\s*\|\s*Factory Droid\s*\|)$/im,
    (_, prefix, __, suffix) => `${prefix}${catalog.agents.count}${suffix}`,
    'docs/zh-CN/README.md parity table (agents)'
  );
  nextContent = replaceOrThrow(
    nextContent,
    /^(\|\s*(?:\*\*)?命令(?:\*\*)?\s*\|\s*)(\d+)(\s+个\s*\|\s*Factory Droid\s*\|)$/im,
    (_, prefix, __, suffix) => `${prefix}${catalog.commands.count}${suffix}`,
    'docs/zh-CN/README.md parity table (commands)'
  );
  nextContent = replaceOrThrow(
    nextContent,
    /^(\|\s*(?:\*\*)?技能(?:\*\*)?\s*\|\s*)(\d+)(\s+项\s*\|\s*Factory Droid\s*\|)$/im,
    (_, prefix, __, suffix) => `${prefix}${catalog.skills.count}${suffix}`,
...
function syncPtBrReadme(content, catalog) {
  let nextContent = content;

  nextContent = replaceOrThrow(
    nextContent,
    /(Você agora tem acesso a\s+)(\d+)(\s+agentes,\s*)(\d+)(\s+skills\s+e\s*)(\d+)(\s+comandos)/i,
    (_, prefix, __, agentsSuffix, ___, skillsSuffix, ____, commandsSuffix) =>
      `${prefix}${catalog.agents.count}${agentsSuffix}${catalog.skills.count}${skillsSuffix}${catalog.commands.count}${commandsSuffix}`,
    'docs/pt-BR/README.md quick-start summary'
  );

  if (nextContent.includes('|-- .claude-plugin/')) {
    nextContent = replaceOrThrow(
      nextContent,
  ...
  if (!nextContent.includes('|-- .factory/')) {
    throw new Error('docs/pt-BR/README.md is missing the expected .factory structure marker');
  }

  nextContent = replaceOrThrow(
    nextContent,
    /(\|--\s*agents\/\s*#\s*)(\d+)(\s+subagentes)/i,
    (_, prefix, __, suffix) => `${prefix}${catalog.agents.count}${suffix}`,
    'docs/pt-BR/README.md structure row'
  );

  return nextContent;
}

function syncTrReadme(content, catalog) {
  let nextContent = content;

  nextContent = replaceOrThrow(
    nextContent,
    /(Artık\s+)(\d+)(\s+agent,\s*)(\d+)(\s+skill\s+ve\s*)(\d+)(\s+command(?:['’]a)?)/i,
    (_, prefix, __, agentsSuffix, ___, skillsSuffix, ____, commandsSuffix) =>
      `${prefix}${catalog.agents.count}${agentsSuffix}${catalog.skills.count}${skillsSuffix}${catalog.commands.count}${commandsSuffix}`,
    'docs/tr/README.md quick-start summary'
  );
  ...
  if (!nextContent.includes('|-- .factory/')) {
    throw new Error('docs/tr/README.md is missing the expected .factory structure marker');
  }

  nextContent = replaceOrThrow(
    nextContent,
    /(\|--\s*agents\/\s*#.*?)(\d+)(\s+özel subagent)/i,
    (_, prefix, __, suffix) => `${prefix}${catalog.agents.count}${suffix}`,
    'docs/tr/README.md structure row'
  );
...
const DOCUMENT_SPECS = [
  ...
  {
    filePath: DOCS_PT_BR_README_PATH,
    parseExpectations: parsePtBrReadmeExpectations,
    syncContent: syncPtBrReadme,
  },
  {
    filePath: DOCS_TR_README_PATH,
    parseExpectations: parseTrReadmeExpectations,
    syncContent: syncTrReadme,
  },
];
...
function renderMarkdown(result) {
  const mismatches = result.checks.filter(check => !check.ok);
  console.log('# EFD Catalog Verification\n');
