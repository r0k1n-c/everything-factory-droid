#!/usr/bin/env node
/**
 * NanoClaw v2 — Barebones Agent REPL for Everything Factory Droid
 *
 * Zero external dependencies. Session-aware REPL around the configured Factory Droid CLI.
 */
...
const DEFAULT_MODEL = process.env.CLAW_MODEL || 'claude-sonnet-4-6';
...
function loadEFDContext(skillList) {
...
function askDroid(systemPrompt, history, userMessage, model) {
  ...
  if (result.status !== 0 && result.stderr) {
    return `[Error: droid exited with code ${result.status}: ${result.stderr.trim()}]`;
  }
...
function main() {
  ...
  let efdContext = loadEFDContext(state.skills);
  ...
  const prompt = () => {
    rl.question('claw> ', (input) => {
      ...
      if (line.startsWith('/load ')) {
        ...
        efdContext = loadEFDContext(state.skills);
      ...
      const response = askDroid(efdContext, history, line, state.model);
      console.log(`\n${response}\n`);
