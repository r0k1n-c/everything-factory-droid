...
function findPluginInstall(rootDir) {
  ...
  const directInstallRoots = [
    path.join(rootDir, '.factory'),
    homeDir && path.join(homeDir, '.factory'),
  ].filter(Boolean);
  for (const directInstallRoot of directInstallRoots) {
    if (fs.existsSync(path.join(directInstallRoot, 'scripts', 'lib', 'utils.js'))) {
      return directInstallRoot;
    }
  }

  const candidates = [
    path.join(rootDir, '.factory', 'plugins', 'everything-factory-droid', 'plugin.json'),
    path.join(rootDir, '.factory', 'plugins', 'everything-factory-droid@everything-factory-droid', 'plugin.json'),
    path.join(rootDir, '.factory', 'plugins', 'marketplace', 'everything-factory-droid', 'plugin.json'),
    homeDir && path.join(homeDir, '.factory', 'plugins', 'everything-factory-droid', 'plugin.json'),
    homeDir && path.join(homeDir, '.factory', 'plugins', 'everything-factory-droid@everything-factory-droid', 'plugin.json'),
    homeDir && path.join(homeDir, '.factory', 'plugins', 'marketplace', 'everything-factory-droid', 'plugin.json'),
  ].filter(Boolean);
  ...
  const cacheRoots = [
    path.join(rootDir, '.factory', 'plugins', 'cache', 'everything-factory-droid'),
    homeDir && path.join(homeDir, '.factory', 'plugins', 'cache', 'everything-factory-droid'),
  ].filter(Boolean);

  for (const cacheRoot of cacheRoots) {
    try {
      for (const orgEntry of fs.readdirSync(cacheRoot, { withFileTypes: true })) {
        if (!orgEntry.isDirectory()) continue;
        const orgPath = path.join(cacheRoot, orgEntry.name);
        for (const versionEntry of fs.readdirSync(orgPath, { withFileTypes: true })) {
          if (!versionEntry.isDirectory()) continue;
          const candidate = path.join(orgPath, versionEntry.name, 'plugin.json');
      ...
      // ignore unreadable / missing cache roots
    }
  }
...
function getConsumerChecks(rootDir) {
  ...
  const projectHooks = safeRead(rootDir, '.factory/settings.json');
  ...
  return [
    {
      ...
      points: 4,
      scopes: ['repo'],
      path: '~/.factory/plugins/everything-factory-droid/',
      description: 'Everything Factory Droid is installed for the active user or project',
      pass: Boolean(pluginInstall),
      fix: 'Install the EFD plugin for this user or project before auditing project-specific harness quality.',
    },
    {
      ...
      path: '.factory/',
      description: 'Project-specific harness overrides exist under .factory/',
      pass: countFiles(rootDir, '.factory/droids', '.md') > 0 ||
        countFiles(rootDir, '.factory/skills', 'SKILL.md') > 0 ||
        countFiles(rootDir, '.factory/commands', '.md') > 0 ||
        fileExists(rootDir, '.factory/settings.json') ||
        fileExists(rootDir, '.factory/hooks.json'),
      fix: 'Add project-local .factory hooks, commands, skills, droids, or settings that tailor EFD to this repo.',
    },
    {
      ...
      pass: fileExists(rootDir, 'AGENTS.md') || fileExists(rootDir, '.factory/AGENTS.md'),
      fix: 'Add AGENTS.md so the harness has project-specific instructions.',
    },
    {
      ...
      scopes: ['repo', 'hooks'],
      path: '.mcp.json',
      description: 'The project declares local MCP or Factory Droid settings',
      pass: fileExists(rootDir, '.mcp.json') || fileExists(rootDir, '.factory/settings.json') || fileExists(rootDir, '.factory/settings.local.json'),
      fix: 'Add .mcp.json or .factory/settings.json so project-local tool configuration is explicit.',
    },
    {
    ...
    },
    {
      ...
      points: 2,
      scopes: ['repo'],
      path: '.factory/memory.md',
      description: 'Project memory or durable notes are checked in',
      pass: fileExists(rootDir, '.factory/memory.md') || countFiles(rootDir, 'docs/adr', '.md') > 0,
      fix: 'Add durable project memory such as .factory/memory.md or ADRs under docs/adr/.',
    },
    {
    ...
    },
    {
      ...
      pass: projectHooks.includes('PreToolUse') || projectHooks.includes('beforeSubmitPrompt') || fileExists(rootDir, '.factory/hooks.json'),
...
Audits the current working directory by default and auto-detects EFD repo mode vs consumer-project mode.
`);
