 ...
 * Cost Tracker Hook
 *
 * Appends lightweight session usage metrics to ~/.factory/metrics/costs.jsonl.
 */
...
process.stdin.on('end', () => {
  try {
    ...
    const model = String(input.model || input._cursor?.model || process.env.FACTORY_MODEL || process.env.CLAUDE_MODEL || 'unknown');
    const sessionId = String(process.env.FACTORY_SESSION_ID || process.env.CLAUDE_SESSION_ID || 'default');

    const metricsDir = path.join(getFactoryDir(), 'metrics');
