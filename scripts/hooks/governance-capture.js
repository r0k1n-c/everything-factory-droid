 ...
 *
 * Enable: Set EFD_GOVERNANCE_CAPTURE=1
 * Configure session: Set EFD_SESSION_ID for session correlation
 */
...
function run(rawInput, options = {}) {
  ...
  if (String(process.env.EFD_GOVERNANCE_CAPTURE || '').toLowerCase() !== '1') {
    return rawInput;
  }

  const sessionId = process.env.EFD_SESSION_ID || process.env.FACTORY_SESSION_ID || process.env.CLAUDE_SESSION_ID || null;
  const hookPhase = process.env.FACTORY_HOOK_EVENT_NAME || process.env.CLAUDE_HOOK_EVENT_NAME || 'unknown';
...
if (require.main === module) {
  ...
  process.stdin.on('end', () => {
    const result = run(raw, {
      truncated,
      maxStdin: Number(process.env.EFD_HOOK_INPUT_MAX_BYTES) || MAX_STDIN,
    });
