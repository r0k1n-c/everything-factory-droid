#!/usr/bin/env python3
"""
InsAIts Security Monitor -- PreToolUse Hook for Factory Droid
============================================================

Real-time security monitoring for Factory Droid tool inputs.
...
How it works:
  Factory Droid passes tool input as JSON on stdin.
  ...
  Stderr output = non-blocking warning shown to Droid.

Environment variables:
  INSAITS_DEV_MODE   Set to "true" to enable dev mode (no API key needed).
                     Defaults to "false" (strict mode).
  INSAITS_MODEL      LLM model identifier for fingerprinting. Default: claude-opus-4-6.
...
def extract_content(data: Dict[str, Any]) -> Tuple[str, str]:
    """Extract inspectable text from a Factory Droid tool input payload.
...
def format_feedback(anomalies: List[Any]) -> str:
    """Format detected anomalies as feedback for Factory Droid.
...
def main() -> None:
    """Entry point for the Factory Droid PreToolUse hook."""
    ...
    try:
        monitor: insAItsMonitor = insAItsMonitor(
            session_name="factory-droid-hook",
    ...
    if has_critical:
        # stdout feedback -> Factory Droid shows to the model
