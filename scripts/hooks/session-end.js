...
const SUMMARY_START_MARKER = '<!-- EFD:SUMMARY:START -->';
const SUMMARY_END_MARKER = '<!-- EFD:SUMMARY:END -->';
  ...
  for (const line of lines) {
    try {
      ...
      if (entry.type === 'user' || entry.role === 'user' || entry.message?.role === 'user') {
        // Support both direct content and nested message.content (Factory Droid JSONL format)
      ...
      // Extract tool uses from assistant message content blocks (Factory Droid JSONL format)
...
// Read hook input from stdin (Factory Droid provides transcript_path via stdin JSON)
...
function getSessionMetadata(overrides = {}) {
  const branchResult = runCommand('git rev-parse --abbrev-ref HEAD');

  return {
    project: overrides.projectName || getProjectName() || 'unknown',
...
async function main() {
  ...
  try {
    ...
    transcriptPath = process.env.FACTORY_TRANSCRIPT_PATH || process.env.CLAUDE_TRANSCRIPT_PATH;
  }
  ...
  reportLegacyMigrationErrors(migrateLegacySessions(), 'SessionEnd');
  ...
  const currentProjectName = observerContext.projectRoot
    ...
    : (getProjectName() || '');
  const currentWorktree = observerContext.projectRoot
    && path.resolve(process.cwd()) !== path.resolve(observerContext.projectRoot)
    && !path.resolve(process.cwd()).startsWith(`${path.resolve(observerContext.projectRoot)}${path.sep}`)
    ? observerContext.projectRoot
    : process.cwd();
  migrateProjectSessions({
    currentProjectName,
  ...
  const shortId = (process.env.FACTORY_SESSION_ID || process.env.CLAUDE_SESSION_ID)
    ? getSessionIdShort()
    : (sanitizeSessionId(currentProjectName) || getSessionIdShort());
  const sessionFile = path.join(sessionsDir, `${today}-${shortId}-session.tmp`);
  const sessionMetadata = getSessionMetadata({ projectName: currentProjectName, worktree: currentWorktree });
  ...
  if (fs.existsSync(sessionFile)) {
    ...
    info(`[SessionEnd] Updated session file: ${sessionFile}`);
  } else {
...
`;
    ...
    info(`[SessionEnd] Created session file: ${sessionFile}`);
  }
