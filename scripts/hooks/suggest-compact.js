...
async function main() {
  ...
  const sessionId = (process.env.FACTORY_SESSION_ID || process.env.CLAUDE_SESSION_ID || 'default').replace(/[^a-zA-Z0-9_-]/g, '') || 'default';
  const counterFile = path.join(getTempDir(), `droid-tool-count-${sessionId}`);
