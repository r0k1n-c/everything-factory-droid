...
function planFactoryDroidLegacyInstall(context) {
  const adapter = getInstallTargetAdapter('factory-droid');
  const targetRoot = adapter.resolveRoot({ projectRoot: context.projectRoot, repoRoot: context.projectRoot });
  const rulesDir = path.join(targetRoot, '.factory', 'rules');
  const installStatePath = adapter.getInstallStatePath({ projectRoot: context.projectRoot, repoRoot: context.projectRoot });
  const operations = [];
  const warnings = [];
  ...
  addRecursiveCopyOperations(operations, {
    moduleId: 'legacy-factory-droid-rules',
  ...
  return {
    ...
    operations,
    warnings,
    selectedModules: ['legacy-factory-droid-rules'],
  };
}
...
function createLegacyInstallPlan(options = {}) {
  ...
  const target = options.target || 'factory-droid';
  ...
  const context = {
    ...
    rulesDir: options.rulesDir || process.env.FACTORY_RULES_DIR || process.env.CLAUDE_RULES_DIR || null,
  };

  let plan;
  if (target === 'factory-droid') {
    plan = planFactoryDroidLegacyInstall(context);
