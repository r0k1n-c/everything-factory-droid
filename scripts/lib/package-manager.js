...
const { commandExists, getFactoryDir, readFile, writeFile } = require('./utils');
...
function getConfigPath() {
  return path.join(getFactoryDir(), 'package-manager.json');
}
 ...
 * 2. Project-specific config (in .factory/package-manager.json)
 * 3. package.json packageManager field
 * 4. Lock file detection
 * 5. Global user preference (in ~/.factory/package-manager.json)
...
function getPackageManager(options = {}) {
  ...
  const envPm = process.env.FACTORY_DROID_PACKAGE_MANAGER;
 ...
 * Returns a message for Droid to show to user
 *
...
function getSelectionPrompt() {
  ...
  message += '  - Global: Set FACTORY_DROID_PACKAGE_MANAGER environment variable\n';
  message += '  - Or add to ~/.factory/package-manager.json: {"packageManager": "pnpm"}\n';
