...
const SESSION_SCHEMA_VERSION = 'efd.session.v1';
const SESSION_RECORDING_SCHEMA_VERSION = 'efd.session.recording.v1';
const DEFAULT_RECORDING_DIR = path.join(os.tmpdir(), 'efd-session-recordings');
...
function resolveRecordingDir(options = {}) {
  ...
  if (typeof process.env.EFD_SESSION_RECORDING_DIR === 'string' && process.env.EFD_SESSION_RECORDING_DIR.length > 0) {
    return path.resolve(process.env.EFD_SESSION_RECORDING_DIR);
  }
...
function normalizeFactoryHistorySession(session, sourceTarget) {
