...
/** Get the canonical EFD sessions directory (~/.factory/session-data). */
export function getSessionsDir(): string;

/** Get session directories to search. */
export function getSessionSearchDirs(): string[];

/** Get the learned skills directory (~/.factory/skills/learned) */
...
/** Import legacy Claude-managed sessions into the canonical EFD session-data directory. */
...
/** Log unresolved legacy session migration issues for the current caller context. */
export function reportLegacyMigrationErrors(
  result: {
    migrated: number;
    removedDuplicates: number;
    removedDirs: number;
...
/** Rename fallback project-named session files when a project directory basename changes. */
export function migrateProjectSessions(options?: {
  currentProjectName?: string;
  currentProjectRoot?: string;
  previousProjects?: Array<string | { name?: string; root?: string }>;
}): {
  migrated: number;
  removedDuplicates: number;
  errors: Array<{ path: string; message: string }>;
...
/** Log a message to stderr (visible to user in Factory Droid terminal) */
export function log(message: string): void;

/** Output data to stdout (returned to Droid's context) */
