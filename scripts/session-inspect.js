...
function usage() {
  console.log([
    ...
    '  factory:latest       Most recent Factory Droid session history entry',
    '  factory:<id|alias>   Specific Factory Droid session or alias',
    '  <session.tmp>        Direct path to a Factory Droid session file',
    ...
    '',
    'Examples:',
    '  node scripts/session-inspect.js .factory/plan/workflow.json',
    '  node scripts/session-inspect.js workflow-visual-proof',
    '  node scripts/session-inspect.js factory:latest',
    '  node scripts/session-inspect.js latest --target-type factory-history',
    '  node scripts/session-inspect.js skills:health',
    '  node scripts/session-inspect.js skills:amendify --skill api-design',
    '  node scripts/session-inspect.js factory:a1b2c3d4 --write /tmp/session.json'
  ].join('\n'));
}
