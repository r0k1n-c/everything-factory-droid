...
function showHelp() {
  console.log(`
Package Manager Setup for Factory Droid
...
Options:
  ...
  --global <pm>   Set global preference (saves to ~/.factory/package-manager.json)
  --project <pm>  Set project preference (saves to .factory/package-manager.json)
...
function detectAndShow() {
  ...
  console.log(`  Environment var: ${process.env.FACTORY_DROID_PACKAGE_MANAGER || 'not set'}`);
  console.log('');
...
function setGlobal(pmName) {
  ...
  try {
    ...
    console.log('  Saved to: ~/.factory/package-manager.json');
    console.log('');
  } catch (err) {
