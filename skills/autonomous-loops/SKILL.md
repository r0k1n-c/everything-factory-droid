...
Patterns, architectures, and reference implementations for running Factory Droid autonomously in loops. Covers everything from simple `droid exec` pipelines to full RFC-driven multi-agent DAG orchestration.
...
|---------|-----------|----------|
| [Sequential Pipeline](#1-sequential-pipeline-droid-exec) | Low | Daily dev steps, scripted workflows |
...
| [Continuous Droid PR Loop](#4-continuous-droid-pr-loop) | Medium | Multi-day iterative projects with CI gates |
...
## 1. Sequential Pipeline (`droid exec`)

**The simplest loop.** Break daily development into a sequence of non-interactive `droid exec` calls. Each call is a focused step with a clear prompt.

### Core Insight

> If you can't figure out a loop like this, it means you can't even drive the LLM to fix your code in interactive mode.

The `droid exec` flag runs Factory Droid non-interactively with a prompt, exits when done. Chain calls to build a pipeline:
...
droid exec "Read the spec in docs/auth-spec.md. Implement OAuth2 login in src/auth/. Write tests first (TDD). Do NOT create any new documentation files."

# Step 2: De-sloppify (cleanup pass)
droid exec "Review all files changed by the previous commit. Remove any unnecessary type tests, overly defensive checks, or testing of language features (e.g., testing that TypeScript generics work). Keep real business logic tests. Run the test suite after cleanup."

# Step 3: Verify
droid exec "Run the full build, lint, type check, and test suite. Fix any failures. Do not add new features."

# Step 4: Commit
droid exec "Create a conventional commit for all staged changes. Use 'feat: add OAuth2 login flow' as the message."
```

### Key Design Principles

1. **Each step is isolated** — A fresh context window per `droid exec` call means no context bleed between steps.
...
droid exec --model opus "Analyze the codebase architecture and write a plan for adding caching..."

# Implement with Sonnet (fast, capable)
droid exec "Implement the caching layer according to the plan in docs/caching-plan.md..."

# Review with Opus (thorough)
droid exec --model opus "Review all changes for security issues, race conditions, and edge cases..."
```

**With environment context:**
```bash
# Pass context via files, not prompt length
echo "Focus areas: auth module, API rate limiting" > .factory-context.md
droid exec "Read .factory-context.md for priorities. Work through them in order."
rm .factory-context.md
```

**With `--allowedTools` restrictions:**
```bash
# Read-only analysis pass
droid exec --enabled-tools "Read,Grep,Glob" "Audit this codebase for security vulnerabilities..."

# Write-only implementation pass
droid exec --enabled-tools "Read,Write,Edit,Bash" "Implement the fixes from security-audit.md..."
```
...
**EFD's built-in persistent loop.** A session-aware REPL that calls `droid exec` synchronously with full conversation history.
...
1. Loads conversation history from `~/.factory/claw/{session}.md`
2. Each user message is sent to `droid exec` with full history as context
...
### Implementation via Factory Droid Commands

Create `.factory/commands/infinite.md`:
...
## 4. Continuous Droid PR Loop

**A production-grade shell script** that runs Factory Droid in a continuous loop, creating PRs, waiting for CI, and merging automatically. Created by AnandChowdhary (credit: @AnandChowdhary).

### Core Loop

```
┌─────────────────────────────────────────────────────┐
│  CONTINUOUS DROID ITERATION                        │
│                                                     │
│  1. Create branch (continuous-droid/iteration-N)   │
│  2. Run droid exec with enhanced prompt              │
│  3. (Optional) Reviewer pass — separate droid exec   │
│  4. Commit changes (droid generates message)       │
│  5. Push + create PR (gh pr create)                 │
│  6. Wait for CI checks (poll gh pr checks)          │
│  7. CI failure? → Auto-fix pass (droid exec)         │
...
> **Warning:** Use a reviewed continuous-droid wrapper in your environment. Do not pipe external scripts directly to bash.

### Usage

```bash
# Basic: 10 iterations
continuous-droid --prompt "Add unit tests for all untested functions" --max-runs 10

# Cost-limited
continuous-droid --prompt "Fix all linter errors" --max-cost 5.00

# Time-boxed
continuous-droid --prompt "Improve test coverage" --max-duration 8h
...
continuous-droid --prompt "Add tests" --max-runs 5 --worktree tests-worker &
continuous-droid --prompt "Refactor code" --max-runs 5 --worktree refactor-worker &
wait
```
...
Droid reads this file at iteration start and updates it at iteration end. This bridges the context gap between independent `droid exec` invocations.

### CI Failure Recovery

When PR checks fail, Continuous Droid automatically:
1. Fetches the failed run ID via `gh run list`
2. Spawns a new `droid exec` with CI fix context
3. Droid inspects logs via `gh run view`, fixes code, commits, pushes
4. Re-waits for checks (up to `--ci-retry-max` attempts)

### Completion Signal

Droid can signal "I'm done" by outputting a magic phrase:

```bash
continuous-droid \
  --prompt "Fix all bugs in the issue tracker" \
  --completion-signal "CONTINUOUS_DROID_PROJECT_COMPLETE" \
...
droid exec "Implement the feature with full TDD. Be thorough with tests."

# Step 2: De-sloppify (separate context, focused cleanup)
droid exec "Review all changes in the working tree. Remove:
...
for feature in "${features[@]}"; do
  # Implement
  droid exec "Implement $feature with TDD."

  # De-sloppify
  droid exec "Cleanup pass: review changes, remove test/code slop, run tests."

  # Verify
  droid exec "Run build + lint + tests. Fix any failures."

  # Commit
  droid exec "Commit with message: feat: add $feature"
done
```
...
| Multi-day project | Yes | Maybe (continuous-droid) |
...
└─ No → Is there a written spec/RFC?
         ...
         │        └─ No → Continuous Droid (iterative PR loop)
...
2. **Continuous Droid + De-Sloppify** — Add `--review-prompt` with a de-sloppify directive to each iteration.

3. **Any loop + Verification** — Use EFD's `/verify` command or `verification-loop` skill as a gate before commits.
...
4. **Ralphinho's tiered approach in simpler loops** — Even in a sequential pipeline, you can route simple tasks to Haiku and complex tasks to Opus:
   ...
   droid exec --model haiku "Fix the import ordering in src/utils.ts"

   # Complex architectural change
   droid exec --model opus "Refactor the auth module to use the strategy pattern"
   ```
...
2. **No context bridge between iterations** — Each `droid exec` call starts fresh. Use `SHARED_TASK_NOTES.md` or filesystem state to bridge context.
...
| Continuous Droid | AnandChowdhary | credit: @AnandChowdhary |
| NanoClaw | EFD | `/claw` command in this repo |
| Verification Loop | EFD | `skills/verification-loop/` in this repo |
