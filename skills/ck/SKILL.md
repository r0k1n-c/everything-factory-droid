...
Scripts live at: `~/.factory/skills/ck/commands/` (expand `~` with `$HOME`).
...
```bash
node "$HOME/.factory/skills/ck/commands/init.mjs"
```
...
```bash
echo '<confirmed-json>' | node "$HOME/.factory/skills/ck/commands/save.mjs" --init
```
...
```bash
echo '<json>' | node "$HOME/.factory/skills/ck/commands/save.mjs"
```
...
```bash
node "$HOME/.factory/skills/ck/commands/resume.mjs" [arg]
```
...
```bash
node "$HOME/.factory/skills/ck/commands/info.mjs" [arg]
```
...
```bash
node "$HOME/.factory/skills/ck/commands/list.mjs"
```
...
If yes:
```bash
node "$HOME/.factory/skills/ck/commands/forget.mjs" [name]
```
...
```bash
node "$HOME/.factory/skills/ck/commands/migrate.mjs"
```
For a dry run first:
```bash
node "$HOME/.factory/skills/ck/commands/migrate.mjs" --dry-run
```
...
The hook at `~/.factory/skills/ck/hooks/session-start.mjs` must be registered in
`~/.factory/settings.json` to auto-load project context on session start:

```json
{
  "hooks": {
    "SessionStart": [
      { "hooks": [{ "type": "command", "command": "node \"~/.factory/skills/ck/hooks/session-start.mjs\"" }] }
    ]
  }
}
```
...
unsaved sessions, git activity since last save, and goal mismatches vs AGENTS.md.
