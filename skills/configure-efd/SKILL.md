...
An interactive, step-by-step installation wizard for the Everything Factory Droid project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.

## When to Activate

- User says "configure efd", "install efd", "setup everything factory droid", or similar
- User wants to selectively install skills or rules from this project
- User wants to verify or fix an existing EFD installation
...
## Step 0: Clone EFD Repository

Before any installation, clone the latest EFD source to `/tmp`:
...
If the clone fails (network issues, etc.), use `AskUserQuestion` to ask the user to provide a local path to an existing EFD clone.
...
```
Question: "Where should EFD components be installed?"
Options:
  - "User-level (~/.factory/)" — "Applies to all your Factory Droid projects"
  - "Project-level (.factory/)" — "Applies only to the current project"
...
Default to **Core (recommended for new users)** — copy the selected core directories from `skills/` (including `search-first`). This bundle covers engineering, evals, verification, security, strategic compaction, frontend design, and Anthropic cross-functional skills (article-writing, content-engine, market-research, frontend-slides).
...
There are 8 selectable category groups below. The detailed confirmation lists that follow cover 45 skills across 8 categories, plus 1 standalone template. Use `AskUserQuestion` with `multiSelect: true`:
...
Options:
  ...
  - "Business & Content" — "Article writing, content engine, market research, investor workflows"
...
```bash
cp -r $EFD_ROOT/skills/<skill-name> $TARGET/skills/
```
...
```bash
# Common rules (copy whole directories into rules/)
cp -r $EFD_ROOT/rules/common $TARGET/rules/

# Language-specific rules (copy whole directories into rules/)
cp -r $EFD_ROOT/rules/typescript $TARGET/rules/   # if selected
cp -r $EFD_ROOT/rules/python $TARGET/rules/        # if selected
cp -r $EFD_ROOT/rules/golang $TARGET/rules/        # if selected
```
...
```bash
grep -rn "~/.factory/" $TARGET/skills/ $TARGET/rules/
grep -rn "../common/" $TARGET/rules/
grep -rn "skills/" $TARGET/skills/
```

**For project-level installs**, flag any references to `~/.factory/` paths:
- If a skill references `~/.factory/settings.json` — this is usually fine (settings are always user-level)
- If a skill references `~/.factory/skills/` or `~/.factory/rules/` — this may be broken if installed only at project level
...
- `continuous-learning-v2` references `~/.factory/homunculus/` directory
...
3. **Issue**: What's wrong (e.g., "references ~/.factory/skills/python-patterns but python-patterns was not installed")
4. **Suggested fix**: What to do (e.g., "install python-patterns skill" or "update path to .factory/skills/")
...
**Critical**: Only modify files in the installation target (`$TARGET/`), NEVER modify files in the source EFD repository (`$EFD_ROOT/`).
...
### "Skills not being picked up by Factory Droid"
- Verify the skill directory contains a `SKILL.md` file (not just loose .md files)
- For user-level: check `~/.factory/skills/<skill-name>/SKILL.md` exists
- For project-level: check `.factory/skills/<skill-name>/SKILL.md` exists

### "Rules not working"
- Rules should keep their language directory structure: `$TARGET/rules/common/coding-style.md` is correct, while flattening to `$TARGET/rules/coding-style.md` breaks the current layout
- Restart Factory Droid after installing rules

### "Path reference errors after project-level install"
- Some skills assume `~/.factory/` paths. Run Step 4 verification to find and fix these.
- For `continuous-learning-v2`, the `~/.factory/homunculus/` directory is always user-level — this is expected and not an error.
