...
# Factory Droid passes hook data via stdin as JSON.
#
...
# Can also be registered manually in ~/.factory/settings.json.
...
# Read JSON from stdin (Factory Droid hook format)
INPUT_JSON=$(cat)
...
#   - EFD observing its own Haiku observer sessions (self-loop)
#   - EFD observing other tools' automated sessions
...
[ "${EFD_HOOK_PROFILE:-standard}" = "minimal" ] && exit 0

# Layer 3: cooperative skip env var for automated sessions.
[ "${EFD_SKIP_OBSERVE:-0}" = "1" ] && exit 0

# Layer 4: subagent sessions are automated by definition.
_EFD_AGENT_ID=$(echo "$INPUT_JSON" | "$PYTHON_CMD" -c "import json,sys; print(json.load(sys.stdin).get('agent_id',''))" 2>/dev/null || true)
[ -n "$_EFD_AGENT_ID" ] && exit 0

# Layer 5: known observer-session path exclusions.
_EFD_SKIP_PATHS="${EFD_OBSERVE_SKIP_PATHS:-observer-sessions,.factory-mem}"
if [ -n "$STDIN_CWD" ]; then
  IFS=',' read -ra _EFD_SKIP_ARRAY <<< "$_EFD_SKIP_PATHS"
  for _pattern in "${_EFD_SKIP_ARRAY[@]}"; do
...
# Pass HOOK_PHASE via env var since Factory Droid does not include hook type in stdin JSON
...
try:
    ...
    # Factory Droid does NOT include a "hook_type" field in the stdin JSON,
    # so we rely on the shell argument ("pre" or "post") instead.
    hook_phase = os.environ.get("HOOK_PHASE", "post")
    event = "tool_start" if hook_phase == "pre" else "tool_complete"

    # Extract fields - Factory Droid hook format
...
# which caused runaway parallel Droid analysis processes.
SIGNAL_EVERY_N="${EFD_OBSERVER_SIGNAL_EVERY_N:-20}"
