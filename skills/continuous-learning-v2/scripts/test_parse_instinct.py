...
def project_tree(tmp_path):
    ...
    homunculus = tmp_path / ".factory" / "homunculus"
...
def test_detect_project_from_env(patch_globals, monkeypatch, tmp_path):
    ...
    project = detect_project()
    assert project["id"] == "my-repo"
    assert project["name"] == "my-repo"


def test_detect_project_migrates_legacy_hash_dirs(patch_globals, monkeypatch, tmp_path):
    fake_repo = tmp_path / "legacy-repo"
    fake_repo.mkdir()
    monkeypatch.setenv("CLAUDE_PROJECT_DIR", str(fake_repo))

    legacy_id = hashlib.sha256(str(fake_repo).encode()).hexdigest()[:12]
    legacy_dir = patch_globals["projects_dir"] / legacy_id / "instincts" / "personal"
    legacy_dir.mkdir(parents=True, exist_ok=True)
    (legacy_dir / "legacy.yaml").write_text(SAMPLE_INSTINCT_YAML)
    ...
    project = detect_project()
    assert project["id"] == "legacy-repo"
    assert (project["instincts_personal"] / "legacy.yaml").exists()
    assert not (patch_globals["projects_dir"] / legacy_id).exists()


def test_detect_project_migrates_renamed_basename_dirs(patch_globals, monkeypatch, tmp_path):
    parent_dir = tmp_path / "workspace"
    old_repo = parent_dir / "old-repo"
    new_repo = parent_dir / "new-repo"
    new_repo.mkdir(parents=True)
    monkeypatch.setenv("CLAUDE_PROJECT_DIR", str(new_repo))

    legacy_project_dir = patch_globals["projects_dir"] / "old-repo" / "instincts" / "personal"
    legacy_project_dir.mkdir(parents=True, exist_ok=True)
    (legacy_project_dir / "legacy.yaml").write_text(SAMPLE_INSTINCT_YAML)
    patch_globals["registry_file"].write_text(json.dumps({
        "old-repo": {
            "id": "old-repo",
            "name": "old-repo",
            "root": str(old_repo),
    ...
    ((patch_globals["projects_dir"] / "old-repo") / "project.json").write_text(json.dumps({
        "id": "old-repo",
        "name": "old-repo",
        "root": str(old_repo),
    }))
    ...
    def mock_run(cmd, **kwargs):
        if "rev-parse" in cmd:
            return SimpleNamespace(returncode=0, stdout=str(new_repo) + "\n", stderr="")
        if "get-url" in cmd:
            return SimpleNamespace(returncode=1, stdout="", stderr="no remote")
        return SimpleNamespace(returncode=1, stdout="", stderr="")
    ...
    project = detect_project()
    assert project["id"] == "new-repo"
    assert (project["instincts_personal"] / "legacy.yaml").exists()
    assert not (patch_globals["projects_dir"] / "old-repo").exists()
    registry = load_registry()
    assert "new-repo" in registry
    assert "old-repo" not in registry


def test_detect_project_suffixes_on_basename_collision(patch_globals, monkeypatch, tmp_path):
    fake_repo = tmp_path / "collision-repo"
    fake_repo.mkdir()
    monkeypatch.setenv("CLAUDE_PROJECT_DIR", str(fake_repo))

    existing_dir = patch_globals["projects_dir"] / "collision-repo"
    existing_dir.mkdir(parents=True, exist_ok=True)
    (existing_dir / "project.json").write_text(json.dumps({
        "id": "collision-repo",
        "name": "collision-repo",
        "root": str(tmp_path / "other-parent" / "collision-repo"),
    }))
    ...
    project = detect_project()
    assert project["id"] == "collision-repo-2"
