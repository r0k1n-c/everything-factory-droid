...
Hookify rules are markdown files with YAML frontmatter that define patterns to watch for and messages to show when those patterns match. Rules are stored in `.factory/hookify.{rule-name}.local.md` files.
...
Message to show Factory Droid when this rule triggers.
...
- **Naming**: `.factory/hookify.{descriptive-name}.local.md`
- **Gitignore**: Add `.factory/*.local.md` to `.gitignore`
