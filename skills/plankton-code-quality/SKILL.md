...
Integration reference for Plankton (credit: @alxfazio), a write-time code quality enforcement system for Factory Droid. Plankton runs formatters and linters on every file edit via PostToolUse hooks, then spawns Claude subprocesses to fix violations the agent didn't catch.
...
Every time Factory Droid edits or writes a file, Plankton's `multi_linter.sh` PostToolUse hook runs:
...
├─ Spawns droid exec subprocess with violations JSON
...
# Start Factory Droid — hooks activate automatically
droid
```

No install command, no plugin config. The hooks in `.factory/settings.json` are picked up automatically when you run Factory Droid in the Plankton directory.

### Per-Project Integration

To use Plankton hooks in your own project:

1. Copy `.factory/hooks/` directory to your project
2. Copy `.factory/settings.json` hook configuration
...
1. Install EFD as your plugin (agents, skills, commands, rules)
2. Add Plankton hooks for write-time quality enforcement
3. Use AgentShield for security audits
4. Use EFD's verification-loop as a final gate before PRs

### Avoiding Hook Conflicts

If running both EFD and Plankton hooks:
- EFD's Prettier hook and Plankton's biome formatter may conflict on JS/TS files
- Resolution: disable EFD's Prettier PostToolUse hook when using Plankton (Plankton's biome is more comprehensive)
- Both can coexist on different file types (EFD handles what Plankton doesn't cover)

## Configuration Reference

Plankton's `.factory/hooks/config.json` controls all behavior:
...
```bash
export EFD_HOOK_PROFILE=strict
export EFD_QUALITY_GATE_FIX=true
export EFD_QUALITY_GATE_STRICT=true
```
