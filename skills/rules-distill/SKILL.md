...
```bash
bash ~/.factory/skills/rules-distill/scripts/scan-skills.sh
```

#### 1b. Collect rules index

```bash
bash ~/.factory/skills/rules-distill/scripts/scan-rules.sh
```
...
Launch a general-purpose droid via the Task tool (for example `worker`) with the following prompt:
