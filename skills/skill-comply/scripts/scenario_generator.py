...
def generate_scenarios(
    ...
    Calls droid exec with the scenario_generator prompt, parses YAML output.
    """
    ...
    result = subprocess.run(
        [CLI_BIN, "exec", "--model", model, "--output-format", "text", prompt],
        capture_output=True,
        text=True,
        timeout=120,
    )
    ...
    if result.returncode != 0:
        raise RuntimeError(f"{CLI_BIN} exec failed: {result.stderr}")

    if not result.stdout.strip():
        raise RuntimeError(f"{CLI_BIN} -p returned empty output")
