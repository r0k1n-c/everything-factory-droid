...
function runCatalogValidator(overrides = {}) {
  ...
  const effectiveRoot = overrides.ROOT || repoRoot;
  const resolvedOverrides = {
    ROOT: effectiveRoot,
    README_PATH: path.join(effectiveRoot, 'README.md'),
    AGENTS_PATH: path.join(effectiveRoot, 'AGENTS.md'),
    README_ZH_CN_PATH: path.join(effectiveRoot, 'README.zh-CN.md'),
    DOCS_ZH_CN_README_PATH: path.join(effectiveRoot, 'docs', 'zh-CN', 'README.md'),
    DOCS_ZH_CN_AGENTS_PATH: path.join(effectiveRoot, 'docs', 'zh-CN', 'AGENTS.md'),
    DOCS_PT_BR_README_PATH: path.join(effectiveRoot, 'docs', 'pt-BR', 'README.md'),
    DOCS_TR_README_PATH: path.join(effectiveRoot, 'docs', 'tr', 'README.md'),
    ...overrides,
  };
...
function writeCatalogFixture(testDir, options = {}) {
  const {
    ...
    ptBrReadmeCounts = { agents: 1, skills: 1, commands: 1 },
    ptBrAgentsStructureCount = ptBrReadmeCounts.agents,
    ptBrLegacyLayout = false,
    trReadmeCounts = { agents: 1, skills: 1, commands: 1 },
    trAgentsStructureCount = trReadmeCounts.agents,
    trLegacyLayout = false,
  } = options;
  ...
  const ptBrReadmePath = path.join(testDir, 'docs', 'pt-BR', 'README.md');
  const trReadmePath = path.join(testDir, 'docs', 'tr', 'README.md');
  ...
  fs.mkdirSync(path.join(testDir, 'docs', 'pt-BR'), { recursive: true });
  ...
  fs.writeFileSync(
    readmePath,
    `Access to ${readmeCounts.agents} agents, ${readmeCounts.skills} skills, and ${readmeCounts.commands} commands.\n| Component | Catalog | Scope |\n|-----------|---------|-------|\n| Agents | PASS: ${readmeTableCounts.agents} agents | Factory Droid |\n| Commands | PASS: ${readmeTableCounts.commands} commands | Factory Droid |\n| Skills | PASS: ${readmeTableCounts.skills} skills | Factory Droid |\n\n| Feature | Count | Format |\n|-----------|-------|---------|\n| Skills | ${readmeUnrelatedSkillsCount} | .agents/skills/ |\n\n## Factory Droid Focus\n\n| Component | Catalog | Scope |\n|-----------|---------|-------|\n| Agents | ${readmeParityCounts.agents} agents | Factory Droid |\n| Commands | ${readmeParityCounts.commands} commands | Factory Droid |\n| Skills | ${readmeParityCounts.skills} skills | Factory Droid |\n`
  );
  fs.writeFileSync(agentsPath, `This repository is a **production-ready Factory Droid workflow plugin** providing ${summaryCounts.agents} specialized agents, ${summaryCounts.skills} skills, ${summaryCounts.commands} commands, and automated hook workflows for software development.\n\n\`\`\`\n${structureLines.join('\n')}\n\`\`\`\n`);
  ...
  fs.writeFileSync(zhDocsReadmePath, `**搞定！** 你现在可以使用 ${zhDocsReadmeCounts.agents} 个智能体、${zhDocsReadmeCounts.skills} 项技能和 ${zhDocsReadmeCounts.commands} 个命令了。\n| 组件 | 目录 | 范围 |\n|-----------|---------|-------|\n| 智能体 | \u2705 ${zhDocsTableCounts.agents} 个 | Factory Droid |\n| 命令 | \u2705 ${zhDocsTableCounts.commands} 个 | Factory Droid |\n| 技能 | \u2705 ${zhDocsTableCounts.skills} 项 | Factory Droid |\n\n| 功能特性 | 数量 | 格式 |\n|-----------|-------|---------|\n| 技能 | ${zhDocsUnrelatedSkillsCount} | .agents/skills/ |\n\n## Factory Droid 聚焦\n\n| 组件 | 目录 | 范围 |\n|-----------|---------|-------|\n| 智能体 | ${zhDocsParityCounts.agents} 个 | Factory Droid |\n| 命令 | ${zhDocsParityCounts.commands} 个 | Factory Droid |\n| 技能 | ${zhDocsParityCounts.skills} 项 | Factory Droid |\n`);
  fs.writeFileSync(zhAgentsPath, `这是一个面向 **Factory Droid** 的生产级工作流插件，提供 ${zhAgentsSummaryCounts.agents} 个专业代理、${zhAgentsSummaryCounts.skills} 项技能、${zhAgentsSummaryCounts.commands} 条命令以及自动化钩子工作流，用于软件开发。\n\n\`\`\`\n${zhAgentsStructureLines.join('\n')}\n\`\`\`\n`);
  fs.writeFileSync(
    ptBrReadmePath,
    `**Pronto!** Você agora tem acesso a ${ptBrReadmeCounts.agents} agentes, ${ptBrReadmeCounts.skills} skills e ${ptBrReadmeCounts.commands} comandos.\n\`\`\`\neverything-factory-droid/\n${ptBrLegacyLayout ? '|-- .claude-plugin/   # Plugin legado\n|   |-- plugin.json         # Metadados\n|   |-- marketplace.json    # Catálogo\n|\n' : '|-- .factory/         # Espelhos e configurações voltados ao Factory Droid\n|   |-- droids/             # Definições de droids convertidas\n|   |-- skills/             # Espelhos de skills para Factory Droid\n|   |-- commands/           # Espelhos de commands para Factory Droid\n|   |-- settings.json       # Configurações de projeto do Factory Droid\n|\n'}|-- agents/           # ${ptBrAgentsStructureCount} subagentes especializados para delegação\n\`\`\`\n`
  );
  fs.writeFileSync(
    trReadmePath,
    `**Bu kadar!** Artık ${trReadmeCounts.agents} agent, ${trReadmeCounts.skills} skill ve ${trReadmeCounts.commands} command'a erişiminiz var.\n\`\`\`\neverything-factory-droid/\n${trLegacyLayout ? '|-- .claude-plugin/   # Eski plugin düzeni\n|   |-- plugin.json         # Metadata\n|   |-- marketplace.json    # Katalog\n|\n' : '|-- .factory/         # Factory Droid aynaları ve ayarları\n|   |-- droids/             # Dönüştürülmüş droid tanımları\n|   |-- skills/             # Factory Droid skill aynaları\n|   |-- commands/           # Factory Droid command aynaları\n|   |-- settings.json       # Proje düzeyi Factory Droid ayarları\n|\n'}|-- agents/           # Delegation için ${trAgentsStructureCount} özel subagent\n\`\`\`\n`
  );

  return { readmePath, agentsPath, zhRootReadmePath, zhDocsReadmePath, zhAgentsPath, ptBrReadmePath, trReadmePath };
}
...
function runTests() {
  ...
  if (test('syncs tracked catalog docs in write mode without rewriting unrelated tables', () => {
    ...
    const {
      ...
      ptBrReadmeCounts: { agents: 18, skills: 19, commands: 20 },
      ptBrAgentsStructureCount: 21,
      ptBrLegacyLayout: true,
      trReadmeCounts: { agents: 22, skills: 23, commands: 24 },
      trAgentsStructureCount: 25,
      trLegacyLayout: true,
    });
    ...
    const ptBrReadme = fs.readFileSync(ptBrReadmePath, 'utf8');
    const trReadme = fs.readFileSync(trReadmePath, 'utf8');

    assert.ok(readme.includes('Access to 1 agents, 1 skills, and 1 legacy command shims'), 'Should sync README quick-start summary');
    assert.ok(readme.includes('| Agents | PASS: 1 agents | Factory Droid |'), 'Should sync README comparison table');
    assert.ok(readme.includes('| Skills | 16 | .agents/skills/ |'), 'Should not rewrite unrelated README tables');
    assert.ok(readme.includes('| Agents | 1 agents | Factory Droid |'), 'Should sync README parity table');
    ...
    assert.ok(zhDocsReadme.includes('| 智能体 | \u2705 1 个 | Factory Droid |'), 'Should sync docs/zh-CN/README comparison table');
    assert.ok(zhDocsReadme.includes('| 技能 | 16 | .agents/skills/ |'), 'Should not rewrite unrelated docs/zh-CN/README tables');
    assert.ok(zhDocsReadme.includes('| 智能体 | 1 个 | Factory Droid |'), 'Should sync docs/zh-CN/README parity table');
    ...
    assert.ok(ptBrReadme.includes('Você agora tem acesso a 1 agentes, 1 skills e 1 comandos'), 'Should sync docs/pt-BR/README quick-start summary');
    assert.ok(ptBrReadme.includes('|-- .factory/'), 'Should sync docs/pt-BR/README structure marker');
    assert.ok(!ptBrReadme.includes('.claude-plugin/'), 'Should remove legacy plugin layout from docs/pt-BR/README');
    assert.ok(ptBrReadme.includes('|-- agents/           # 1 subagentes especializados para delegação'), 'Should sync docs/pt-BR/README structure count');
    assert.ok(trReadme.includes("Artık 1 agent, 1 skill ve 1 command'a erişiminiz var"), 'Should sync docs/tr/README quick-start summary');
    assert.ok(trReadme.includes('|-- .factory/'), 'Should sync docs/tr/README structure marker');
    assert.ok(!trReadme.includes('.claude-plugin/'), 'Should remove legacy plugin layout from docs/tr/README');
    assert.ok(trReadme.includes('|-- agents/           # Delegation için 1 özel subagent'), 'Should sync docs/tr/README structure count');
  ...
  if (test('accepts shorthand and explicit valid model values', () => {
    const testDir = createTestDir();
    ...
    fs.writeFileSync(path.join(testDir, 'opus.md'),
      '---\nmodel: opus\ntools: Read, Write, Bash\n---\n# Opus Agent');
    fs.writeFileSync(path.join(testDir, 'haiku-explicit.md'),
      '---\nmodel: claude-haiku-4-5-20251001\ntools: Read\n---\n# Haiku Explicit Agent');
    fs.writeFileSync(path.join(testDir, 'sonnet-explicit.md'),
      '---\nmodel: claude-sonnet-4-6\ntools: Read, Write\n---\n# Sonnet Explicit Agent');
    fs.writeFileSync(path.join(testDir, 'opus-explicit.md'),
      '---\nmodel: claude-opus-4-6\ntools: Read, Write, Bash\n---\n# Opus Explicit Agent');

    const result = runValidatorWithDir('validate-agents', 'AGENTS_DIR', testDir);
    assert.strictEqual(result.code, 0, 'All valid models should pass');
    assert.ok(result.stdout.includes('6'), 'Should validate 6 agent files');
    cleanupTestDir(testDir);
  })) passed++; else failed++;
