...
function runTests() {
  ...
  const structuredDirPaths = [
    ...
    '.history/session.md',
    'memory/patterns.md',
    '.factory/commands/deploy.md',
    '.factory/plans/roadmap.md',
    '.factory/projects/myproject.md',
  ...
  (test('warns on ad-hoc name inside legacy .claude directory', () => {
    const { code, stderr } = runScript({ tool_input: { file_path: '.claude/TODO.md' } });
    assert.strictEqual(code, 0, 'should still exit 0');
    assert.ok(stderr.includes('WARNING'), 'expected warning for legacy .claude path');
  }) ? passed++ : failed++);
