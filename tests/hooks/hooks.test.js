...
function getCanonicalSessionsDir(homeDir) {
  return path.join(homeDir, '.factory', 'session-data');
}
...
async function runTests() {
  ...
  if (
    await asyncTest('runs from a non-project directory without detected project context', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-start-global-${Date.now()}`);
      const cwdDir = path.join(isoHome, 'scratch');
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.factory', 'skills', 'learned'), { recursive: true });
      ...
      try {
        ...
        const payload = JSON.parse(stdout);
        assert.ok(payload.hookSpecificOutput, 'session-start should still emit payload in global mode');
      } finally {
  ...
  if (
    await asyncTest('runs from filesystem root when no project name can be derived', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-start-root-${Date.now()}`);
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.factory', 'skills', 'learned'), { recursive: true });
      ...
      try {
        ...
        assert.strictEqual(code, 0);
        assert.ok(JSON.parse(stdout).hookSpecificOutput, 'session-start should emit payload even without a derived project name');
      } finally {
        fs.rmSync(isoHome, { recursive: true, force: true });
      }
    })
  ...
  if (
    await asyncTest('surfaces unresolved legacy migration conflicts to stderr', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-start-legacy-conflict-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      const legacySessionsDir = path.join(isoHome, '.claude', 'sessions');
      const sessionName = '2026-02-12-startwarn-session.tmp';
      const canonicalConflictPath = path.join(sessionsDir, sessionName);
      const legacyConflictPath = path.join(legacySessionsDir, sessionName);

      fs.mkdirSync(canonicalConflictPath, { recursive: true });
      fs.mkdirSync(legacySessionsDir, { recursive: true });
      fs.writeFileSync(legacyConflictPath, '# Legacy conflict');
      ...
      try {
        ...
        assert.ok(result.stderr.includes('[SessionStart] 1 legacy session migration issue(s) need manual resolution'));
        assert.ok(result.stderr.includes(legacyConflictPath));
      } finally {
        fs.rmSync(isoHome, { recursive: true, force: true });
      }
    })
  ...
  if (
    await asyncTest('exits 0 even with isolated empty HOME', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-iso-start-${Date.now()}`);
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.factory', 'skills', 'learned'), { recursive: true });
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
  ...
  if (
    await asyncTest('skips template session content', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-tpl-start-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
  ...
  if (
    await asyncTest('injects real session content', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-real-start-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.factory', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('reads canonical session-data content for duplicate-named sessions', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-canonical-start-${Date.now()}`);
  ...
  if (
    await asyncTest('imports legacy session files into canonical session-data without preserving legacy paths', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-session-canonical-start-${Date.now()}`);
      const canonicalDir = getCanonicalSessionsDir(isoHome);
      const legacyDataDir = path.join(isoHome, '.claude', 'session-data');
      const legacySessionsDir = path.join(isoHome, '.claude', 'sessions');
      const filename = '2026-02-11-migrate123-session.tmp';

      fs.mkdirSync(legacyDataDir, { recursive: true });
      fs.mkdirSync(legacySessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.factory', 'skills', 'learned'), { recursive: true });

      fs.writeFileSync(path.join(legacyDataDir, filename), '# Legacy session-data\n', 'utf8');
      fs.writeFileSync(path.join(legacySessionsDir, 'compaction-log.txt'), 'legacy compaction log\n', 'utf8');
      ...
      try {
        ...
        assert.ok(fs.existsSync(path.join(canonicalDir, filename)), 'legacy session file should be imported');
        assert.ok(fs.existsSync(path.join(canonicalDir, 'compaction-log.txt')), 'legacy session metadata should be imported');
        assert.ok(!fs.existsSync(path.join(legacyDataDir, filename)), 'legacy session-data file should be removed');
        assert.ok(!fs.existsSync(path.join(legacySessionsDir, 'compaction-log.txt')), 'legacy sessions file should be removed');
        assert.ok(!fs.existsSync(legacyDataDir), 'legacy session-data directory should be cleaned up');
        assert.ok(!fs.existsSync(legacySessionsDir), 'legacy sessions directory should be cleaned up');
      } finally {
        fs.rmSync(isoHome, { recursive: true, force: true });
      }
    })
  ...
  if (
    await asyncTest('strips ANSI escape codes from injected session content', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-ansi-start-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.factory', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('reports learned skills count', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-skills-start-${Date.now()}`);
      const learnedDir = path.join(isoHome, '.factory', 'skills', 'learned');
  ...
  if (
    await asyncTest('passes through input unchanged when integration is disabled', async () => {
      ...
      const result = await runScript(path.join(scriptsDir, 'insaits-security-wrapper.js'), stdinData, { EFD_ENABLE_INSAITS: '' });
      assert.strictEqual(result.code, 0, `Exit code should be 0, got ${result.code}`);
  ...
  if (
    await asyncTest('creates or updates session file', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-session-create-${Date.now()}`);
  ...
  if (
    await asyncTest('session-end reports unresolved legacy migration conflicts', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-end-legacy-conflict-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      const legacySessionsDir = path.join(isoHome, '.claude', 'sessions');
      const sessionName = '2026-02-12-endwarn-session.tmp';
      const canonicalConflictPath = path.join(sessionsDir, sessionName);
      const legacyConflictPath = path.join(legacySessionsDir, sessionName);
      ...
      try {
        ...
        assert.strictEqual(result.code, 0);
        assert.ok(result.stderr.includes('[SessionEnd] 1 legacy session migration issue(s) need manual resolution'));
        assert.ok(result.stderr.includes(legacyConflictPath));
      } finally {
        fs.rmSync(isoHome, { recursive: true, force: true });
      }
  ...
  if (
    await asyncTest('includes session ID in filename', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-session-id-${Date.now()}`);
  ...
  if (
    await asyncTest('writes project, branch, and worktree metadata into new session files', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-session-metadata-${Date.now()}`);
  ...
  if (
    await asyncTest('uses CLAUDE_PROJECT_DIR metadata when cwd is outside the project root', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-session-project-root-${Date.now()}`);
      const projectRoot = path.join(isoHome, 'workspace', 'outside-project');
      const scratchDir = path.join(isoHome, 'scratch');
      ...
      try {
        ...
        const expectedFile = path.join(
          getCanonicalSessionsDir(isoHome),
          `${today}-${utils.sanitizeSessionId('outside-project')}-session.tmp`
        );
        ...
        assert.ok(fs.existsSync(expectedFile), 'session-end should use the detected project basename');
        const content = fs.readFileSync(expectedFile, 'utf8');
        assert.ok(content.includes('**Project:** outside-project'));
        assert.ok(content.includes(`**Worktree:** ${projectRoot}`));
      } finally {
        fs.rmSync(isoHome, { recursive: true, force: true });
      }
    })
  ...
  if (
    await asyncTest('pre-compact reports unresolved legacy migration conflicts', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-precompact-legacy-conflict-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      const legacySessionsDir = path.join(isoHome, '.claude', 'sessions');
      const sessionName = '2026-02-12-prewarn-session.tmp';
      const canonicalConflictPath = path.join(sessionsDir, sessionName);
      const legacyConflictPath = path.join(legacySessionsDir, sessionName);
      ...
      try {
        ...
        assert.strictEqual(result.code, 0);
        assert.ok(result.stderr.includes('[PreCompact] 1 legacy session migration issue(s) need manual resolution'));
        assert.ok(result.stderr.includes(legacyConflictPath));
      } finally {
        fs.rmSync(isoHome, { recursive: true, force: true });
      }
  ...
  if (
    await asyncTest('pre-compact runs from a non-project directory without detected project context', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-compact-global-${Date.now()}`);
      const cwdDir = path.join(isoHome, 'scratch');
      fs.mkdirSync(cwdDir, { recursive: true });

      try {
        const proc = spawn('node', [path.join(scriptsDir, 'pre-compact.js')], {
          cwd: cwdDir,
          env: {
            ...process.env,
            HOME: isoHome,
        ...
        assert.strictEqual(code, 0);
        assert.ok(stderr.includes('[PreCompact]'));
      } finally {
        fs.rmSync(isoHome, { recursive: true, force: true });
      }
    })
  ...
  if (
    await asyncTest('pre-compact runs from filesystem root when no project name can be derived', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-compact-root-${Date.now()}`);
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
  ...
  if (
    await asyncTest('annotates active session file with compaction marker', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-compact-annotate-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('compaction log contains timestamp', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-compact-ts-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('increments counter on each call', async () => {
      ...
      // Check counter file
      const counterFile = path.join(os.tmpdir(), `droid-tool-count-${sessionId}`);
  ...
  if (
    await asyncTest('uses default session ID when no env var', async () => {
      ...
      const counterFile = path.join(os.tmpdir(), 'droid-tool-count-default');
  ...
  if (
    await asyncTest('finds formatter config in parent dirs without package.json', async () => {
      ...
      fs.mkdirSync(path.join(isolatedHome, '.factory'), { recursive: true });
  ...
  if (
    await asyncTest('respects FACTORY_DROID_PACKAGE_MANAGER for formatter fallback runner', async () => {
      const testDir = createTestDir();
      ...
      const stdinJson = JSON.stringify({ tool_input: { file_path: filePath } });
      const result = await runScript(path.join(scriptsDir, 'post-edit-format.js'), stdinJson, withPrependedPath(binDir, { FACTORY_DROID_PACKAGE_MANAGER: 'pnpm' }));
  ...
  if (
    await asyncTest('respects project package-manager config for formatter fallback runner', async () => {
      ...
      fs.writeFileSync(path.join(rootDir, '.factory', 'package-manager.json'), JSON.stringify({ packageManager: 'bun' }));
      fs.writeFileSync(path.join(rootDir, '.prettierrc'), '{}');
      fs.writeFileSync(filePath, 'export const value = 1;\n');
  ...
  if (
    await asyncTest('extracts tool names and file paths from transcript', async () => {
      ...
      assert.ok(result.stdout.includes('Created session file') || result.stdout.includes('Updated session file'), 'Should create/update session file');
      cleanupTestDir(testDir);
    })
  )
    passed++;
  ...
  if (
    await asyncTest('parses Factory Droid JSONL format (entry.message.content)', async () => {
      const testDir = createTestDir();
      const transcriptPath = path.join(testDir, 'transcript.jsonl');

      // Factory Droid v2.1.41+ JSONL format: user messages nested in entry.message
  ...
  if (
    await asyncTest('extracts tool_use from assistant message content blocks', async () => {
      ...
      // Factory Droid JSONL: tool uses nested in assistant message content array
      const lines = [
        '{"type":"user","content":"Edit the config"}',
        JSON.stringify({
          type: 'assistant',
  ...
  if (
    test('deprecated InsAIts hook is not shipped in the Factory Droid fork', () => {
      const hooksPath = path.join(__dirname, '..', '..', 'hooks', 'hooks.json');
      const hooks = JSON.parse(fs.readFileSync(hooksPath, 'utf8'));
      const insaitsHook = hooks.hooks.PreToolUse.find(entry => entry.description && entry.description.includes('InsAIts'));

      assert.strictEqual(insaitsHook, undefined, 'Factory Droid fork should not ship the deprecated InsAIts hook');
    })
  )
    passed++;
  else failed++;

  // legacy plugin manifest validation
  console.log('\nLegacy plugin manifest validation:');

  if (
    test('legacy .claude-plugin manifest is removed from the Factory Droid fork', () => {
      const pluginPath = path.join(__dirname, '..', '..', '.claude-plugin', 'plugin.json');
      assert.ok(!fs.existsSync(pluginPath), 'Legacy .claude-plugin manifest should not exist');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('observer-loop uses a configurable max-turn budget with safe default', () => {
      ...
      assert.ok(observerLoopSource.includes('EFD_OBSERVER_MAX_TURNS'), 'observer-loop should allow max-turn overrides');
      assert.ok(observerLoopSource.includes('max_turns="${EFD_OBSERVER_MAX_TURNS:-10}"'), 'observer-loop should default to 10 turns');
      assert.ok(!observerLoopSource.includes('--max-turns 3'), 'observer-loop should not hardcode a 3-turn limit');
      assert.ok(observerLoopSource.includes('EFD_SKIP_OBSERVE=1'), 'observer-loop should suppress observe.sh for automated sessions');
      assert.ok(observerLoopSource.includes('EFD_HOOK_PROFILE=minimal'), 'observer-loop should run automated analysis with the minimal hook profile');
    })
  )
    passed++;
  else failed++;
  ...
  if (SKIP_BASH) {
    ...
    await asyncTest('detect-project writes project metadata to the registry and project directory', async () => {
      ...
      try {
        ...
        spawnSync('git', ['remote', 'add', 'origin', 'https://token@example.com/example/efd-test.git'], { cwd: repoDir, stdio: 'ignore' });
        const legacyProjectId = crypto.createHash('sha256').update('https://token@example.com/example/efd-test.git').digest('hex').slice(0, 12);
        const legacyProjectDir = path.join(homeDir, '.factory', 'homunculus', 'projects', legacyProjectId);
        ...
        const registryPath = path.join(homeDir, '.factory', 'homunculus', 'projects.json');
        const expectedProjectId = path.basename(repoDir);
        ...
        assert.strictEqual(projectId, expectedProjectId, 'detect-project should use the project basename as the id');
        ...
        assert.ok(!fs.existsSync(legacyProjectDir), 'legacy hash-based project dir should be migrated');
        assert.ok(fs.existsSync(path.join(expectedProjectDir, 'observations.jsonl')), 'legacy project data should be preserved');
        ...
        assert.strictEqual(metadata.remote, 'https://example.com/example/efd-test.git', 'project.json should include the sanitized remote');
  ...
  if (SKIP_BASH) {
    console.log('  ⊘ detect-project migrates renamed basename storage (skipped on Windows)');
    passed++;
  } else if (
    await asyncTest('detect-project migrates renamed basename storage to the current project name', async () => {
      const testRoot = createTestDir();
      const homeDir = path.join(testRoot, 'home');
      const workspaceDir = path.join(testRoot, 'workspace');
      const newRepoDir = path.join(workspaceDir, 'new-repo');
      const oldRepoDir = path.join(workspaceDir, 'old-repo');
      const detectProjectPath = path.join(__dirname, '..', '..', 'skills', 'continuous-learning-v2', 'scripts', 'detect-project.sh');
      ...
      try {
        ...
        const legacyProjectDir = path.join(homeDir, '.factory', 'homunculus', 'projects', 'old-repo');
        fs.mkdirSync(legacyProjectDir, { recursive: true });
        fs.writeFileSync(path.join(legacyProjectDir, 'observations.jsonl'), '{"legacy":true}\n', 'utf8');
        ...
        fs.writeFileSync(
          path.join(homeDir, '.factory', 'homunculus', 'projects.json'),
          JSON.stringify({
            'old-repo': {
              id: 'old-repo',
              name: 'old-repo',
        ...
        const shellCommand = [
          ...
          `export CLAUDE_PROJECT_DIR="${toBashPath(newRepoDir)}"`,
        ...
        assert.strictEqual(code, 0, `detect-project rename migration should source cleanly, stderr: ${stderr}`);
        const [projectId, projectDir] = stdout.trim().split(/\r?\n/);
        const expectedProjectDir = path.join(homeDir, '.factory', 'homunculus', 'projects', 'new-repo');

        assert.strictEqual(projectId, 'new-repo');
        assert.strictEqual(normalizeComparablePath(projectDir), normalizeComparablePath(expectedProjectDir));
        assert.ok(fs.existsSync(path.join(expectedProjectDir, 'observations.jsonl')), 'renamed project data should be preserved');
        assert.ok(!fs.existsSync(legacyProjectDir), 'old basename project dir should be removed');
      } finally {
        cleanupTestDir(testRoot);
      }
    })
  ...
  if (SKIP_BASH) {
    ...
    await asyncTest('observe.sh falls back to legacy output fields when tool_response is null', async () => {
      ...
      try {
        ...
        const projectsDir = path.join(homeDir, '.factory', 'homunculus', 'projects');
  ...
  if (SKIP_BASH) { console.log("  ⊘ observe.sh skips minimal hook profile (skipped on Windows)"); passed++; } else if (
    await asyncTest('observe.sh skips minimal hook profile before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: { CLAUDE_CODE_ENTRYPOINT: 'cli', EFD_HOOK_PROFILE: 'minimal' }
      });
    })
  )
    passed++;
  ...
  if (SKIP_BASH) { console.log("  ⊘ observe.sh skips cooperative skip env (skipped on Windows)"); passed++; } else if (
    await asyncTest('observe.sh skips cooperative skip env before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: { CLAUDE_CODE_ENTRYPOINT: 'cli', EFD_SKIP_OBSERVE: '1' }
      });
    })
  )
    passed++;
    ...
    await asyncTest('observe.sh skips configured observer-session paths before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: {
          ...
          EFD_OBSERVE_SKIP_PATHS: ' observer-sessions , .factory-mem '
        },
  ...
  if (
    await asyncTest('updates Last Updated timestamp in existing session file', async () => {
      ...
      assert.ok(result.stdout.includes('Updated session file'), 'Should log update');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    await asyncTest('migrates renamed project session files before updating the current session', async () => {
      const testDir = createTestDir();
      const sessionsDir = getCanonicalSessionsDir(testDir);
      const homunculusDir = path.join(testDir, '.factory', 'homunculus');
      const workspaceDir = path.join(testDir, 'workspace');
      const oldProjectRoot = path.join(workspaceDir, 'old-name');
      const newProjectRoot = path.join(workspaceDir, 'new-name');
      ...
      const utils = require('../../scripts/lib/utils');
      const today = utils.getDateString();
      const oldShortId = utils.sanitizeSessionId('old-name');
      const newShortId = utils.sanitizeSessionId('new-name');
      const oldSessionFile = path.join(sessionsDir, `${today}-${oldShortId}-session.tmp`);
      const newSessionFile = path.join(sessionsDir, `${today}-${newShortId}-session.tmp`);

      fs.writeFileSync(
        oldSessionFile,
        `# Session: ${today}\n**Date:** ${today}\n**Started:** 09:00\n**Last Updated:** 09:00\n**Project:** old-name\n**Branch:** main\n**Worktree:** ${path.join(oldProjectRoot, 'src')}\n\n---\n\n## Current State\n\nLegacy context\n`,
        'utf8'
      );

      const legacyProjectDir = path.join(homunculusDir, 'projects', 'old-name');
      fs.mkdirSync(legacyProjectDir, { recursive: true });
      fs.writeFileSync(path.join(legacyProjectDir, 'project.json'), JSON.stringify({
        id: 'old-name',
        name: 'old-name',
      ...
      assert.strictEqual(code, 0, stderr);
      assert.ok(!fs.existsSync(oldSessionFile), 'old project-named session file should be removed');
      assert.ok(fs.existsSync(newSessionFile), 'renamed project session file should exist');

      const migrated = fs.readFileSync(newSessionFile, 'utf8');
      assert.ok(migrated.includes('**Project:** new-name'));
      assert.ok(migrated.includes(`**Worktree:** ${newProjectRoot}`));
      assert.ok(stdout.includes('Updated session file'), 'session-end should keep updating the migrated file');
  ...
  if (
    await asyncTest('only annotates *-session.tmp files, not other .tmp files', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-compact-glob-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('handles no active session files gracefully', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-compact-nosession-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('extracts tool_use from assistant message content blocks', async () => {
      const testDir = createTestDir();
      const transcriptPath = path.join(testDir, 'transcript.jsonl');
      // Factory Droid JSONL format: tool_use blocks inside assistant message content array
      const lines = [
  ...
  if (
    await asyncTest('exits 0 with empty sessions directory (no recent sessions)', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-start-empty-${Date.now()}`);
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.factory', 'skills', 'learned'), { recursive: true });
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
  ...
  if (
    await asyncTest('does not inject blank template session into context', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-start-blank-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.factory', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('annotates only the newest session file when multiple exist', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-compact-multi-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('does not inject empty session file content into context', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-start-empty-file-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.factory', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('summary omits Files Modified and Tools Used when none found', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-notools-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('reports available session aliases on startup', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-start-alias-${Date.now()}`);
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.factory', 'skills', 'learned'), { recursive: true });

      // Pre-populate the aliases file
      fs.writeFileSync(
        path.join(isoHome, '.factory', 'session-aliases.json'),
        JSON.stringify({
          version: '1.0',
          aliases: {
  ...
  if (
    await asyncTest('parallel compaction runs all append to log without loss', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-compact-par-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 when sessions path is a file (not a directory)', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-start-blocked-${Date.now()}`);
      fs.mkdirSync(path.join(isoHome, '.factory'), { recursive: true });
  ...
  if (
    await asyncTest('excludes session files older than 7 days', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-start-7day-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.factory', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('injects newest session when multiple recent sessions exist', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-start-multi-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.factory', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('handles stdin exceeding MAX_STDIN (1MB) gracefully', async () => {
      ...
      try {
        ...
        assert.ok(result.stdout.includes('Created session file') || result.stdout.includes('Updated session file'), `Should create/update session file via env var fallback, stdout: ${result.stdout}`);
      } finally {
        cleanupTestDir(testDir);
      }
    })
  ...
  if (
    await asyncTest('does not inject content when session file is unreadable', async () => {
      ...
      const isoHome = path.join(os.tmpdir(), `efd-start-unreadable-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 when session file is read-only (appendFile fails)', async () => {
      ...
      const isoHome = path.join(os.tmpdir(), `efd-compact-ro-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('logs warning when existing session file lacks Last Updated field', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-end-nots-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      ...
      assert.ok(
        result.stderr.includes('Failed to update')
          || result.stderr.includes('[SessionEnd]')
          || result.stdout.includes('[SessionEnd]'),
        'Should surface SessionEnd output when timestamp pattern not found in session file'
      );
  ...
  if (
    await asyncTest('extracts user messages from role-only format (no type field)', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-role-only-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('logs "Transcript not found" for nonexistent transcript_path', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-notfound-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('extracts tool name and file path from entry.name/entry.input (not tool_name/tool_input)', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-r70-entryname-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('shows selection prompt when no package manager preference found (default source)', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-r71-ss-default-${Date.now()}`);
      ...
      try {
        const result = await new Promise((resolve, reject) => {
          ...
          delete env.FACTORY_DROID_PACKAGE_MANAGER; // Remove any env-level PM override
          const proc = spawn('node', [path.join(scriptsDir, 'session-start.js')], {
            env,
  ...
  if (
    await asyncTest('extracts user messages from entries where only message.role is user (not type or role)', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-msgrole-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('skips user messages with numeric content (non-string non-array branch)', async () => {
      ...
      const isoHome = path.join(os.tmpdir(), `efd-r81-numcontent-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      const transcriptPath = path.join(isoHome, 'transcript.jsonl');
  ...
  if (
    await asyncTest('collects tool name from entry with tool_name but non-tool_use type', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-r82-toolname-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('preserves file when marker present but regex does not match corrupted template', async () => {
      const isoHome = path.join(os.tmpdir(), `efd-r82-tmpl-${Date.now()}`);
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
