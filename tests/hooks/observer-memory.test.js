...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'efd-observer-test-'));
}
...
test('observe.sh default throttle is 20 observations per signal', () => {
  const content = fs.readFileSync(observeShPath, 'utf8');
  assert.ok(content.includes('EFD_OBSERVER_SIGNAL_EVERY_N:-20'), 'Default signal frequency should be every 20 observations');
});
...
test('default cooldown is 60 seconds', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('EFD_OBSERVER_ANALYSIS_COOLDOWN:-60'), 'Default cooldown should be 60 seconds');
});
...
test('observer-loop.sh defines idle timeout fallback', () => {
  ...
  assert.ok(content.includes('EFD_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800'), 'Default idle timeout should be 30 minutes');
});
...
test('default max analysis lines is 500', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('EFD_OBSERVER_MAX_ANALYSIS_LINES:-500'), 'Default should sample last 500 lines');
});

test('analysis temp file is created and cleaned up', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('efd-observer-analysis'), 'Should create a temp analysis file');
...
test('observer-loop uses project-local temp directory for analysis artifacts', () => {
  ...
  assert.ok(content.includes('mktemp "${observer_tmp_dir}/efd-observer-analysis.'), 'Analysis temp file should use the project temp dir');
  assert.ok(content.includes('mktemp "${observer_tmp_dir}/efd-observer-prompt.'), 'Prompt temp file should use the project temp dir');
});
...
test('observe.sh creates counter file and increments on each call', () => {
  ...
  // We use EFD_SKIP_OBSERVE=0 and minimal JSON so observe.sh processes but
...
test('droid invocation includes --enabled-tools flag', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('--enabled-tools'), 'observer-loop.sh should include --enabled-tools flag in droid invocation');
});

test('enabled-tools includes Read permission', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  const match = content.match(/--enabled-tools\s+"([^"]+)"/);
  assert.ok(match, 'Should find --enabled-tools with quoted value');
  assert.ok(match[1].includes('Read'), `enabled-tools should include Read, got: ${match[1]}`);
});

test('enabled-tools includes Write permission', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  const match = content.match(/--enabled-tools\s+"([^"]+)"/);
  assert.ok(match, 'Should find --enabled-tools with quoted value');
  assert.ok(match[1].includes('Write'), `enabled-tools should include Write, got: ${match[1]}`);
});

test('droid invocation still includes EFD_SKIP_OBSERVE and EFD_HOOK_PROFILE guards', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  // Find the droid execution line(s)
  const lines = content.split('\n');
  const droidLine = lines.find(l => l.includes('droid exec --model claude-haiku-4-5-20251001'));
  assert.ok(droidLine, 'Should find droid exec --model claude-haiku-4-5-20251001 invocation line');
  // The env vars are on the same line as the droid command
  const droidLineIndex = lines.indexOf(droidLine);
  const fullCommand = lines.slice(Math.max(0, droidLineIndex - 1), droidLineIndex + 3).join(' ');
  assert.ok(fullCommand.includes('EFD_SKIP_OBSERVE=1'), 'droid invocation should include EFD_SKIP_OBSERVE=1 guard');
  assert.ok(fullCommand.includes('EFD_HOOK_PROFILE=minimal'), 'droid invocation should include EFD_HOOK_PROFILE=minimal guard');
});
