...
/**
 * Run a hook script with simulated Factory Droid input
...
function runHookWithInput(scriptPath, input = {}, env = {}, timeoutMs = 10000) {
  return new Promise((resolve, reject) => {
    ...
    // Send JSON input on stdin (simulating Factory Droid hook invocation)
...
async function runTests() {
  ...
  if (await asyncTest('PostToolUse PR logging hook is not shipped in the Factory Droid fork', async () => {
    const prHook = hooks.hooks.PostToolUse.find(h =>
      h.description && h.description.includes('PR URL')
    );

    assert.strictEqual(prHook, undefined, 'PR logging hook should be absent after the fork conversion');
  })) passed++; else failed++;
  ...
  if (await asyncTest('session-end extracts summary from mixed JSONL formats', async () => {
    ...
    try {
      ...
      assert.ok(result.stdout.includes('[SessionEnd]'), 'Should emit SessionEnd info on stdout');
      assert.ok(!result.stderr.includes('Updated session file') && !result.stderr.includes('Created session file'), 'Success messages should not be emitted on stderr');

      // Verify a session file was created
      const sessionsDir = path.join(testDir, '.factory', 'session-data');
  ...
  if (await asyncTest('session-end creates session file with nested user messages', async () => {
    ...
    // Factory Droid JSONL format uses nested message.content arrays
    const lines = [
