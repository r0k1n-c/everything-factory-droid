...
function runTests() {
  ...
  if (test('resolves relative config paths from the provided cwd', () => {
    ...
    const resolved = resolveInstallConfigPath('configs/efd-install.json', { cwd });
    assert.strictEqual(resolved, path.join(cwd, 'configs', 'efd-install.json'));
  })) passed++; else failed++;
  ...
  if (test('finds the default project install config in the provided cwd', () => {
    ...
    try {
      const configPath = path.join(cwd, 'efd-install.json');
  ...
  if (test('loads and normalizes a valid install config', () => {
    ...
    try {
      ...
      const config = loadInstallConfig('efd-install.json', { cwd });
      assert.strictEqual(config.path, configPath);
      assert.strictEqual(config.target, 'factory-droid');
  ...
  if (test('rejects invalid config schema values', () => {
    ...
    try {
      writeJson(path.join(cwd, 'efd-install.json'), {
        version: 2,
        target: 'ghost-target',
      });

      assert.throws(
        () => loadInstallConfig('efd-install.json', { cwd }),
