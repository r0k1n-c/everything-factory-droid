...
function runTests() {
  ...
  if (test('resolves a real project profile for the factory-droid target', () => {
    const projectRoot = '/workspace/app';
    const plan = resolveInstallPlan({ profileId: 'developer', target: 'factory-droid', projectRoot });
    ...
    assert.ok(plan.selectedModuleIds.includes('orchestration'),
      'Should keep orchestration for the factory-droid target');
    assert.deepStrictEqual(plan.skippedModuleIds, []);
    assert.strictEqual(plan.targetAdapterId, 'factory-droid-project');
    ...
    assert.ok(
      plan.operations.some(operation => (
        operation.sourceRelativePath === 'rules'
        && operation.strategy === 'preserve-relative-path'
      )),
      'Should plan managed repo paths directly into the project root'
    );
  })) passed++; else failed++;

  if (test('resolves core profiles for the factory-droid target', () => {
    const projectRoot = '/workspace/app';
    const plan = resolveInstallPlan({ profileId: 'core', target: 'factory-droid', projectRoot });

    assert.deepStrictEqual(
      plan.selectedModuleIds,
      ['rules-core', 'agents-core', 'commands-core', 'hooks-runtime', 'platform-configs', 'workflow-quality']
    );
    assert.deepStrictEqual(plan.skippedModuleIds, []);
    assert.strictEqual(plan.targetAdapterId, 'factory-droid-project');
    assert.strictEqual(plan.targetRoot, projectRoot);
  ...
  if (test('keeps factory-droid legacy compatibility selections complete', () => {
    const selection = resolveLegacyCompatibilitySelection({
      target: 'factory-droid',
      legacyLanguages: ['typescript'],
    });

    assert.deepStrictEqual(selection.moduleIds, [
      'rules-core',
      'agents-core',
      'commands-core',
      'hooks-runtime',
  ...
  if (test('validates projectRoot and homeDir option types before adapter planning', () => {
    assert.throws(
      () => resolveInstallPlan({ profileId: 'core', target: 'factory-droid', projectRoot: 42 }),
      /projectRoot must be a non-empty string when provided/
    );
    assert.throws(
      () => resolveInstallPlan({ profileId: 'core', target: 'factory-droid', homeDir: {} }),
      /homeDir must be a non-empty string when provided/
    );
  })) passed++; else failed++;

  if (test('selects a requested module when its dependency chain supports the target', () => {
    ...
    try {
      ...
      const plan = resolveInstallPlan({ repoRoot, profileId: 'core', target: 'factory-droid' });
      assert.deepStrictEqual(plan.selectedModuleIds, ['parent', 'child']);
      assert.deepStrictEqual(plan.skippedModuleIds, []);
    } finally {
  ...
  if (test('fails fast when install manifest module targets is not an array', () => {
    ...
    try {
      ...
      assert.throws(
        () => resolveInstallPlan({ repoRoot, profileId: 'core', target: 'factory-droid' }),
  ...
  if (test('keeps factory-droid modules selected when custom manifests include mixed source paths', () => {
    const repoRoot = createTestRepo();
    try {
      writeJson(path.join(repoRoot, 'manifests', 'install-modules.json'), {
        version: 1,
