...
function runTests() {
  ...
  if (test('parses manifest-mode CLI arguments', () => {
    ...
    assert.strictEqual(parsed.configPath, 'efd-install.json');
  ...
  if (test('merges config-backed component selections with CLI overrides', () => {
    const request = normalizeInstallRequest({
      ...
      excludeComponentIds: ['capability:media'],
      languages: [],
      configPath: '/workspace/app/efd-install.json',
      config: {
        path: '/workspace/app/efd-install.json',
        target: 'factory-droid',
        profileId: 'developer',
    ...
    assert.strictEqual(request.configPath, '/workspace/app/efd-install.json');
  })) passed++; else failed++;
