...
function runTests() {
  ...
  if (test('creates a valid install-state payload', () => {
    const state = createInstallState({
      ...
      installStatePath: '/repo/.cursor/efd-install-state.json',
      request: {
        profile: 'developer',
    ...
    assert.strictEqual(state.schemaVersion, 'efd.install.v1');
  ...
  if (test('writes and reads install-state from disk', () => {
    ...
    const statePath = path.join(testDir, 'efd-install-state.json');

    try {
      const state = createInstallState({
        adapter: { id: 'factory-droid-project' },
        targetRoot: path.join(testDir, '.factory'),
      ...
      assert.strictEqual(loaded.target.id, 'factory-droid-project');
  ...
  if (test('rejects invalid install-state payloads on read', () => {
    ...
    try {
      fs.writeFileSync(statePath, JSON.stringify({ schemaVersion: 'efd.install.v1' }, null, 2));
      assert.throws(
