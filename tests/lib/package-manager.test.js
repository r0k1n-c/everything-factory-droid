...
function runTests() {
  ...
  if (test('respects environment variable', () => {
    const originalEnv = process.env.FACTORY_DROID_PACKAGE_MANAGER;
    try {
      process.env.FACTORY_DROID_PACKAGE_MANAGER = 'yarn';
      const result = pm.getPackageManager();
      assert.strictEqual(result.name, 'yarn');
      assert.strictEqual(result.source, 'environment');
    } finally {
      if (originalEnv !== undefined) {
        process.env.FACTORY_DROID_PACKAGE_MANAGER = originalEnv;
      } else {
        delete process.env.FACTORY_DROID_PACKAGE_MANAGER;
      }
    }
  })) passed++;
  else failed++;
  ...
  if (test('returns correct install command', () => {
    const originalEnv = process.env.FACTORY_DROID_PACKAGE_MANAGER;
    try {
      process.env.FACTORY_DROID_PACKAGE_MANAGER = 'pnpm';
  ...
  if (test('returns correct test command', () => {
    const originalEnv = process.env.FACTORY_DROID_PACKAGE_MANAGER;
    try {
      process.env.FACTORY_DROID_PACKAGE_MANAGER = 'npm';
  ...
  if (test('returns informative prompt', () => {
    ...
    assert.ok(prompt.includes('FACTORY_DROID_PACKAGE_MANAGER'), 'Should mention env var');
  ...
  if (test('sets project package manager', () => {
    ...
    try {
      ...
      const configPath = path.join(testDir, '.factory', 'package-manager.json');
  ...
  if (test('falls through on corrupted project config JSON', () => {
    ...
    const claudeDir = path.join(testDir, '.factory');
  ...
  if (test('detects from valid project-config (.factory/package-manager.json)', () => {
  ...
  if (test('successfully saves preferred package manager', () => {
    // This writes to ~/.factory/package-manager.json — read original to restore
  ...
  if (test('pnpm custom script: pnpm (no run keyword)', () => {
    ...
    try {
      ...
      if (originalEnv !== undefined) process.env.FACTORY_DROID_PACKAGE_MANAGER = originalEnv;
      else delete process.env.FACTORY_DROID_PACKAGE_MANAGER;
    }
  })) passed++;
  else failed++;
  ...
  if (test('bun custom script: bun run <script>', () => {
    const originalEnv = process.env.FACTORY_DROID_PACKAGE_MANAGER;
    try {
      process.env.FACTORY_DROID_PACKAGE_MANAGER = 'bun';
  ...
  if (test('ignores unknown env var package manager', () => {
    const originalEnv = process.env.FACTORY_DROID_PACKAGE_MANAGER;
    try {
      process.env.FACTORY_DROID_PACKAGE_MANAGER = 'totally-fake-pm';
      const result = pm.getPackageManager();
  ...
  if (test('setProjectPackageManager creates .factory directory if missing', () => {
    const testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pm-mkdir-'));
    try {
      const claudeDir = path.join(testDir, '.factory');
      assert.ok(!fs.existsSync(claudeDir), '.factory should not pre-exist');
      pm.setProjectPackageManager('npm', testDir);
      assert.ok(fs.existsSync(claudeDir), '.factory should be created');
  ...
  if (test('getPackageManager falls through corrupted global config to npm default', () => {
    ...
    const origPM = process.env.FACTORY_DROID_PACKAGE_MANAGER;

    try {
      // Create corrupted global config file
      const claudeDir = path.join(tmpDir, '.factory');
      fs.mkdirSync(claudeDir, { recursive: true });
      ...
      if (origPM !== undefined) process.env.FACTORY_DROID_PACKAGE_MANAGER = origPM;
  ...
  if (test('setPreferredPackageManager throws wrapped error when save fails', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `efd-pm-r71-${Date.now()}`);
    const claudeDir = path.join(isoHome, '.factory');
    fs.mkdirSync(claudeDir, { recursive: true });
    ...
    try {
      ...
      delete require.cache[require.resolve('../../scripts/lib/utils')];
      const freshPm = require('../../scripts/lib/package-manager');
      // Make .factory directory read-only — can't create new files (package-manager.json)
  ...
  if (test('setProjectPackageManager throws wrapped error when write fails', () => {
    ...
    const isoProject = path.join(os.tmpdir(), `efd-pm-proj-r72-${Date.now()}`);
    const claudeDir = path.join(isoProject, '.factory');
    fs.mkdirSync(claudeDir, { recursive: true });
    // Make .factory directory read-only — can't create new files
    fs.chmodSync(claudeDir, 0o555);
    try {
      assert.throws(() => {
  ...
  // ── Round 94: getPackageManager with empty string FACTORY_DROID_PACKAGE_MANAGER ──
  console.log('\nRound 94: getPackageManager (empty string FACTORY_DROID_PACKAGE_MANAGER env var):');

  if (test('getPackageManager skips empty string FACTORY_DROID_PACKAGE_MANAGER (falsy short-circuit)', () => {
    ...
    const originalEnv = process.env.FACTORY_DROID_PACKAGE_MANAGER;
    try {
      process.env.FACTORY_DROID_PACKAGE_MANAGER = '';
      const result = pm.getPackageManager();
