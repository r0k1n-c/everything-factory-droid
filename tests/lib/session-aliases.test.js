 ...
 * the real ~/.factory/session-aliases.json.
 *
...
const tmpHome = path.join(os.tmpdir(), `efd-alias-test-${Date.now()}`);
...
function runTests() {
  ...
  if (test('saveAliases creates backup before write and removes on success', () => {
    ...
    const aliasesPath = path.join(tmpHome, '.factory', 'session-aliases.json');
  ...
  if (test('updateAliasTitle returns failure when saveAliases fails (read-only dir)', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `efd-alias-r70-${Date.now()}`);
    const isoClaudeDir = path.join(isoHome, '.factory');
    ...
    try {
      ...
      // Make .factory dir read-only so saveAliases fails when creating .bak
  ...
  if (test('deleteAlias returns failure when saveAliases fails (read-only dir)', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `efd-alias-r72-${Date.now()}`);
    const isoClaudeDir = path.join(isoHome, '.factory');
    fs.mkdirSync(isoClaudeDir, { recursive: true });
    const savedHome = process.env.HOME;
    const savedProfile = process.env.USERPROFILE;
    try {
      ...
      // Make .factory directory read-only — save will fail (can't create temp file)
      fs.chmodSync(isoClaudeDir, 0o555);
  ...
  if (test('cleanupAliases returns failure when saveAliases fails after removing aliases', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `efd-alias-r73-cleanup-${Date.now()}`);
    const isoClaudeDir = path.join(isoHome, '.factory');
    fs.mkdirSync(isoClaudeDir, { recursive: true });
    const savedHome = process.env.HOME;
    const savedProfile = process.env.USERPROFILE;
    try {
      ...
      // Make .factory dir read-only so save will fail
      fs.chmodSync(isoClaudeDir, 0o555);
  ...
  if (test('setAlias returns failure when saveAliases fails', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `efd-alias-r73-set-${Date.now()}`);
    const isoClaudeDir = path.join(isoHome, '.factory');
    fs.mkdirSync(isoClaudeDir, { recursive: true });
    const savedHome = process.env.HOME;
    const savedProfile = process.env.USERPROFILE;
    try {
      ...
      // Make .factory dir read-only BEFORE any setAlias call
      fs.chmodSync(isoClaudeDir, 0o555);
  ...
  if (test('saveAliases triggers inner restoreErr catch when both save and restore fail', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `efd-r90-restore-fail-${Date.now()}`);
    ...
    // Make .factory directory read-only (0o555):
