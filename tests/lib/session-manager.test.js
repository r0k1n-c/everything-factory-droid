...
function createTempSessionDir() {
  const dir = path.join(os.tmpdir(), `efd-test-sessions-${Date.now()}`);
...
\`\`\``;
    ...
    assert.strictEqual(meta.project, 'everything-factory-droid');
    assert.strictEqual(meta.branch, 'feature/session-metadata');
    assert.strictEqual(meta.worktree, '/tmp/efd-worktree');
...
`;
  ...
  const tmpHome = path.join(os.tmpdir(), `efd-session-mgr-test-${Date.now()}`);
  const tmpCanonicalSessionsDir = path.join(tmpHome, '.factory', 'session-data');
  ...
  for (let i = 0; i < testSessions.length; i++) {
    const filePath = path.join(tmpCanonicalSessionsDir, testSessions[i].name);
  ...
  if (test('getAllSessions reads old-format session filenames from canonical session-data', () => {
    const result = sessionManager.getAllSessions({ date: '2026-02-10', limit: 100 });
    assert.strictEqual(result.total, 1);
    assert.strictEqual(result.sessions[0].filename, '2026-02-10-session.tmp');
    assert.ok(result.sessions[0].sessionPath.includes('session-data'));
  })) passed++; else failed++;

  if (test('getAllSessions migrates legacy-only sessions into canonical session-data', () => {
    const isoHome = path.join(os.tmpdir(), `efd-legacy-import-${Date.now()}`);
    const legacyOnlyName = '2026-02-12-legacy999-session.tmp';
    const legacySessionsDir = path.join(isoHome, '.claude', 'sessions');
    const legacyOnlyPath = path.join(legacySessionsDir, legacyOnlyName);
    const canonicalOnlyPath = path.join(isoHome, '.factory', 'session-data', legacyOnlyName);
    ...
    try {
      ...
      const result = freshSM.getAllSessions({ search: 'legacy999', limit: 100 });
      assert.strictEqual(result.total, 1);
      assert.ok(fs.existsSync(canonicalOnlyPath), 'legacy session should be imported to canonical dir');
      assert.ok(!fs.existsSync(legacyOnlyPath), 'legacy source should be removed after import');
      assert.ok(result.sessions[0].sessionPath.includes('session-data'));
    } finally {
  ...
  if (test('getAllSessions warns when legacy migration conflicts leave sessions stranded', () => {
    const isoHome = path.join(os.tmpdir(), `efd-legacy-conflict-${Date.now()}`);
    const sessionName = '2026-02-12-conflict999-session.tmp';
    const legacySessionsDir = path.join(isoHome, '.claude', 'sessions');
    const legacySessionPath = path.join(legacySessionsDir, sessionName);
    const canonicalSessionPath = path.join(isoHome, '.factory', 'session-data', sessionName);
    const savedHome = process.env.HOME;
    const savedProfile = process.env.USERPROFILE;
    ...
    try {
      fs.mkdirSync(legacySessionsDir, { recursive: true });
      fs.mkdirSync(canonicalSessionPath, { recursive: true });
      fs.writeFileSync(legacySessionPath, '# Legacy conflict');
      process.env.HOME = isoHome;
      process.env.USERPROFILE = isoHome;
      ...
      const result = freshSM.getAllSessions({ search: 'conflict999', limit: 100 });
      assert.strictEqual(result.total, 0);
      assert.ok(captured.some(line => line.includes('[SessionManager] 1 legacy session migration issue(s) need manual resolution')));
      assert.ok(captured.some(line => line.includes(legacySessionPath)));
    } finally {
  ...
  if (test('getSessionById resolves old-format session filenames from canonical session-data', () => {
    const result = sessionManager.getSessionById('2026-02-10');
    assert.ok(result, 'Should resolve old-format session from canonical storage');
    assert.strictEqual(result.filename, '2026-02-10-session.tmp');
    assert.ok(result.sessionPath.includes('session-data'));
  })) passed++; else failed++;

  if (test('getSessionById imports legacy sessions before resolving matches', () => {
    const isoHome = path.join(os.tmpdir(), `efd-legacy-resolve-${Date.now()}`);
    const legacyName = '2026-02-12-legacyres-session.tmp';
    const legacySessionsDir = path.join(isoHome, '.claude', 'sessions');
    const legacyPath = path.join(legacySessionsDir, legacyName);
    const canonicalPath = path.join(isoHome, '.factory', 'session-data', legacyName);
    const savedHome = process.env.HOME;
    const savedProfile = process.env.USERPROFILE;

    try {
      fs.mkdirSync(legacySessionsDir, { recursive: true });
      fs.writeFileSync(legacyPath, '# Legacy resolution');
      process.env.HOME = isoHome;
      process.env.USERPROFILE = isoHome;
      delete require.cache[require.resolve('../../scripts/lib/session-manager')];
      delete require.cache[require.resolve('../../scripts/lib/utils')];
      ...
      const result = freshSM.getSessionById('legacyres');
      assert.ok(result, 'Should resolve imported legacy session');
      assert.ok(fs.existsSync(canonicalPath), 'legacy session should be imported to canonical dir');
      assert.ok(!fs.existsSync(legacyPath), 'legacy source should be removed after import');
      assert.ok(result.sessionPath.includes('session-data'));
    } finally {
      process.env.HOME = savedHome;
      process.env.USERPROFILE = savedProfile;
  ...
  if (test('getSessionPath returns absolute path for session filename', () => {
    ...
    assert.ok(result.includes('.factory'), 'Path should include .factory directory');
  ...
  const r33Home = path.join(os.tmpdir(), `efd-r33-birthtime-${Date.now()}`);
  const r33SessionsDir = path.join(r33Home, '.factory', 'session-data');
  ...
  const r33CanonicalPath = path.join(r33Home, '.factory', 'session-data', r33Filename);
  ...
  if (test('getAllSessions returns createdTime from birthtime when available', () => {
    ...
    const stats = fs.statSync(r33CanonicalPath);
  ...
  if (test('getAllSessions returns empty when sessions dir missing', () => {
    ...
    const origUserProfile = process.env.USERPROFILE;
    try {
      // Point HOME to a dir with no .factory/sessions/
  ...
  if (test('getAllSessions hasContent is true for non-empty and false for empty files', () => {
    const isoHome = path.join(os.tmpdir(), `efd-hascontent-${Date.now()}`);
    const isoSessions = path.join(isoHome, '.factory', 'session-data');
  ...
  if (test('getAllSessions skips broken symlink .tmp files gracefully', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `efd-r83-toctou-${Date.now()}`);
    const sessionsDir = path.join(isoHome, '.factory', 'session-data');
  ...
  if (test('getSessionById returns null when matching session is a broken symlink', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `efd-r84-getbyid-toctou-${Date.now()}`);
    const sessionsDir = path.join(isoHome, '.factory', 'session-data');
    fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (test('getAllSessions skips subdirectories inside sessions dir', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `efd-r89-subdir-skip-${Date.now()}`);
    const sessionsDir = path.join(isoHome, '.factory', 'session-data');
    fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  const tmpHome2 = path.join(os.tmpdir(), `efd-session-mgr-test-2-${Date.now()}`);
  const tmpSessionsDir2 = path.join(tmpHome2, '.factory', 'session-data');
...
`;
  ...
  if (test('getAllSessions coerces array/object limit via Number() with NaN fallback to 50', () => {
    const isoHome = path.join(os.tmpdir(), `efd-r106-limit-coerce-${Date.now()}`);
    const isoSessionsDir = path.join(isoHome, '.factory', 'session-data');
  ...
  if (test('getAllSessions ignores .tmp files with non-matching filenames', () => {
    const isoHome = path.join(os.tmpdir(), `efd-r109-nonsession-${Date.now()}`);
    const isoSessionsDir = path.join(isoHome, '.factory', 'session-data');
    fs.mkdirSync(isoSessionsDir, { recursive: true });
  ...
  if (test('getSessionById matches old format YYYY-MM-DD-session.tmp via noIdMatch path', () => {
    ...
    try {
      ...
      const sessionsDir = path.join(factoryDir, 'session-data');
      fs.mkdirSync(sessionsDir, { recursive: true });
      process.env.HOME = tmpDir;
  ...
  if (test('getAllSessions date filter uses strict equality so wrong format returns empty', () => {
    ...
    fs.mkdirSync(path.join(homeDir, '.factory', 'session-data'), { recursive: true });
