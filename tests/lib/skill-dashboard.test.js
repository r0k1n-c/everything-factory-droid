...
function runTests() {
  ...
  const learnedRoot = path.join(homeDir, '.factory', 'skills', 'learned');
  const importedRoot = path.join(homeDir, '.factory', 'skills', 'imported');
  const runsFile = path.join(homeDir, '.factory', 'state', 'skill-runs.jsonl');
  ...
  try {
    ...
    if (test('renderDashboard produces all four panels', () => {
      ...
      assert.ok(result.text.includes('EFD Skill Health Dashboard'));
      assert.ok(result.text.includes('Success Rate'));
      assert.ok(result.text.includes('Failure Patterns'));
      assert.ok(result.text.includes('Pending Amendments'));
      assert.ok(result.text.includes('Version History'));
    ...
    if (test('dashboard renders gracefully with no execution records', () => {
      const emptyRunsFile = path.join(homeDir, '.factory', 'state', 'empty-runs.jsonl');
      ...
      const result = dashboard.renderDashboard({
        ...
        learnedRoot: path.join(homeDir, '.factory', 'skills', 'empty-learned'),
        importedRoot: path.join(homeDir, '.factory', 'skills', 'empty-imported'),
        homeDir,
