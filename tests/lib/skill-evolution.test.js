...
function runTests() {
  ...
  try {
    ...
    if (test('ignores malformed JSONL rows when reading execution records', () => {
      const malformedRunsFile = path.join(homeDir, '.factory', 'state', 'malformed-skill-runs.jsonl');
      fs.writeFileSync(
        malformedRunsFile,
        `${JSON.stringify({
          skill_id: 'alpha',
    ...
    if (test('treats an unsnapshotted SKILL.md as v1 and orders last_run by actual time', () => {
      ...
      const offsetRunsFile = path.join(homeDir, '.factory', 'state', 'offset-skill-runs.jsonl');
