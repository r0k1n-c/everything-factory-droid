...
test('observation layer writes and reads structured skill outcomes', () => {
  const projectRoot = makeProjectRoot('efd-skill-observe-');
  ...
  try {
    ...
    assert.strictEqual(records[0].schemaVersion, 'efd.skill-observation.v1');
    ...
    assert.strictEqual(getSkillObservationsPath({ projectRoot }), path.join(projectRoot, '.factory', 'efd', 'skills', 'observations.jsonl'));
  } finally {
    cleanup(projectRoot);
  }
});
...
test('health inspector traces recurring failures for a skill across runs', () => {
  const projectRoot = makeProjectRoot('efd-skill-health-');
...
test('amendify proposes SKILL.md patch content from failure evidence', () => {
  ...
  assert.strictEqual(proposal.schemaVersion, 'efd.skill-amendment-proposal.v1');
...
test('evaluation scaffold compares amended and baseline performance', () => {
  ...
  assert.strictEqual(evaluation.schemaVersion, 'efd.skill-evaluation.v1');
