/**
 * Tests for the SQLite-backed EFD state store and CLI commands.
 */
...
const EFD_SCRIPT = path.join(__dirname, '..', '..', 'scripts', 'efd.js');
...
async function seedStore(dbPath) {
      ...
      workers: [
        {
          ...
          worktree: '/tmp/efd-repo/.worktrees/worker-1',
        },
        {
          id: 'worker-2',
          label: 'Worker 2',
          state: 'idle',
          branch: 'feat/state-store',
          worktree: '/tmp/efd-repo/.worktrees/worker-2',
        },
      ],
      aggregates: {
        workerCount: 2,
  ...
  store.insertSkillRun({
    ...
    skillVersion: '1.0.0',
    sessionId: 'session-recorded',
    taskDescription: 'Outline EFD 2.0 work',
    outcome: 'unknown',
    failureReason: null,
    tokensUsed: 300,
    durationMs: 500,
  ...
  store.insertDecision({
    ...
    rationale: 'Need queryable local state for EFD control plane',
  ...
  store.upsertInstallState({
    ...
    targetRoot: '/tmp/home/.factory',
    profile: 'developer',
    modules: ['rules-core', 'orchestration'],
    operations: [
      {
        kind: 'copy-file',
        destinationPath: '/tmp/home/.factory/agents/planner.md',
      },
    ],
...
async function runTests() {
  ...
  if (await test('creates the default state.db path and applies migrations idempotently', async () => {
    const homeDir = createTempDir('efd-state-home-');

    try {
      const expectedPath = path.join(homeDir, '.factory', 'efd', 'state.db');
  ...
  if (await test('preserves SQLite special database names like :memory:', async () => {
    const tempDir = createTempDir('efd-state-memory-');
  ...
  if (await test('stores sessions and returns detailed session views with workers, skill runs, and decisions', async () => {
    const testDir = createTempDir('efd-state-db-');
  ...
  if (await test('status CLI supports human-readable and --json output', async () => {
    const testDir = createTempDir('efd-state-cli-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('efd CLI delegates the new status and sessions subcommands', async () => {
    const testDir = createTempDir('efd-state-cli-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      ...
      const statusResult = runNode(EFD_SCRIPT, ['status', '--db', dbPath, '--json']);
      ...
      const sessionsResult = runNode(EFD_SCRIPT, ['sessions', 'session-active', '--db', dbPath, '--json']);
