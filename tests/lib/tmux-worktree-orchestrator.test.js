...
test('buildOrchestrationPlan creates worktrees, branches, and tmux commands', () => {
  const repoRoot = path.join('/tmp', 'efd');
  ...
  assert.ok(
    plan.workerPlans[0].worktreePath.endsWith(path.join('efd-skill-audit-docs-a')),
...
test('buildOrchestrationPlan normalizes global and worker seed paths', () => {
  const plan = buildOrchestrationPlan({
    ...
    seedPaths: ['scripts/orchestrate-worktrees.js', './.factory/plan/workflow-e2e-test.json'],
    workers: [
      {
        name: 'Docs',
        task: 'Update docs',
...
test('normalizeSeedPaths rejects paths outside the repo root', () => {
  assert.throws(
    () => normalizeSeedPaths(['../outside.txt'], '/tmp/efd'),
    /inside repoRoot/
  );
});

test('materializePlan keeps worker instructions inside the worktree boundary', () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'efd-orchestrator-test-'));

  try {
    const plan = buildOrchestrationPlan({
      repoRoot: tempRoot,
      coordinationRoot: path.join(tempRoot, '.factory', 'orchestration'),
...
test('overlaySeedPaths copies local overlays into the worker worktree', () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'efd-orchestrator-overlay-'));
  const repoRoot = path.join(tempRoot, 'repo');
  const worktreePath = path.join(tempRoot, 'worktree');

  try {
    fs.mkdirSync(path.join(repoRoot, 'scripts'), { recursive: true });
    fs.mkdirSync(path.join(repoRoot, '.factory', 'plan'), { recursive: true });
    ...
    fs.writeFileSync(
      path.join(repoRoot, '.factory', 'plan', 'workflow-e2e-test.json'),
      '{"seeded":true}\n',
      'utf8'
    );
    fs.writeFileSync(
    ...
    assert.strictEqual(
      fs.readFileSync(path.join(worktreePath, '.factory', 'plan', 'workflow-e2e-test.json'), 'utf8'),
      '{"seeded":true}\n'
    );
  } finally {
    fs.rmSync(tempRoot, { recursive: true, force: true });
...
test('executePlan rolls back partial setup when orchestration fails mid-run', () => {
  const plan = {
    ...
    coordinationDir: '/tmp/efd/.orchestration/rollback-test',
    replaceExisting: false,
    workerPlans: [
      {
        workerName: 'Docs',
        workerSlug: 'docs',
        worktreePath: '/tmp/efd-rollback-docs',
        seedPaths: ['commands/orchestrate.md'],
        gitArgs: ['worktree', 'add', '-b', 'orchestrator-rollback-test-docs', '/tmp/efd-rollback-docs', 'HEAD'],
...
test('executePlan does not mark pre-existing resources for rollback when worktree creation fails', () => {
  const plan = {
    ...
    coordinationDir: '/tmp/efd/.orchestration/rollback-existing',
    replaceExisting: false,
    workerPlans: [
      {
        workerName: 'Docs',
        workerSlug: 'docs',
        worktreePath: '/tmp/efd-existing-docs',
        seedPaths: [],
        gitArgs: ['worktree', 'add', '-b', 'orchestrator-rollback-existing-docs', '/tmp/efd-existing-docs', 'HEAD'],
        launchCommand: 'echo run',
