...
function runTests() {
  ...
  if (test('getFactoryDir returns path under home', () => {
    const factoryDir = utils.getFactoryDir();
    const homeDir = utils.getHomeDir();
    assert.ok(factoryDir.startsWith(homeDir), 'Factory dir should be under home');
    assert.ok(factoryDir.includes('.factory'), 'Should contain .factory');
  })) passed++; else failed++;

  if (test('getClaudeDir remains an alias for getFactoryDir', () => {
    assert.strictEqual(utils.getClaudeDir(), utils.getFactoryDir());
  })) passed++; else failed++;

  if (test('getSessionsDir returns path under Factory dir', () => {
    const sessionsDir = utils.getSessionsDir();
    const factoryDir = utils.getFactoryDir();
    assert.ok(sessionsDir.startsWith(factoryDir), 'Sessions should be under Factory dir');
    assert.ok(sessionsDir.endsWith(path.join('.factory', 'session-data')) || sessionsDir.endsWith('/.factory/session-data'), 'Should use canonical session-data directory');
  })) passed++; else failed++;

  if (test('getSessionSearchDirs only includes canonical session-data path', () => {
    const searchDirs = utils.getSessionSearchDirs();
    assert.deepStrictEqual(searchDirs, [utils.getSessionsDir()]);
  })) passed++; else failed++;

  if (test('migrateLegacySessions imports legacy session files into canonical session-data and removes empty legacy dirs', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-session-migrate-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      const canonicalDir = utils.getSessionsDir();
      const legacyDataDir = path.join(tempHome, '.claude', 'session-data');
      const legacySessionsDir = path.join(tempHome, '.claude', 'sessions');

      fs.mkdirSync(path.join(legacyDataDir, 'nested'), { recursive: true });
      fs.mkdirSync(legacySessionsDir, { recursive: true });
      fs.writeFileSync(path.join(legacyDataDir, '2026-02-01-legacy-session.tmp'), '# legacy');
      fs.writeFileSync(path.join(legacyDataDir, 'nested', 'note.txt'), 'nested');
      fs.writeFileSync(path.join(legacySessionsDir, 'compaction-log.txt'), 'compaction');

      const result = utils.migrateLegacySessions();

      assert.strictEqual(result.migrated, 3);
      assert.ok(fs.existsSync(path.join(canonicalDir, '2026-02-01-legacy-session.tmp')));
      assert.ok(fs.existsSync(path.join(canonicalDir, 'nested', 'note.txt')));
      assert.ok(fs.existsSync(path.join(canonicalDir, 'compaction-log.txt')));
      assert.ok(fs.existsSync(path.join(canonicalDir, '.claude-migration-complete')));
      assert.ok(!fs.existsSync(legacyDataDir), 'legacy session-data dir should be removed when empty');
      assert.ok(!fs.existsSync(legacySessionsDir), 'legacy sessions dir should be removed when empty');
    } finally {
  ...
  if (test('migrateLegacySessions keeps canonical duplicates and removes legacy copies', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-session-dupe-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      fs.writeFileSync(path.join(canonicalDir, 'duplicate.tmp'), 'canonical');
      fs.writeFileSync(path.join(legacySessionsDir, 'duplicate.tmp'), 'legacy');

      const result = utils.migrateLegacySessions();

      assert.strictEqual(result.removedDuplicates, 1);
      assert.strictEqual(fs.readFileSync(path.join(canonicalDir, 'duplicate.tmp'), 'utf8'), 'canonical');
      assert.ok(!fs.existsSync(path.join(legacySessionsDir, 'duplicate.tmp')));
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
      if (originalUserProfile === undefined) delete process.env.USERPROFILE;
  ...
  if (test('migrateLegacySessions only imports legacy paths once', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-session-once-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      const canonicalDir = utils.getSessionsDir();
      const legacySessionsDir = path.join(tempHome, '.claude', 'sessions');
      const firstLegacyFile = path.join(legacySessionsDir, '2026-02-01-first-session.tmp');
      const secondLegacyFile = path.join(legacySessionsDir, '2026-02-01-second-session.tmp');

      fs.mkdirSync(legacySessionsDir, { recursive: true });
      fs.writeFileSync(firstLegacyFile, 'first import');

      const firstResult = utils.migrateLegacySessions();
      assert.strictEqual(firstResult.migrated, 1);
      assert.ok(fs.existsSync(path.join(canonicalDir, '2026-02-01-first-session.tmp')));

      fs.mkdirSync(legacySessionsDir, { recursive: true });
      fs.writeFileSync(secondLegacyFile, 'second import should be skipped');

      const secondResult = utils.migrateLegacySessions();
      ...
      assert.ok(!fs.existsSync(path.join(canonicalDir, '2026-02-01-second-session.tmp')));
      assert.ok(fs.existsSync(secondLegacyFile), 'subsequent legacy files should be ignored after the one-time import');
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
      if (originalUserProfile === undefined) delete process.env.USERPROFILE;
  ...
  if (test('migrateLegacySessions does not write completion marker when import errors occur', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-session-error-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      const canonicalDir = utils.getSessionsDir();
      const legacySessionsDir = path.join(tempHome, '.claude', 'sessions');
      const legacyNestedDir = path.join(legacySessionsDir, 'nested');
      const blockingPath = path.join(canonicalDir, 'nested');

      fs.mkdirSync(legacyNestedDir, { recursive: true });
      fs.mkdirSync(canonicalDir, { recursive: true });
      fs.writeFileSync(path.join(legacyNestedDir, 'note.txt'), 'blocked');
      fs.writeFileSync(blockingPath, 'not a directory');

      const result = utils.migrateLegacySessions();

      assert.ok(result.errors.length > 0, 'expected at least one import error');
      assert.ok(!fs.existsSync(path.join(canonicalDir, '.claude-migration-complete')));
      assert.ok(fs.existsSync(path.join(legacyNestedDir, 'note.txt')), 'legacy file should remain for retry');
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
      if (originalUserProfile === undefined) delete process.env.USERPROFILE;
  ...
  if (test('migrateLegacySessions preserves legacy files when canonical path is a directory', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-session-type-conflict-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      const canonicalDir = utils.getSessionsDir();
      const legacySessionsDir = path.join(tempHome, '.claude', 'sessions');
      const legacyFile = path.join(legacySessionsDir, 'nested');
      const canonicalConflictDir = path.join(canonicalDir, 'nested');

      fs.mkdirSync(legacySessionsDir, { recursive: true });
      fs.mkdirSync(canonicalConflictDir, { recursive: true });
      fs.writeFileSync(legacyFile, 'legacy file should survive conflict');

      const result = utils.migrateLegacySessions();

      assert.ok(result.errors.length > 0, 'expected a type-conflict error');
      assert.ok(fs.existsSync(legacyFile), 'legacy file should remain for manual resolution');
      assert.ok(fs.statSync(canonicalConflictDir).isDirectory(), 'canonical directory should remain intact');
      assert.ok(!fs.existsSync(path.join(canonicalDir, '.claude-migration-complete')));
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
  ...
  if (test('migrateProjectSessions renames project-named session files and rewrites headers', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-project-session-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      const sessionsDir = utils.getSessionsDir();
      const oldRoot = path.join(tempHome, 'workspace', 'old-name');
      const newRoot = path.join(tempHome, 'workspace', 'new-name');
      fs.mkdirSync(sessionsDir, { recursive: true });

      const sourceFile = path.join(
        sessionsDir,
        `2026-04-07-${utils.sanitizeSessionId('old-name')}-session.tmp`
      );
      fs.writeFileSync(
        sourceFile,
        '# Session: 2026-04-07\n**Date:** 2026-04-07\n**Started:** 09:00\n**Last Updated:** 09:15\n**Project:** old-name\n**Branch:** main\n**Worktree:** ' +
          path.join(oldRoot, 'src') +
          '\n\n---\n\n## Current State\n\nLegacy context\n',
        'utf8'
      );

      const result = utils.migrateProjectSessions({
        currentProjectName: 'new-name',
        currentProjectRoot: newRoot,
        previousProjects: [{ name: 'old-name', root: oldRoot }]
      });

      const targetFile = path.join(
        sessionsDir,
        `2026-04-07-${utils.sanitizeSessionId('new-name')}-session.tmp`
      );
      const migrated = fs.readFileSync(targetFile, 'utf8');

      assert.strictEqual(result.migrated, 1);
      assert.ok(!fs.existsSync(sourceFile), 'old project-named session file should be removed');
      assert.ok(migrated.includes('**Project:** new-name'));
      assert.ok(migrated.includes(`**Worktree:** ${path.join(newRoot, 'src')}`));
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
      if (originalUserProfile === undefined) delete process.env.USERPROFILE;
  ...
  if (test('migrateProjectSessions removes old duplicates when the renamed target already exists', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-project-session-dupe-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      const sourceFile = path.join(sessionsDir, `2026-04-07-${utils.sanitizeSessionId('old-name')}-session.tmp`);
      const targetFile = path.join(sessionsDir, `2026-04-07-${utils.sanitizeSessionId('new-name')}-session.tmp`);
      fs.writeFileSync(sourceFile, `**Project:** old-name\n**Worktree:** ${oldRoot}\n`, 'utf8');
      fs.writeFileSync(targetFile, `**Project:** new-name\n**Worktree:** ${newRoot}\n`, 'utf8');
      ...
      assert.strictEqual(result.removedDuplicates, 1);
      assert.ok(!fs.existsSync(sourceFile), 'old duplicate should be removed');
      assert.strictEqual(fs.readFileSync(targetFile, 'utf8'), `**Project:** new-name\n**Worktree:** ${newRoot}\n`);
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
      if (originalUserProfile === undefined) delete process.env.USERPROFILE;
  ...
  if (test('migrateProjectSessions skips files whose headers do not match the renamed project', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-project-session-skip-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      fs.writeFileSync(
        sourceFile,
        `**Project:** unrelated-project\n**Worktree:** ${path.join(tempHome, 'elsewhere')}\n`,
        'utf8'
      );
      ...
      assert.strictEqual(result.removedDuplicates, 0);
      assert.ok(fs.existsSync(sourceFile), 'unrelated session should not be renamed');
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
      if (originalUserProfile === undefined) delete process.env.USERPROFILE;
  ...
  if (test('migrateProjectSessions rewrites session-id files whose headers match the renamed project', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-project-session-id-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      const sessionIdFile = path.join(sessionsDir, '2026-04-07-session-1234-session.tmp');
      fs.writeFileSync(
        sessionIdFile,
        `**Project:** old-name\n**Worktree:** ${path.join(oldRoot, 'packages', 'app')}\n`,
        'utf8'
      );
      ...
      const migrated = fs.readFileSync(sessionIdFile, 'utf8');
      assert.strictEqual(result.migrated, 1);
      assert.ok(migrated.includes('**Project:** new-name'));
      assert.ok(migrated.includes(`**Worktree:** ${path.join(newRoot, 'packages', 'app')}`));
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
      if (originalUserProfile === undefined) delete process.env.USERPROFILE;
  ...
  if (test('migrateProjectSessions rewrites project-only session-id files when worktree metadata is absent', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-project-session-project-only-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      const sessionIdFile = path.join(sessionsDir, '2026-04-07-session-project-only-session.tmp');
      fs.writeFileSync(sessionIdFile, '**Project:** old-name\n', 'utf8');
      ...
      assert.strictEqual(result.migrated, 1);
      assert.strictEqual(fs.readFileSync(sessionIdFile, 'utf8'), '**Project:** new-name\n');
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
      if (originalUserProfile === undefined) delete process.env.USERPROFILE;
  ...
  if (test('migrateProjectSessions renames headerless project-named files using the short-id fallback', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-project-session-headerless-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      const sourceFile = path.join(sessionsDir, `2026-04-07-${utils.sanitizeSessionId('old-name')}-session.tmp`);
      const targetFile = path.join(sessionsDir, `2026-04-07-${utils.sanitizeSessionId('new-name')}-session.tmp`);
      fs.writeFileSync(sourceFile, 'Legacy session without headers\n', 'utf8');
      ...
      assert.strictEqual(result.migrated, 1);
      assert.ok(!fs.existsSync(sourceFile), 'old headerless session file should be removed');
      assert.strictEqual(fs.readFileSync(targetFile, 'utf8'), 'Legacy session without headers\n');
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
      if (originalUserProfile === undefined) delete process.env.USERPROFILE;
  ...
  if (test('migrateProjectSessions skips same-name session-id files when worktree is outside the renamed project root', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-project-session-cross-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      const oldRoot = path.join(tempHome, 'workspace', 'old-name');
      const newRoot = path.join(tempHome, 'workspace', 'new-name');
      const unrelatedRoot = path.join(tempHome, 'archive', 'old-name');
      fs.mkdirSync(sessionsDir, { recursive: true });

      const sessionIdFile = path.join(sessionsDir, '2026-04-07-session-5678-session.tmp');
      const originalContent = `**Project:** old-name\n**Worktree:** ${path.join(unrelatedRoot, 'packages', 'app')}\n`;
      fs.writeFileSync(sessionIdFile, originalContent, 'utf8');
      ...
      assert.strictEqual(result.migrated, 0);
      assert.strictEqual(fs.readFileSync(sessionIdFile, 'utf8'), originalContent);
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
      if (originalUserProfile === undefined) delete process.env.USERPROFILE;
  ...
  if (test('migrateProjectSessions keeps case-only rename targets on case-insensitive filesystems', () => {
    const tempHome = path.join(utils.getTempDir(), `utils-project-session-case-${Date.now()}`);
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    ...
    try {
      ...
      const sessionsDir = utils.getSessionsDir();
      const projectRoot = path.join(tempHome, 'workspace', 'project');
      fs.mkdirSync(sessionsDir, { recursive: true });

      const sourceFile = path.join(sessionsDir, '2026-04-07-OldName-session.tmp');
      const targetFile = path.join(sessionsDir, '2026-04-07-oldname-session.tmp');
      fs.writeFileSync(sourceFile, `**Project:** OldName\n**Worktree:** ${projectRoot}\n`, 'utf8');
      ...
      const result = utils.migrateProjectSessions({
        ...
        previousProjects: [{ name: 'OldName', root: projectRoot }]
      });

      assert.strictEqual(result.removedDuplicates, 0);
      assert.strictEqual(result.errors.length, 0);
      assert.ok(fs.existsSync(sourceFile) || fs.existsSync(targetFile), 'case-only rename should keep a session file');
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
      if (originalUserProfile === undefined) delete process.env.USERPROFILE;
  ...
  if (test('reportLegacyMigrationErrors logs each unresolved issue only once per process', () => {
    const captured = [];
    const origError = console.error;
    console.error = (v) => { captured.push(v); };
    try {
      const result = {
        migrated: 0,
        removedDuplicates: 0,
        removedDirs: 0,
        errors: [
          { path: '/tmp/legacy-session.tmp', message: 'type conflict' }
        ]
      };

      utils.reportLegacyMigrationErrors(result, 'SessionStart');
      utils.reportLegacyMigrationErrors(result, 'SessionEnd');

      assert.strictEqual(captured.length, 2);
      assert.ok(captured[0].includes('[SessionStart] 1 legacy session migration issue(s) need manual resolution'));
      assert.ok(captured[1].includes('/tmp/legacy-session.tmp: type conflict'));
    } finally {
      console.error = origError;
    }
  })) passed++; else failed++;
  ...
  if (test('isGitRepo returns true in a git repo', () => {
    ...
    const tempRepo = fs.mkdtempSync(path.join(utils.getTempDir(), 'utils-test-git-repo-'));
  ...
  if (test('getLearnedSkillsDir returns path under Claude dir', () => {
    ...
    assert.ok(dir.includes('.factory'));
  ...
  if (test('findFiles recursive scan skips unreadable subdirectory silently', () => {
    ...
    const tmpDir = path.join(utils.getTempDir(), `efd-findfiles-r71-${Date.now()}`);
  ...
  if (test('findFiles skips broken symlinks that match the pattern', () => {
    ...
    const tmpDir = path.join(utils.getTempDir(), `efd-r84-findfiles-toctou-${Date.now()}`);
  ...
  if (test('replaceInFile with empty string replacement deletes matched text', () => {
    const tmpDir = path.join(utils.getTempDir(), `efd-r88-replace-empty-${Date.now()}`);
    fs.mkdirSync(tmpDir, { recursive: true });
  ...
  if (test('countInFile returns 0 for valid file with no pattern matches', () => {
    const tmpDir = path.join(utils.getTempDir(), `efd-r88-count-zero-${Date.now()}`);
    fs.mkdirSync(tmpDir, { recursive: true });
  ...
  if (test('countInFile with /apple|banana/ (alternation, no g) counts all matches', () => {
    const tmpDir = path.join(utils.getTempDir(), `efd-r95-alternation-${Date.now()}`);
    fs.mkdirSync(tmpDir, { recursive: true });
