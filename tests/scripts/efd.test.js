...
const SCRIPT = path.join(__dirname, '..', '..', 'scripts', 'efd.js');
...
function main() {
  ...
  const tests = [
    ['shows top-level help', () => {
      ...
      assert.match(result.stdout, /EFD selective-install CLI/);
    ...
    ['delegates lifecycle commands', () => {
      const homeDir = createTempDir('efd-cli-home-');
      const projectRoot = createTempDir('efd-cli-project-');
    ...
    ['delegates session-inspect command', () => {
      ...
      const result = runCli(['session-inspect', 'factory:latest'], {
        env: { HOME: homeDir },
      });
      ...
      assert.strictEqual(payload.workers[0].branch, 'feat/efd-cli');
    }],
