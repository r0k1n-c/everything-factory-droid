...
function runTests() {
  ...
  if (test('audits consumer projects from cwd instead of the EFD repo root', () => {
    const homeDir = createTempDir('harness-audit-home-');
    const projectRoot = createTempDir('harness-audit-project-');

    try {
      fs.mkdirSync(path.join(homeDir, '.factory', 'plugins', 'everything-factory-droid'), { recursive: true });
      fs.writeFileSync(
        path.join(homeDir, '.factory', 'plugins', 'everything-factory-droid', 'plugin.json'),
        JSON.stringify({ name: 'everything-factory-droid' }, null, 2)
      );
      ...
      fs.writeFileSync(path.join(projectRoot, '.factory', 'settings.json'), JSON.stringify({ hooks: ['PreToolUse'] }, null, 2));
      fs.writeFileSync(
