...
if (
  test('audit mode logs sanitized bash commands and preserves stdout', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efd-bash-log-'));
    const payload = {
      tool_input: {
    ...
    try {
      ...
      const logFile = path.join(homeDir, '.factory', 'bash-commands.log');
...
if (
  test('cost mode writes command metrics log', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efd-cost-log-'));
    const payload = {
      tool_input: {
        command: 'npm publish',
      },
    ...
    try {
      ...
      const logFile = path.join(homeDir, '.factory', 'cost-tracker.log');
      const logContent = fs.readFileSync(logFile, 'utf8');
