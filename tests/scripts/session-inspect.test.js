...
function runTests() {
  ...
  if (test('lists registered adapters', () => {
    ...
    assert.ok(payload.adapters.some(adapter => adapter.id === 'factory-history'));
    assert.ok(payload.adapters.some(adapter => adapter.id === 'dmux-tmux'));
  })) passed++; else failed++;

  if (test('prints canonical JSON for factory history targets', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efd-session-inspect-home-'));
    const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efd-session-inspect-recordings-'));
    ...
    try {
      ...
      const result = run(['factory:latest'], {
        env: {
          HOME: homeDir,
          EFD_SESSION_RECORDING_DIR: recordingDir
        }
      });
      ...
      assert.strictEqual(payload.adapterId, 'factory-history');
  ...
  if (test('supports explicit target types for structured registry routing', () => {
    ...
    try {
      ...
      const result = run(['latest', '--target-type', 'factory-history'], {
        env: { HOME: homeDir }
      });
      ...
      const payload = JSON.parse(result.stdout);
      assert.strictEqual(payload.adapterId, 'factory-history');
      assert.strictEqual(payload.session.sourceTarget.type, 'factory-history');
  ...
  if (test('writes snapshot JSON to disk when --write is provided', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efd-session-inspect-home-'));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), 'efd-session-inspect-out-'));
    const sessionsDir = path.join(homeDir, '.factory', 'session-data');
    fs.mkdirSync(sessionsDir, { recursive: true });
    ...
    try {
      ...
      const result = run(['factory:latest', '--write', outputPath], {
        env: { HOME: homeDir }
      });
      ...
      assert.strictEqual(written.adapterId, 'factory-history');
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('inspects skill health from recorded observations', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'efd-session-inspect-skills-'));
    const observationsDir = path.join(projectRoot, '.factory', 'efd', 'skills');
    fs.mkdirSync(observationsDir, { recursive: true });
    fs.writeFileSync(
      path.join(observationsDir, 'observations.jsonl'),
      [
        JSON.stringify({
          schemaVersion: 'efd.skill-observation.v1',
          observationId: 'obs-1',
    ...
    try {
      ...
      assert.strictEqual(result.code, 0, result.stderr);
      const payload = JSON.parse(result.stdout);
      assert.strictEqual(payload.schemaVersion, 'efd.skill-health.v1');
  ...
  if (test('proposes skill amendments through session-inspect', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'efd-session-inspect-amend-'));
    const observationsDir = path.join(projectRoot, '.factory', 'efd', 'skills');
    fs.mkdirSync(observationsDir, { recursive: true });
    fs.writeFileSync(
      path.join(observationsDir, 'observations.jsonl'),
    ...
    try {
      ...
      assert.strictEqual(result.code, 0, result.stderr);
      const payload = JSON.parse(result.stdout);
      assert.strictEqual(payload.schemaVersion, 'efd.skill-amendment-proposal.v1');
  ...
  if (test('builds skill evaluation scaffolding through session-inspect', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'efd-session-inspect-eval-'));
    const observationsDir = path.join(projectRoot, '.factory', 'efd', 'skills');
    fs.mkdirSync(observationsDir, { recursive: true });
    fs.writeFileSync(
      path.join(observationsDir, 'observations.jsonl'),
    ...
    try {
      ...
      assert.strictEqual(result.code, 0, result.stderr);
      const payload = JSON.parse(result.stdout);
      assert.strictEqual(payload.schemaVersion, 'efd.skill-evaluation.v1');
