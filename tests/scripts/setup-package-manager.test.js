...
function runTests() {
  ...
  if (test('detects env var override', () => {
    const result = run(['--detect'], { FACTORY_DROID_PACKAGE_MANAGER: 'pnpm' });
    assert.strictEqual(result.code, 0);
    assert.ok(result.stdout.includes('pnpm'));
  })) passed++; else failed++;
  ...
  if (test('--global npm writes config and succeeds', () => {
    ...
    try {
      ...
      const configPath = path.join(tmpDir, '.factory', 'package-manager.json');
  ...
  if (test('--global npm fails when HOME is not a directory', () => {
    ...
    // HOME=/dev/null causes ensureDir to throw ENOTDIR when creating ~/.factory/
  ...
  if (test('--project npm fails when CWD is read-only', () => {
    ...
    fs.mkdirSync(tmpDir, { recursive: true });
    try {
      // Make CWD read-only so .factory/ dir creation fails with EACCES
