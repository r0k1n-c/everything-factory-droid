...
_everything factory droid / research / security_

---

It's been a while since my last article now. Spent time working on building out the EFD devtooling ecosystem. One of the few hot but important topics during that stretch has been agent security.

Widespread adoption of open source agents is here. OpenClaw and others run about your computer. Continuous run harnesses like Factory Droid and Codex (using EFD) increase the surface area; and on February 25, 2026, Check Point Research published a Factory Droid disclosure that should have ended the "this could happen but won't / is overblown" phase of the conversation for good. With the tooling reaching critical mass, the gravity of exploits multiplies.
...
Video: Bad Rudi Exploit — good rudi (grok animated AI character for children) gets exploited with a prompt jailbreak after repeated attempts in order to reveal sensitive information. its a humorous example but nonetheless the possibilities go a lot further.
...
## Factory Droid CVEs (February 2026)

Check Point Research published the Factory Droid findings on February 25, 2026. The issues were reported between July and December 2025, then patched before publication.
...
>
> Hijacking Factory Droid users via poisoned config files with rogue hooks actions.
>
...
> _I hacked Factory Droid! It turns out "agentic" is just a fancy new way to get a shell. I achieved full RCE and hijacked organization API keys. CVE-2025-59536 | CVE-2026-21852_
...
Anthropic's own docs reflect that reality. Project settings live in `.factory/`. Project-scoped MCP servers live in `.mcp.json`. They are shared through source control. They are supposed to be guarded by a trust boundary. That trust boundary is exactly what attackers will go after.

## What Changed In The Last Year

This conversation moved fast in 2025 and early 2026.

Factory Droid had its repo-controlled hooks, MCP settings, and env-var trust paths tested publicly. Amazon Q Developer had a 2025 supply chain incident involving a malicious prompt payload in the VS Code extension, then a separate disclosure around overly broad GitHub token exposure in build infrastructure. Weak credential boundaries plus agent-adjacent tooling is an entrypoint for opportunists.
...
| Stat | Detail |
|------|--------|
| **CVSS 8.7** | Factory Droid hook / pre-trust execution issue: CVE-2025-59536 |
...
Root access is dangerous. Broad local access is dangerous. Long-lived credentials on the same machine are dangerous. "YOLO, Droid has me covered" is not the correct approach to take here. The answer is isolation.
...
Anthropic documents that Factory Droid loads memory at session start. So keep memory narrow:
...
Anthropic has hardened Factory Droid and published concrete security guidance around trust, permissions, MCP, memory, hooks, and isolated environments.
...
And if you are using EFD specifically, this is also the problem space I built AgentShield for: suspicious hooks, hidden prompt injection patterns, over-broad permissions, risky MCP config, secret exposure, and the stuff people absolutely will miss in manual review.
...
- Check Point Research, "Caught in the Hook: RCE and API Token Exfiltration Through Factory Droid Project Files" (February 25, 2026): [research.checkpoint.com](https://research.checkpoint.com/2026/rce-and-api-token-exfiltration-through-claude-code-project-files-cve-2025-59536/)
...
- Factory Droid docs, "Settings": [docs.factory.ai](https://docs.factory.ai/cli/configuration/settings.md)
- Factory Droid docs, "MCP": [docs.factory.ai](https://docs.factory.ai/cli/configuration/mcp.md)
- Factory Droid docs, "Security": [docs.factory.ai](https://docs.factory.ai/cli/account/security.md)
- Factory Droid docs, "Memory": [docs.factory.ai](https://docs.factory.ai/guides/power-user/memory-management.md)
...
> [The Shorthand Guide to Everything Factory Droid](https://x.com/affaanmustafa/status/2012378465664745795)
